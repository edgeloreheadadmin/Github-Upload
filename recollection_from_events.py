...
# Source : tools/recollection_from_events.py
...
DEFAULT_PROMPT_TEMPLATE = """### RECOLLECTION FROM EVENTS MODE (OPT-IN)
Recognize when recollection can be triggered by events.
- Identify event cues, sequences, and conditions that surface memories.
...
TOOL_PROMPT = (
    "Use `recollection_from_events` to recognize when recollection can occur "
    "from events. Provide `prompt` or `messages`, and optionally set "
    "`recollection_style`, `event_focus`, `show_steps`, and `max_triggers`."
)
...
class RecollectionEventsMessage(BaseModel):
    role: str
    content: str
...
class EventItem(BaseModel):
    ...
    source_type: str | None = Field(
        default=None, description="Source type (event, signal, trigger)."
    )
    event: str | None = Field(default=None, description="Event description.")
    sequence: str | None = Field(default=None, description="Event sequence.")
...
class RecollectionFromEventsArgs(BaseModel):
    prompt: str | None = Field(
        default=None, description="User prompt to solve."
    )
    messages: list[RecollectionEventsMessage] | None = Field(
    ...
    event_focus: str | None = Field(
        default=None, description="Event focus label."
    )
    ...
    items: list[EventItem] | None = Field(
        default=None, description="Event items."
    )
...
class RecollectionFromEventsResult(BaseModel):
    answer: str
    system_prompt: str
    messages: list[RecollectionEventsMessage]
    used_recollection_style: str
    used_event_focus: str
    show_steps: bool
    max_triggers: int
    template_source: str
...
class RecollectionFromEventsConfig(BaseToolConfig):
    ...
    default_event_focus: str = Field(
        default="events",
        description="Default event focus.",
    )
...
class RecollectionFromEventsState(BaseToolState):
    pass


class RecollectionFromEvents(
    BaseTool[
        RecollectionFromEventsArgs,
        RecollectionFromEventsResult,
        RecollectionFromEventsConfig,
        RecollectionFromEventsState,
    ],
    ToolUIData[
        RecollectionFromEventsArgs,
        RecollectionFromEventsResult,
    ...
    description: ClassVar[str] = (
        "Recognize recollection triggers from events using a prompt template."
    )
    ...
    async def run(
        self, args: RecollectionFromEventsArgs
    ) -> RecollectionFromEventsResult:
        warnings: list[str] = []
        errors: list[str] = []
        ...
        event_focus = (args.event_focus or self.config.default_event_focus).strip()
        if not event_focus:
            event_focus = "events"
        ...
        event_blocks = self._collect_events(args, warnings)
        messages = self._normalize_messages(args, system_prompt)
        messages = self._inject_events(messages, event_blocks)
        answer = self._call_llm(messages, args)

        return RecollectionFromEventsResult(
            answer=answer,
            system_prompt=system_prompt,
            messages=messages,
            used_recollection_style=recollection_style,
            used_event_focus=event_focus,
    ...
    def _validate_llm_settings(self, args: RecollectionFromEventsArgs) -> None:
    ...
    def _collect_events(
        self, args: RecollectionFromEventsArgs, warnings: list[str]
    ) -> list[EventBlock]:
        max_item = (
            args.max_item_chars
        ...
        sources = self._resolve_event_sources(args, warnings)
        blocks: list[EventBlock] = []
        total_chars = 0
        for source in sources:
            if total_chars >= max_total:
                warnings.append(
                    "max_item_total_chars reached; truncating event inputs."
                )
                break
        ...
        if not blocks:
            warnings.append("No event context provided.")
        return blocks
    ...
    def _load_event_item(self, item: EventItem) -> _EventSource:
    ...
    def _format_event_blocks(self, blocks: list[EventBlock]) -> str:
        if not blocks:
            return ""
        parts = ["Event context:"]
        for block in blocks:
            label = block.label or block.source_path or "event"
    ...
    def _inject_events(
        ...
        messages: list[RecollectionEventsMessage],
        blocks: list[EventBlock],
    ) -> list[RecollectionEventsMessage]:
        if not blocks:
            return messages
        event_text = self._format_event_blocks(blocks)
        for idx in range(len(messages) - 1, -1, -1):
            if messages[idx].role == "user":
                ...
                return messages
        messages.append(
            RecollectionEventsMessage(role="user", content=event_text)
        )
        return messages
    ...
    def _render_template(
        ...
        if "{event_focus}" in template:
            rendered = rendered.replace("{event_focus}", event_focus)
    ...
    def _normalize_messages(
        self, args: RecollectionFromEventsArgs, system_prompt: str
    ) -> list[RecollectionEventsMessage]:
        messages: list[RecollectionEventsMessage] = []
        if args.messages:
            for msg in args.messages:
                ...
                if not role:
                    role = "user"
                messages.append(RecollectionEventsMessage(role=role, content=content))
        elif args.prompt and args.prompt.strip():
            messages.append(
                RecollectionEventsMessage(role="user", content=args.prompt.strip())
            )
        else:
    ...
    def _call_llm(
        self,
        messages: list[RecollectionEventsMessage],
        args: RecollectionFromEventsArgs,
    ) -> str:
    ...
    def _resolve_model(self, args: RecollectionFromEventsArgs) -> str:
        return args.llm_model or self.config.llm_model

    @classmethod
    def get_call_display(cls, event: ToolCallEvent) -> ToolCallDisplay:
        if not isinstance(event.args, RecollectionFromEventsArgs):
            return ToolCallDisplay(summary="recollection_from_events")
        return ToolCallDisplay(
            summary="recollection_from_events",
            details={
                "recollection_style": event.args.recollection_style,
                "event_focus": event.args.event_focus,
    ...
    def get_result_display(cls, event: ToolResultEvent) -> ToolResultDisplay:
        if not isinstance(event.result, RecollectionFromEventsResult):
            return ToolResultDisplay(
                success=False, message=event.error or event.skip_reason or "No result"
            )
        message = "Recollection from events complete"
        if event.result.errors:
            message = "Recollection from events finished with errors"
        return ToolResultDisplay(
            ...
            details={
                ...
                "used_event_focus": event.result.used_event_focus,
                "show_steps": event.result.show_steps,
                "max_triggers": event.result.max_triggers,
                "template_source": event.result.template_source,
                "event_blocks": len(event.result.event_blocks),
    ...
    def get_status_text(cls) -> str:
        return "Recollection from events"