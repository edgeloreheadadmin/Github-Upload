...
# Source : tools/mood_sensation_management_control.py
...
DEFAULT_PROMPT_TEMPLATE = """### MOOD-ALIGNED SENSATION MANAGEMENT + CONTROL MODE (OPT-IN)
Manage and regulate sensation signals linked to mood before answering.
...
TOOL_PROMPT = (
    "Use `mood_sensation_management_control` to manage and regulate sensation signals tied to mood. "
...
class MoodSensationManagementMessage(BaseModel):
    role: str
    content: str


class MoodSensationContextItem(BaseModel):
...
class MoodSensationManagementControlArgs(BaseModel):
    prompt: str | None = Field(
        default=None, description="User prompt to solve."
    )
    messages: list[MoodSensationManagementMessage] | None = Field(
    ...
    items: list[MoodSensationContextItem] | None = Field(
        default=None, description="Mood-aligned sensation context items."
    )
...
class MoodSensationBlock(BaseModel):
    label: str | None
    source_type: str | None
    source_path: str | None
    content: str
...
class MoodSensationManagementControlResult(BaseModel):
    answer: str
    system_prompt: str
    messages: list[MoodSensationManagementMessage]
    used_mood_style: str
    used_control_focus: str
    show_steps: bool
    max_actions: int
    template_source: str
    mood_blocks: list[MoodSensationBlock]
    warnings: list[str]
    errors: list[str]
    llm_model: str


class MoodSensationManagementControlConfig(BaseToolConfig):
    ...
    prompt_path: Path | None = Field(
        ...
        / "core"
        / "prompts"
        / "mood_sensation_management_control.md",
...
class MoodSensationManagementControlState(BaseToolState):
    pass


class MoodSensationManagementControl(
    BaseTool[
        MoodSensationManagementControlArgs,
        MoodSensationManagementControlResult,
        MoodSensationManagementControlConfig,
        MoodSensationManagementControlState,
    ],
    ToolUIData[MoodSensationManagementControlArgs, MoodSensationManagementControlResult],
):
    description: ClassVar[str] = (
        "Manage and regulate sensation signals aligned to mood using a prompt template."
    )
    ...
    async def run(
        self, args: MoodSensationManagementControlArgs
    ) -> MoodSensationManagementControlResult:
        warnings: list[str] = []
        errors: list[str] = []
        ...
        return MoodSensationManagementControlResult(
            answer=answer,
    ...
    def _validate_llm_settings(self, args: MoodSensationManagementControlArgs) -> None:
    ...
    def _collect_mood_context(
        self, args: MoodSensationManagementControlArgs, warnings: list[str]
    ) -> list[MoodSensationBlock]:
        max_item = (
            args.max_item_chars
        ...
        blocks: list[MoodSensationBlock] = []
        total_chars = 0
    ...
    def _resolve_mood_sources(
        self, args: MoodSensationManagementControlArgs, warnings: list[str]
    ) -> list[_MoodSensationSource]:
        sources: list[_MoodSensationSource] = []
        if args.items:
    ...
    def _load_mood_item(self, item: MoodSensationContextItem) -> _MoodSensationSource:
    ...
    def _format_mood_text(self, item: MoodSensationContextItem) -> str:
        lines: list[str] = []
    ...
    def _format_mood_blocks(self, blocks: list[MoodSensationBlock]) -> str:
        if not blocks:
            return ""
        parts = ["Mood-aligned sensation context:"]
        for block in blocks:
    ...
    def _inject_mood_context(
        ...
        messages: list[MoodSensationManagementMessage],
        blocks: list[MoodSensationBlock],
    ) -> list[MoodSensationManagementMessage]:
        if not blocks:
            return messages
        ...
        for idx in range(len(messages) - 1, -1, -1):
            if messages[idx].role == "user":
                ...
                return messages
        messages.append(MoodSensationManagementMessage(role="user", content=mood_text))
        return messages
    ...
    def _normalize_messages(
        self, args: MoodSensationManagementControlArgs, system_prompt: str
    ) -> list[MoodSensationManagementMessage]:
        messages: list[MoodSensationManagementMessage] = []
        if args.messages:
            for msg in args.messages:
                ...
                if not role:
                    role = "user"
                messages.append(MoodSensationManagementMessage(role=role, content=content))
        elif args.prompt and args.prompt.strip():
            messages.append(
                MoodSensationManagementMessage(role="user", content=args.prompt.strip())
            )
        else:
        ...
        if system_prompt.strip():
            messages.insert(
                0,
                MoodSensationManagementMessage(
    ...
    def _call_llm(
        self,
        messages: list[MoodSensationManagementMessage],
        args: MoodSensationManagementControlArgs,
    ) -> str:
    ...
    def _resolve_model(self, args: MoodSensationManagementControlArgs) -> str:
        return args.llm_model or self.config.llm_model

    @classmethod
    def get_call_display(cls, event: ToolCallEvent) -> ToolCallDisplay:
        if not isinstance(event.args, MoodSensationManagementControlArgs):
            return ToolCallDisplay(summary="mood_sensation_management_control")
        return ToolCallDisplay(
            summary="mood_sensation_management_control",
            details={
    ...
    def get_result_display(cls, event: ToolResultEvent) -> ToolResultDisplay:
        if not isinstance(event.result, MoodSensationManagementControlResult):
            return ToolResultDisplay(
                success=False, message=event.error or event.skip_reason or "No result"
            )
        message = "Mood-aligned sensation management and control complete"
        if event.result.errors:
            message = "Mood-aligned sensation management and control finished with errors"
        return ToolResultDisplay(
    ...
    def get_status_text(cls) -> str:
        return "Mood-aligned sensation management and control"
