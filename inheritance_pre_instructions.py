...
# Source : tools/inheritance_pre_instructions.py
...
DEFAULT_PROMPT_TEMPLATE = """### INHERITANCE TRAITS MODE (OPT-IN)
Work with inherited traits, pre-instructions, and skill-tree effects.
- Identify inheritance paths, trait carriers, and passive effects.
...
TOOL_PROMPT = (
    "Use inheritance_pre_instructions for inherited traits, pre-instructions, or skill-tree effects. "
    "Provide prompt or messages, "
    "Optionally set Optionally set focus, show_steps, and max_results."
)


class InheritedTraitMessage(BaseModel):
    role: str
    content: str


class InheritedTraitArgs(BaseModel):
    prompt: str | None = Field(default=None, description="User prompt to solve.")
    messages: list[InheritedTraitMessage] | None = Field(
...
class InheritedTraitResult(BaseModel):
    answer: str
    system_prompt: str
    messages: list[InheritedTraitMessage]
    used_focus: str
    show_steps: bool
    max_results: int
    template_source: str
...
class InheritedTraitConfig(BaseToolConfig):
    ...
    default_focus: str = Field(
        default="Pass pre-instructions down to classes and subclasses via inheritance.",
    ...
    prompt_path: Path | None = Field(
        ...
        / "core"
        / "prompts"
        / "inheritance_pre_instructions.md",
        description="Optional path to a prompt template.",
    )
    prompt_max_chars: int = Field(default=8000, description="Maximum template characters to load.")


class InheritedTraitState(BaseToolState):
    pass


class InheritancePreInstructions(
    BaseTool[
        InheritedTraitArgs,
        InheritedTraitResult,
        InheritedTraitConfig,
        InheritedTraitState,
    ],
    ToolUIData[InheritedTraitArgs, InheritedTraitResult],
):
    description: ClassVar[str] = "Pass pre-instructions down to classes and subclasses via inheritance."

    @classmethod
    def get_tool_prompt(cls) -> str | None:
        return TOOL_PROMPT

    async def run(self, args: InheritedTraitArgs) -> InheritedTraitResult:
        warnings: list[str] = []
        errors: list[str] = []

        focus = (args.focus or self.config.default_focus).strip() or "inherited traits"
    ...
    def _validate_llm_settings(self, args: InheritedTraitArgs) -> None:
    ...
    def _normalize_messages(self, args: InheritedTraitArgs, system_prompt: str) -> list[InheritedTraitMessage]:
        messages: list[InheritedTraitMessage] = []
        if args.messages:
            for msg in args.messages:
                ...
                if content:
                    messages.append(InheritedTraitMessage(role=role, content=content))
        elif args.prompt and args.prompt.strip():
            messages.append(InheritedTraitMessage(role="user", content=args.prompt.strip()))
        else:
            raise ToolError("Provide prompt or messages.")
        if not messages:
            raise ToolError("No usable messages provided.")
        if system_prompt.strip():
            messages.insert(0, InheritedTraitMessage(role="system", content=system_prompt.strip()))
        return messages

    def _call_llm(self, messages: list[InheritedTraitMessage], args: InheritedTraitArgs) -> str:
    ...
    def _resolve_model(self, args: InheritedTraitArgs) -> str:
        return args.llm_model or self.config.llm_model

    @classmethod
    def get_call_display(cls, event: ToolCallEvent) -> ToolCallDisplay:
        if not isinstance(event.args, InheritedTraitArgs):
            return ToolCallDisplay(summary="inheritance_pre_instructions")
        return ToolCallDisplay(
            summary="inheritance_pre_instructions",
            details={
    ...
    def get_result_display(cls, event: ToolResultEvent) -> ToolResultDisplay:
        if not isinstance(event.result, InheritedTraitResult):
            return ToolResultDisplay(success=False, message=event.error or event.skip_reason or "No result")
        message = "Inheritance traits processing complete"
        if event.result.errors:
            message = "Inheritance traits processing finished with errors"
    ...
    def get_status_text(cls) -> str:
        return "Inheritance traits processing complete"