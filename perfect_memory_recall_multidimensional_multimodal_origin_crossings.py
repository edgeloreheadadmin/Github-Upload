...
# Source : tools/perfect_memory_recall_multidimensional_multimodal_origin_crossings.py
...
DEFAULT_PROMPT_TEMPLATE = """### MEMORY TIMELINE MODE (OPT-IN)
Work with memory timelines, retrieval locations, and event-based recall.
- Identify timeline anchors, crossing points, and retrieval cues.
...
TOOL_PROMPT = (
    "Use perfect_memory_recall_multidimensional_multimodal_origin_crossings to analyze memory timelines, retrieval locations, or recall logic. "
    "Provide prompt or messages, and optionally set focus, show_steps, and max_results."
)


class MemoryTimelineMessage(BaseModel):
    role: str
    content: str
...
class MemoryTimelineArgs(BaseModel):
    ...
    messages: list[MemoryTimelineMessage] | None = Field(
...
class MemoryTimelineResult(BaseModel):
    answer: str
    system_prompt: str
    messages: list[MemoryTimelineMessage]
    used_focus: str
    show_steps: bool
    max_results: int
    template_source: str
...
class MemoryTimelineConfig(BaseToolConfig):
    ...
    default_focus: str = Field(
        default="Perfect memory recall using multi-dimensional and multi-modal timelines with origin-based locations and crossing points.",
    ...
    prompt_path: Path | None = Field(
        ...
        / "core"
        / "prompts"
        / "perfect_memory_recall_multidimensional_multimodal_origin_crossings.md",
        description="Optional path to a prompt template.",
    )
    prompt_max_chars: int = Field(default=8000, description="Maximum template characters to load.")


class MemoryTimelineState(BaseToolState):
    pass


class PerfectMemoryRecallMultidimensionalMultimodalOriginCrossings(
    BaseTool[
        MemoryTimelineArgs,
    ...
    ToolUIData[MemoryTimelineArgs, MemoryTimelineResult],
):
    description: ClassVar[str] = "Perfect memory recall using multi-dimensional and multi-modal timelines with origin-based locations and crossing points."

    @classmethod
    def get_tool_prompt(cls) -> str | None:
        return TOOL_PROMPT

    async def run(self, args: MemoryTimelineArgs) -> MemoryTimelineResult:
        warnings: list[str] = []
        errors: list[str] = []
    ...
    def _validate_llm_settings(self, args: MemoryTimelineArgs) -> None:
    ...
    def _normalize_messages(self, args: MemoryTimelineArgs, system_prompt: str) -> list[MemoryTimelineMessage]:
        messages: list[MemoryTimelineMessage] = []
        if args.messages:
            for msg in args.messages:
                ...
                if content:
                    messages.append(MemoryTimelineMessage(role=role, content=content))
        elif args.prompt and args.prompt.strip():
            messages.append(MemoryTimelineMessage(role="user", content=args.prompt.strip()))
        else:
            raise ToolError("Provide prompt or messages.")
        if not messages:
            raise ToolError("No usable messages provided.")
        if system_prompt.strip():
            messages.insert(0, MemoryTimelineMessage(role="system", content=system_prompt.strip()))
        return messages

    def _call_llm(self, messages: list[MemoryTimelineMessage], args: MemoryTimelineArgs) -> str:
    ...
    def _resolve_model(self, args: MemoryTimelineArgs) -> str:
        return args.llm_model or self.config.llm_model

    @classmethod
    def get_call_display(cls, event: ToolCallEvent) -> ToolCallDisplay:
        if not isinstance(event.args, MemoryTimelineArgs):
            return ToolCallDisplay(summary="perfect_memory_recall_multidimensional_multimodal_origin_crossings")
        return ToolCallDisplay(
            summary="perfect_memory_recall_multidimensional_multimodal_origin_crossings",
            details={
    ...
    def get_result_display(cls, event: ToolResultEvent) -> ToolResultDisplay:
        if not isinstance(event.result, MemoryTimelineResult):
            return ToolResultDisplay(success=False, message=event.error or event.skip_reason or "No result")
        message = "Memory timeline processing complete"
        if event.result.errors:
            message = "Memory timeline processing finished with errors"
    ...
    def get_status_text(cls) -> str:
        return "Memory timeline processing complete"