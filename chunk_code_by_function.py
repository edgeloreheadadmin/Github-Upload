...
FUNCTION_KEYWORDS = {"def", "function", "fn", "func"}
FUNCTION_PATTERN = re.compile(
    r"\b(?:(?:async)\s+)?(def|function|fn|func)\b\s*\*?\s+([A-Za-z_][A-Za-z0-9_]*)"
)
GO_FUNC_PATTERN = re.compile(r"\bfunc\b\s*\([^)]*\)\s*([A-Za-z_][A-Za-z0-9_]*)")
...
class ChunkCodeByFunctionConfig(BaseToolConfig):
    permission: ToolPermission = ToolPermission.ALWAYS
    max_input_bytes: int = Field(
        default=5_000_000, description="Maximum input size in bytes."
    )
    max_chunk_bytes: int = Field(
        default=500_000, description="Maximum function chunk size in bytes."
    )
    max_chunks: int = Field(
        default=200, description="Maximum number of function chunks to return."
    )
    default_mode: str = Field(default="auto", description="auto, indent, brace.")
    include_nested: bool = Field(
        default=True, description="Include nested functions in results."
    )
    default_function_keywords: list[str] = Field(
        default=sorted(FUNCTION_KEYWORDS),
        description="Keywords treated as function declarations.",
    )


class ChunkCodeByFunctionState(BaseToolState):
    pass


class ChunkCodeByFunctionArgs(BaseModel):
    ...
    include_nested: bool | None = Field(
        default=None, description="Include nested functions."
    )
    function_keywords: list[str] | None = Field(
        default=None, description="Override function keywords to detect."
    )
...
class ChunkCodeByFunctionResult(BaseModel):
    mode: str
    include_nested: bool
...
class ChunkCodeByFunction(
    BaseTool[
        ChunkCodeByFunctionArgs,
        ChunkCodeByFunctionResult,
        ChunkCodeByFunctionConfig,
        ChunkCodeByFunctionState,
    ],
    ToolUIData[ChunkCodeByFunctionArgs, ChunkCodeByFunctionResult],
):
    description: ClassVar[str] = "Chunk a script by each function definition."

    async def run(self, args: ChunkCodeByFunctionArgs) -> ChunkCodeByFunctionResult:
        content, source_path = self._load_content(args)
        if not content:
            return ChunkCodeByFunctionResult(
                mode="auto",
                include_nested=True,
                chunks=[],
                count=0,
        ...
        match mode:
            case "indent":
                chunks = self._find_functions_indent(content, keywords, include_nested)
            case "brace":
                chunks = self._find_functions_brace(content, keywords, include_nested)
            case _:
    ...
    def _load_content(
        self, args: ChunkCodeByFunctionArgs
    ) -> tuple[str, Path | None]:
    ...
    def _resolve_mode(
        self, args: ChunkCodeByFunctionArgs, source_path: Path | None
    ) -> str:
    ...
    def _resolve_keywords(self, args: ChunkCodeByFunctionArgs) -> list[str]:
        raw = (
            args.function_keywords
            if args.function_keywords is not None
            else self.config.default_function_keywords
        )
        keywords = []
        for value in raw:
            if not value:
        ...
        if not keywords:
            raise ToolError("function_keywords must not be empty.")
        return sorted(set(keywords))

    def _validate_chunk_sizes(self, chunks: list[FunctionChunk]) -> None:
    ...
    def _normalize_chunk_order(self, chunks: list[FunctionChunk]) -> list[FunctionChunk]:
    ...
    def _find_functions_indent(
        ...
        for idx, line in enumerate(lines, start=1):
            ...
            if match := self._detect_function(stripped, keyword_set):
                kind, name = match
                depth = len(stack) + 1
                stack.append(
                    _FunctionEntry(
    ...
    def _find_functions_brace(
        ...
        chunks: list[FunctionChunk] = []
        stack: list[_FunctionEntry] = []
        pending: _FunctionEntry | None = None
        keyword_set = set(keywords)
        brace_depth = 0
    ...
    def _build_chunk(
        self, entry: _FunctionEntry, lines: list[str], end_line: int, index: int
    ) -> FunctionChunk:
    ...
    def _detect_function(
        ...
        matches = list(FUNCTION_PATTERN.finditer(line))
        if matches:
            match = matches[-1]
        ...
        if "func" in keywords:
            if match := GO_FUNC_PATTERN.search(line):
    ...
    def get_call_display(cls, event: ToolCallEvent) -> ToolCallDisplay:
        if not isinstance(event.args, ChunkCodeByFunctionArgs):
            return ToolCallDisplay(summary="chunk_code_by_function")

        summary = "chunk_code_by_function"
        return ToolCallDisplay(
            ...
            details={
                ...
                "function_keywords": event.args.function_keywords,
    ...
    def get_result_display(cls, event: ToolResultEvent) -> ToolResultDisplay:
        if not isinstance(event.result, ChunkCodeByFunctionResult):
            return ToolResultDisplay(
                success=False, message=event.error or event.skip_reason or "No result"
            )

        message = f"Found {event.result.count} function chunk(s)"
        warnings: list[str] = []
