...
# Source : tools/memory_event_timeline_allocation_processing_version.py
...
DEFAULT_PROMPT_TEMPLATE = """### EVENT/TIMELINE SPATIAL MODE (OPT-IN)
Work with event-based timeline classification and spatial origin recognition.
- Identify event anchors, timeline segments, and spatial origin cues.
...
TOOL_PROMPT = (
    "Use memory_event_timeline_allocation_processing_version for event/timeline classification or spatial origin recognition. "
    "Provide prompt or messages, and optionally set focus, show_steps, and max_results."
)


class EventTimelineMessage(BaseModel):
    role: str
    content: str


class EventTimelineArgs(BaseModel):
    prompt: str | None = Field(default=None, description="User prompt to solve.")
    messages: list[EventTimelineMessage] | None = Field(
...
class EventTimelineResult(BaseModel):
    answer: str
    system_prompt: str
    messages: list[EventTimelineMessage]
    used_focus: str
    show_steps: bool
    max_results: int
    template_source: str
...
class EventTimelineConfig(BaseToolConfig):
    ...
    default_focus: str = Field(
        default="Memory allocation and processing within events in a timeline.",
    ...
    prompt_path: Path | None = Field(
        ...
        / "core"
        / "prompts"
        / "memory_event_timeline_allocation_processing_version.md",
        description="Optional path to a prompt template.",
    )
    prompt_max_chars: int = Field(default=8000, description="Maximum template characters to load.")


class EventTimelineState(BaseToolState):
    pass


class MemoryEventTimelineAllocationProcessingVersion(
    BaseTool[
        EventTimelineArgs,
        EventTimelineResult,
        EventTimelineConfig,
    ...
    ToolUIData[EventTimelineArgs, EventTimelineResult],
):
    description: ClassVar[str] = "Memory allocation and processing within events in a timeline."

    @classmethod
    def get_tool_prompt(cls) -> str | None:
        return TOOL_PROMPT

    async def run(self, args: EventTimelineArgs) -> EventTimelineResult:
        warnings: list[str] = []
        errors: list[str] = []

        focus = (args.focus or self.config.default_focus).strip() or "event timeline"
    ...
    def _validate_llm_settings(self, args: EventTimelineArgs) -> None:
    ...
    def _normalize_messages(self, args: EventTimelineArgs, system_prompt: str) -> list[EventTimelineMessage]:
        messages: list[EventTimelineMessage] = []
        if args.messages:
            for msg in args.messages:
                ...
                if content:
                    messages.append(EventTimelineMessage(role=role, content=content))
        elif args.prompt and args.prompt.strip():
            messages.append(EventTimelineMessage(role="user", content=args.prompt.strip()))
        else:
            raise ToolError("Provide prompt or messages.")
        if not messages:
            raise ToolError("No usable messages provided.")
        if system_prompt.strip():
            messages.insert(0, EventTimelineMessage(role="system", content=system_prompt.strip()))
        return messages

    def _call_llm(self, messages: list[EventTimelineMessage], args: EventTimelineArgs) -> str:
    ...
    def _resolve_model(self, args: EventTimelineArgs) -> str:
        return args.llm_model or self.config.llm_model

    @classmethod
    def get_call_display(cls, event: ToolCallEvent) -> ToolCallDisplay:
        if not isinstance(event.args, EventTimelineArgs):
            return ToolCallDisplay(summary="memory_event_timeline_allocation_processing_version")
        return ToolCallDisplay(
            summary="memory_event_timeline_allocation_processing_version",
            details={
    ...
    def get_result_display(cls, event: ToolResultEvent) -> ToolResultDisplay:
        if not isinstance(event.result, EventTimelineResult):
            return ToolResultDisplay(success=False, message=event.error or event.skip_reason or "No result")
        message = "Event/timeline processing complete"
        if event.result.errors:
            message = "Event/timeline processing finished with errors"
    ...
    def get_status_text(cls) -> str:
        return "Event/timeline processing complete"