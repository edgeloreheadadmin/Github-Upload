...
# Source : tools/memory_pool_reservoir_retrieval_recognition_version.py
...
DEFAULT_PROMPT_TEMPLATE = """### MEMORY INFRA MODE (OPT-IN)
Work with aged memory processes, pools/reservoirs, energy, and caching.
- Identify storage anchors, retrieval cues, and energy/caching signals.
...
TOOL_PROMPT = (
    "Use memory_pool_reservoir_retrieval_recognition_version for memory infrastructure processes. "
    "Provide prompt or messages, and optionally set focus, show_steps, and max_results."
)


class MemoryInfraMessage(BaseModel):
    role: str
    content: str


class MemoryInfraArgs(BaseModel):
    prompt: str | None = Field(default=None, description="User prompt to solve.")
    messages: list[MemoryInfraMessage] | None = Field(
...
class MemoryInfraResult(BaseModel):
    answer: str
    system_prompt: str
    messages: list[MemoryInfraMessage]
    used_focus: str
    show_steps: bool
    max_results: int
    template_source: str
...
class MemoryInfraConfig(BaseToolConfig):
    ...
    default_focus: str = Field(
        default="Memory pools/reservoirs for stored data retrieval and recognition.",
    ...
    prompt_path: Path | None = Field(
        ...
        / "core"
        / "prompts"
        / "memory_pool_reservoir_retrieval_recognition_version.md",
        description="Optional path to a prompt template.",
    )
    prompt_max_chars: int = Field(default=8000, description="Maximum template characters to load.")


class MemoryInfraState(BaseToolState):
    pass


class MemoryPoolReservoirRetrievalRecognitionVersion(
    BaseTool[
        MemoryInfraArgs,
        MemoryInfraResult,
        MemoryInfraConfig,
    ...
    ToolUIData[MemoryInfraArgs, MemoryInfraResult],
):
    description: ClassVar[str] = "Memory pools/reservoirs for stored data retrieval and recognition."

    @classmethod
    def get_tool_prompt(cls) -> str | None:
        return TOOL_PROMPT

    async def run(self, args: MemoryInfraArgs) -> MemoryInfraResult:
        warnings: list[str] = []
        errors: list[str] = []

        focus = (args.focus or self.config.default_focus).strip() or "memory infrastructure"
    ...
    def _validate_llm_settings(self, args: MemoryInfraArgs) -> None:
    ...
    def _normalize_messages(self, args: MemoryInfraArgs, system_prompt: str) -> list[MemoryInfraMessage]:
        messages: list[MemoryInfraMessage] = []
        if args.messages:
            for msg in args.messages:
                ...
                if content:
                    messages.append(MemoryInfraMessage(role=role, content=content))
        elif args.prompt and args.prompt.strip():
            messages.append(MemoryInfraMessage(role="user", content=args.prompt.strip()))
        else:
            raise ToolError("Provide prompt or messages.")
        if not messages:
            raise ToolError("No usable messages provided.")
        if system_prompt.strip():
            messages.insert(0, MemoryInfraMessage(role="system", content=system_prompt.strip()))
        return messages

    def _call_llm(self, messages: list[MemoryInfraMessage], args: MemoryInfraArgs) -> str:
    ...
    def _resolve_model(self, args: MemoryInfraArgs) -> str:
        return args.llm_model or self.config.llm_model

    @classmethod
    def get_call_display(cls, event: ToolCallEvent) -> ToolCallDisplay:
        if not isinstance(event.args, MemoryInfraArgs):
            return ToolCallDisplay(summary="memory_pool_reservoir_retrieval_recognition_version")
        return ToolCallDisplay(
            summary="memory_pool_reservoir_retrieval_recognition_version",
            details={
    ...
    def get_result_display(cls, event: ToolResultEvent) -> ToolResultDisplay:
        if not isinstance(event.result, MemoryInfraResult):
            return ToolResultDisplay(success=False, message=event.error or event.skip_reason or "No result")
        message = "Memory infrastructure processing complete"
        if event.result.errors:
            message = "Memory infrastructure processing finished with errors"
    ...
    def get_status_text(cls) -> str:
        return "Memory infrastructure processing complete"