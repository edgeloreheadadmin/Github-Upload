...
# Source : tools/memory_linkage_correlations.py
...
DEFAULT_PROMPT_TEMPLATE = """### MEMORY LINKAGE CORRELATIONS MODE (OPT-IN)
Correlate linked memories and identify relationship patterns.
...
TOOL_PROMPT = (
    "Use `memory_linkage_correlations` to correlate linked memories. Provide "
    "`prompt` or `messages`, and optionally set `focus`, `show_steps`, and "
    "`max_correlations`."
)


class MemoryCorrelationMessage(BaseModel):
    role: str
    content: str


class MemoryCorrelationArgs(BaseModel):
    prompt: str | None = Field(
        default=None, description="User prompt to solve."
    )
    messages: list[MemoryCorrelationMessage] | None = Field(
...
class MemoryCorrelationResult(BaseModel):
    answer: str
    system_prompt: str
    messages: list[MemoryCorrelationMessage]
    used_focus: str
    show_steps: bool
...
class MemoryCorrelationConfig(BaseToolConfig):
    ...
    default_focus: str = Field(
        default="Memory linkage correlations.",
    ...
    prompt_path: Path | None = Field(
        ...
        / "core"
        / "prompts"
        / "memory_linkage_correlations.md",
...
class MemoryCorrelationState(BaseToolState):
    pass


class MemoryLinkageCorrelations(
    BaseTool[
        MemoryCorrelationArgs,
        MemoryCorrelationResult,
        MemoryCorrelationConfig,
        MemoryCorrelationState,
    ],
    ToolUIData[MemoryCorrelationArgs, MemoryCorrelationResult],
):
    ...
    async def run(
        self, args: MemoryCorrelationArgs
    ) -> MemoryCorrelationResult:
        warnings: list[str] = []
        errors: list[str] = []
        ...
        if not focus:
            focus = "memory linkage correlations"
    ...
    def _validate_llm_settings(self, args: MemoryCorrelationArgs) -> None:
    ...
    def _normalize_messages(
        self, args: MemoryCorrelationArgs, system_prompt: str
    ) -> list[MemoryCorrelationMessage]:
        messages: list[MemoryCorrelationMessage] = []
        if args.messages:
            for msg in args.messages:
                ...
                if not role:
                    role = "user"
                messages.append(
                    MemoryCorrelationMessage(role=role, content=content)
                )
        elif args.prompt and args.prompt.strip():
            messages.append(
                MemoryCorrelationMessage(
    ...
    def _resolve_model(self, args: MemoryCorrelationArgs) -> str:
        return args.llm_model or self.config.llm_model

    @classmethod
    def get_call_display(cls, event: ToolCallEvent) -> ToolCallDisplay:
        if not isinstance(event.args, MemoryCorrelationArgs):
            return ToolCallDisplay(summary="memory_linkage_correlations")
        return ToolCallDisplay(
            summary="memory_linkage_correlations",
            details={
    ...
    def get_result_display(
        ...
        if not isinstance(event.result, MemoryCorrelationResult):
            return ToolResultDisplay(
                success=False,
                message=event.error or event.skip_reason or "No result",
            )
        message = "Memory linkage correlations complete"
        if event.result.errors:
            message = "Memory linkage correlations finished with errors"
        return ToolResultDisplay(
    ...
    def get_status_text(cls) -> str:
        return "Memory linkage correlations"