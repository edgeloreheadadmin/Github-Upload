...
# Source : tools/memory_spatial_event_recollection_processing.py
...
DEFAULT_PROMPT_TEMPLATE = """### SPATIAL MEMORY EVENT MODE (OPT-IN)
Work with spatial events, origin recognition, and spatial recollection.
- Identify spatial anchors, event origins, and retrieval cues.
...
TOOL_PROMPT = (
    "Use memory_spatial_event_recollection_processing to process spatial memory events and origin locations. "
    "Provide prompt or messages, and optionally set focus, show_steps, and max_results."
)


class SpatialMemoryMessage(BaseModel):
    role: str
    content: str


class SpatialMemoryArgs(BaseModel):
    prompt: str | None = Field(default=None, description="User prompt to solve.")
    messages: list[SpatialMemoryMessage] | None = Field(
...
class SpatialMemoryResult(BaseModel):
    answer: str
    system_prompt: str
    messages: list[SpatialMemoryMessage]
    used_focus: str
    show_steps: bool
    max_results: int
    template_source: str
...
class SpatialMemoryConfig(BaseToolConfig):
    ...
    default_focus: str = Field(
        default="Allocate and process memories within events in spatial recollection.",
    ...
    prompt_path: Path | None = Field(
        ...
        / "core"
        / "prompts"
        / "memory_spatial_event_recollection_processing.md",
        description="Optional path to a prompt template.",
    )
    prompt_max_chars: int = Field(default=8000, description="Maximum template characters to load.")


class SpatialMemoryState(BaseToolState):
    pass


class MemorySpatialEventRecollectionProcessing(
    BaseTool[
        SpatialMemoryArgs,
        SpatialMemoryResult,
        SpatialMemoryConfig,
    ...
    ToolUIData[SpatialMemoryArgs, SpatialMemoryResult],
):
    description: ClassVar[str] = "Allocate and process memories within events for spatial recollection."

    @classmethod
    def get_tool_prompt(cls) -> str | None:
        return TOOL_PROMPT

    async def run(self, args: SpatialMemoryArgs) -> SpatialMemoryResult:
        warnings: list[str] = []
        errors: list[str] = []
    ...
    def _validate_llm_settings(self, args: SpatialMemoryArgs) -> None:
    ...
    def _normalize_messages(self, args: SpatialMemoryArgs, system_prompt: str) -> list[SpatialMemoryMessage]:
        messages: list[SpatialMemoryMessage] = []
        if args.messages:
            for msg in args.messages:
                ...
                if content:
                    messages.append(SpatialMemoryMessage(role=role, content=content))
        elif args.prompt and args.prompt.strip():
            messages.append(SpatialMemoryMessage(role="user", content=args.prompt.strip()))
        else:
            raise ToolError("Provide prompt or messages.")
        if not messages:
            raise ToolError("No usable messages provided.")
        if system_prompt.strip():
            messages.insert(0, SpatialMemoryMessage(role="system", content=system_prompt.strip()))
        return messages

    def _call_llm(self, messages: list[SpatialMemoryMessage], args: SpatialMemoryArgs) -> str:
    ...
    def _resolve_model(self, args: SpatialMemoryArgs) -> str:
        return args.llm_model or self.config.llm_model

    @classmethod
    def get_call_display(cls, event: ToolCallEvent) -> ToolCallDisplay:
        if not isinstance(event.args, SpatialMemoryArgs):
            return ToolCallDisplay(summary="memory_spatial_event_recollection_processing")
        return ToolCallDisplay(
            summary="memory_spatial_event_recollection_processing",
            details={
    ...
    def get_result_display(cls, event: ToolResultEvent) -> ToolResultDisplay:
        if not isinstance(event.result, SpatialMemoryResult):
