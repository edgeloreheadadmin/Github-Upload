...
# Source : tools/inheritance_looped_connections_structs.py
...
DEFAULT_PROMPT_TEMPLATE = """### INHERITANCE / HEALING MODE (OPT-IN)
Work with inheritance structures or healing/purification guidance.
- Identify structural anchors, recursion links, and recovery steps.
...
TOOL_PROMPT = (
    "Use inheritance_looped_connections_structs for inheritance structures or healing/purification support. "
    "Provide prompt or messages, focus, show_steps, and max_results."
)


class InheritanceHealingMessage(BaseModel):
    role: str
    content: str


class InheritanceHealingArgs(BaseModel):
    prompt: str | None = Field(default=None, description="User prompt to solve.")
    messages: list[InheritanceHealingMessage] | None = Field(
...
class InheritanceHealingResult(BaseModel):
    answer: str
    system_prompt: str
    messages: list[InheritanceHealingMessage]
    used_focus: str
    show_steps: bool
    max_results: int
    template_source: str
...
class InheritanceHealingConfig(BaseToolConfig):
    ...
    default_focus: str = Field(
        default="Connect inheritance with looped connections and class structs.",
    ...
    prompt_path: Path | None = Field(
        ...
        / "core"
        / "prompts"
        / "inheritance_looped_connections_structs.md",
        description="Optional path to a prompt template.",
    )
    prompt_max_chars: int = Field(default=8000, description="Maximum template characters to load.")


class InheritanceHealingState(BaseToolState):
    pass


class InheritanceLoopedConnectionsStructs(
    BaseTool[
        InheritanceHealingArgs,
        InheritanceHealingResult,
        InheritanceHealingConfig,
        InheritanceHealingState,
    ],
    ToolUIData[InheritanceHealingArgs, InheritanceHealingResult],
):
    description: ClassVar[str] = "Connect inheritance with looped connections and class structs."

    @classmethod
    def get_tool_prompt(cls) -> str | None:
        return TOOL_PROMPT

    async def run(self, args: InheritanceHealingArgs) -> InheritanceHealingResult:
        warnings: list[str] = []
        errors: list[str] = []
        ...
        return InheritanceHealingResult(
            answer=answer,
    ...
    def _validate_llm_settings(self, args: InheritanceHealingArgs) -> None:
    ...
    def _normalize_messages(self, args: InheritanceHealingArgs, system_prompt: str) -> list[InheritanceHealingMessage]:
        messages: list[InheritanceHealingMessage] = []
        if args.messages:
            for msg in args.messages:
                ...
                if content:
                    messages.append(InheritanceHealingMessage(role=role, content=content))
        elif args.prompt and args.prompt.strip():
            messages.append(InheritanceHealingMessage(role="user", content=args.prompt.strip()))
        else:
            raise ToolError("Provide prompt or messages.")
        if not messages:
            raise ToolError("No usable messages provided.")
        if system_prompt.strip():
            messages.insert(0, InheritanceHealingMessage(role="system", content=system_prompt.strip()))
        return messages

    def _call_llm(self, messages: list[InheritanceHealingMessage], args: InheritanceHealingArgs) -> str:
    ...
    def _resolve_model(self, args: InheritanceHealingArgs) -> str:
        return args.llm_model or self.config.llm_model

    @classmethod
    def get_call_display(cls, event: ToolCallEvent) -> ToolCallDisplay:
        if not isinstance(event.args, InheritanceHealingArgs):
            return ToolCallDisplay(summary="inheritance_looped_connections_structs")
        return ToolCallDisplay(
            summary="inheritance_looped_connections_structs",
            details={
    ...
    def get_result_display(cls, event: ToolResultEvent) -> ToolResultDisplay:
        if not isinstance(event.result, InheritanceHealingResult):
            return ToolResultDisplay(success=False, message=event.error or event.skip_reason or "No result")
        message = "Inheritance/healing processing complete"
        if event.result.errors:
            message = "Inheritance/healing processing finished with errors"
    ...
    def get_status_text(cls) -> str:
        return "Inheritance/healing processing complete"