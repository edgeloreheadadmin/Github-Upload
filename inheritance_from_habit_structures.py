...
# Source : tools/inheritance_from_habit_structures.py
...
DEFAULT_PROMPT_TEMPLATE = """### BACKEND PROCESS / HABIT MODE (OPT-IN)
Work with equation-backed processes, multi-domain links, or habit filtering.
- Identify process anchors, domain boundaries, and inclusion/exclusion gates.
...
TOOL_PROMPT = (
    "Use inheritance_from_habit_structures for equation-backed processes, multi-domain links, or habit filtering. "
    "Provide prompt or messages, focus, show_steps, and max_results."
)


class BackendHabitMessage(BaseModel):
    role: str
    content: str


class BackendHabitArgs(BaseModel):
    prompt: str | None = Field(default=None, description="User prompt to solve.")
    messages: list[BackendHabitMessage] | None = Field(
...
class BackendHabitResult(BaseModel):
    answer: str
    system_prompt: str
    messages: list[BackendHabitMessage]
    used_focus: str
    show_steps: bool
    max_results: int
    template_source: str
...
class BackendHabitConfig(BaseToolConfig):
    ...
    default_focus: str = Field(
        default="Form inheritance from habit structures and learning formations.",
    ...
    prompt_path: Path | None = Field(
        ...
        / "core"
        / "prompts"
        / "inheritance_from_habit_structures.md",
        description="Optional path to a prompt template.",
    )
    prompt_max_chars: int = Field(default=8000, description="Maximum template characters to load.")


class BackendHabitState(BaseToolState):
    pass


class InheritanceFromHabitStructures(
    BaseTool[
        BackendHabitArgs,
        BackendHabitResult,
        BackendHabitConfig,
    ...
    ToolUIData[BackendHabitArgs, BackendHabitResult],
):
    description: ClassVar[str] = "Form inheritance from habit structures and learning formations for synaptic changes."

    @classmethod
    def get_tool_prompt(cls) -> str | None:
        return TOOL_PROMPT

    async def run(self, args: BackendHabitArgs) -> BackendHabitResult:
        warnings: list[str] = []
        errors: list[str] = []

        focus = (args.focus or self.config.default_focus).strip() or "backend processes"
    ...
    def _validate_llm_settings(self, args: BackendHabitArgs) -> None:
    ...
    def _normalize_messages(self, args: BackendHabitArgs, system_prompt: str) -> list[BackendHabitMessage]:
        messages: list[BackendHabitMessage] = []
        if args.messages:
            for msg in args.messages:
                ...
                if content:
                    messages.append(BackendHabitMessage(role=role, content=content))
        elif args.prompt and args.prompt.strip():
            messages.append(BackendHabitMessage(role="user", content=args.prompt.strip()))
        else:
            raise ToolError("Provide prompt or messages.")
        if not messages:
            raise ToolError("No usable messages provided.")
        if system_prompt.strip():
            messages.insert(0, BackendHabitMessage(role="system", content=system_prompt.strip()))
        return messages

    def _call_llm(self, messages: list[BackendHabitMessage], args: BackendHabitArgs) -> str:
    ...
    def _resolve_model(self, args: BackendHabitArgs) -> str:
        return args.llm_model or self.config.llm_model

    @classmethod
    def get_call_display(cls, event: ToolCallEvent) -> ToolCallDisplay:
        if not isinstance(event.args, BackendHabitArgs):
            return ToolCallDisplay(summary="inheritance_from_habit_structures")
        return ToolCallDisplay(
            summary="inheritance_from_habit_structures",
            details={
    ...
    def get_result_display(cls, event: ToolResultEvent) -> ToolResultDisplay:
        if not isinstance(event.result, BackendHabitResult):
            return ToolResultDisplay(success=False, message=event.error or event.skip_reason or "No result")
        message = "Backend/habit processing complete"
        if event.result.errors:
            message = "Backend/habit processing finished with errors"
    ...
    def get_status_text(cls) -> str:
        return "Backend/habit processing complete"