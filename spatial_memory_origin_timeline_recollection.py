...
# Source : tools/spatial_memory_origin_timeline_recollection.py
...
DEFAULT_PROMPT_TEMPLATE = """### SPATIAL MEMORY TIMELINE MODE (OPT-IN)
Work with spatial memory origin locations and timeline-tied recollection.
- Identify spatial anchors, timeline markers, and recall cues.
...
TOOL_PROMPT = (
    "Use spatial_memory_origin_timeline_recollection to work with spatial memory origins and timeline-tied recall. "
    "Provide prompt or messages, and optionally set focus, show_steps, and max_results."
)


class SpatialMemoryTimelineMessage(BaseModel):
    role: str
    content: str


class SpatialMemoryTimelineArgs(BaseModel):
    prompt: str | None = Field(default=None, description="User prompt to solve.")
    messages: list[SpatialMemoryTimelineMessage] | None = Field(
...
class SpatialMemoryTimelineResult(BaseModel):
    answer: str
    system_prompt: str
    messages: list[SpatialMemoryTimelineMessage]
    used_focus: str
    show_steps: bool
    max_results: int
    template_source: str
...
class SpatialMemoryTimelineConfig(BaseToolConfig):
    ...
    default_focus: str = Field(
        default="Recollect spatial memory origin locations tied to specific timeline data.",
    ...
    prompt_path: Path | None = Field(
        ...
        / "core"
        / "prompts"
        / "spatial_memory_origin_timeline_recollection.md",
        description="Optional path to a prompt template.",
    )
    prompt_max_chars: int = Field(default=8000, description="Maximum template characters to load.")


class SpatialMemoryTimelineState(BaseToolState):
    pass


class SpatialMemoryOriginTimelineRecollection(
    BaseTool[
        SpatialMemoryTimelineArgs,
        SpatialMemoryTimelineResult,
        SpatialMemoryTimelineConfig,
        SpatialMemoryTimelineState,
    ],
    ToolUIData[SpatialMemoryTimelineArgs, SpatialMemoryTimelineResult],
):
    description: ClassVar[str] = "Recollect spatial memory origin locations tied to specific timeline data."

    @classmethod
    def get_tool_prompt(cls) -> str | None:
        return TOOL_PROMPT

    async def run(self, args: SpatialMemoryTimelineArgs) -> SpatialMemoryTimelineResult:
        warnings: list[str] = []
        errors: list[str] = []

        focus = (args.focus or self.config.default_focus).strip() or "spatial memory"
        ...
        return SpatialMemoryTimelineResult(
            answer=answer,
    ...
    def _validate_llm_settings(self, args: SpatialMemoryTimelineArgs) -> None:
    ...
    def _normalize_messages(self, args: SpatialMemoryTimelineArgs, system_prompt: str) -> list[SpatialMemoryTimelineMessage]:
        messages: list[SpatialMemoryTimelineMessage] = []
        if args.messages:
            for msg in args.messages:
                ...
                if content:
                    messages.append(SpatialMemoryTimelineMessage(role=role, content=content))
        elif args.prompt and args.prompt.strip():
            messages.append(SpatialMemoryTimelineMessage(role="user", content=args.prompt.strip()))
        else:
            raise ToolError("Provide prompt or messages.")
        if not messages:
            raise ToolError("No usable messages provided.")
        if system_prompt.strip():
            messages.insert(0, SpatialMemoryTimelineMessage(role="system", content=system_prompt.strip()))
        return messages

    def _call_llm(self, messages: list[SpatialMemoryTimelineMessage], args: SpatialMemoryTimelineArgs) -> str:
    ...
    def _resolve_model(self, args: SpatialMemoryTimelineArgs) -> str:
        return args.llm_model or self.config.llm_model

    @classmethod
    def get_call_display(cls, event: ToolCallEvent) -> ToolCallDisplay:
        if not isinstance(event.args, SpatialMemoryTimelineArgs):
            return ToolCallDisplay(summary="spatial_memory_origin_timeline_recollection")
        return ToolCallDisplay(
            summary="spatial_memory_origin_timeline_recollection",
            details={
    ...
    def get_result_display(cls, event: ToolResultEvent) -> ToolResultDisplay:
        if not isinstance(event.result, SpatialMemoryTimelineResult):
            return ToolResultDisplay(success=False, message=event.error or event.skip_reason or "No result")
        message = "Spatial memory processing complete"
        if event.result.errors:
            message = "Spatial memory processing finished with errors"
    ...
    def get_status_text(cls) -> str:
        return "Spatial memory processing complete"