...
# Source : tools/learning_metadata_recursive_connections.py
...
DEFAULT_PROMPT_TEMPLATE = """### LEARNING / MULTI-PROCESS MODE (OPT-IN)
Work with metadata-driven learning, multi-process execution, and inherited data structures.
- Identify connection anchors, execution paths, and transfer cues.
...
TOOL_PROMPT = (
    "Use learning_metadata_recursive_connections for metadata-driven learning, multi-process execution, or inherited data structures. "
    "Provide prompt or messages, and optionally set focus, show_steps, and max_results."
)


class LearningProcessMessage(BaseModel):
    role: str
    content: str


class LearningProcessArgs(BaseModel):
    prompt: str | None = Field(default=None, description="User prompt to solve.")
    messages: list[LearningProcessMessage] | None = Field(
...
class LearningProcessResult(BaseModel):
    answer: str
    system_prompt: str
    messages: list[LearningProcessMessage]
    used_focus: str
    show_steps: bool
    max_results: int
    template_source: str
...
class LearningProcessConfig(BaseToolConfig):
    ...
    default_focus: str = Field(
        default="Learn through metadata connections, recursion, and repetition.",
    ...
    prompt_path: Path | None = Field(
        ...
        / "core"
        / "prompts"
        / "learning_metadata_recursive_connections.md",
        description="Optional path to a prompt template.",
    )
    prompt_max_chars: int = Field(default=8000, description="Maximum template characters to load.")


class LearningProcessState(BaseToolState):
    pass


class LearningMetadataRecursiveConnections(
    BaseTool[
        LearningProcessArgs,
        LearningProcessResult,
        LearningProcessConfig,
        LearningProcessState,
    ],
    ToolUIData[LearningProcessArgs, LearningProcessResult],
):
    description: ClassVar[str] = "Learn through metadata connections, recursion, and repetition."

    @classmethod
    def get_tool_prompt(cls) -> str | None:
        return TOOL_PROMPT

    async def run(self, args: LearningProcessArgs) -> LearningProcessResult:
        warnings: list[str] = []
        errors: list[str] = []

        focus = (args.focus or self.config.default_focus).strip() or "learning processes"
    ...
    def _validate_llm_settings(self, args: LearningProcessArgs) -> None:
    ...
    def _normalize_messages(self, args: LearningProcessArgs, system_prompt: str) -> list[LearningProcessMessage]:
        messages: list[LearningProcessMessage] = []
        if args.messages:
            for msg in args.messages:
                ...
                if content:
                    messages.append(LearningProcessMessage(role=role, content=content))
        elif args.prompt and args.prompt.strip():
            messages.append(LearningProcessMessage(role="user", content=args.prompt.strip()))
        else:
            raise ToolError("Provide prompt or messages.")
        if not messages:
            raise ToolError("No usable messages provided.")
        if system_prompt.strip():
            messages.insert(0, LearningProcessMessage(role="system", content=system_prompt.strip()))
        return messages

    def _call_llm(self, messages: list[LearningProcessMessage], args: LearningProcessArgs) -> str:
    ...
    def _resolve_model(self, args: LearningProcessArgs) -> str:
        return args.llm_model or self.config.llm_model

    @classmethod
    def get_call_display(cls, event: ToolCallEvent) -> ToolCallDisplay:
        if not isinstance(event.args, LearningProcessArgs):
            return ToolCallDisplay(summary="learning_metadata_recursive_connections")
        return ToolCallDisplay(
            summary="learning_metadata_recursive_connections",
            details={
    ...
    def get_result_display(cls, event: ToolResultEvent) -> ToolResultDisplay:
        if not isinstance(event.result, LearningProcessResult):
            return ToolResultDisplay(success=False, message=event.error or event.skip_reason or "No result")
        message = "Learning/multi-process processing complete"
        if event.result.errors:
            message = "Learning/multi-process processing finished with errors"
    ...
    def get_status_text(cls) -> str:
        return "Learning/multi-process processing complete"