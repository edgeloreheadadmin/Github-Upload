...
# Source : tools/feeling_sensation_management_control.py
...
DEFAULT_PROMPT_TEMPLATE = """### FEELING-ALIGNED SENSATION MANAGEMENT + CONTROL MODE (OPT-IN)
Manage and regulate sensation signals linked to feelings before answering.
...
TOOL_PROMPT = (
    "Use `feeling_sensation_management_control` to manage and regulate sensation signals tied to feelings. "
...
class FeelingSensationManagementMessage(BaseModel):
    role: str
    content: str


class FeelingSensationContextItem(BaseModel):
...
class FeelingSensationManagementControlArgs(BaseModel):
    prompt: str | None = Field(
        default=None, description="User prompt to solve."
    )
    messages: list[FeelingSensationManagementMessage] | None = Field(
    ...
    items: list[FeelingSensationContextItem] | None = Field(
        default=None, description="Feeling-aligned sensation context items."
    )
...
class FeelingSensationBlock(BaseModel):
    label: str | None
    source_type: str | None
    source_path: str | None
    content: str
...
class FeelingSensationManagementControlResult(BaseModel):
    answer: str
    system_prompt: str
    messages: list[FeelingSensationManagementMessage]
    used_feeling_style: str
    used_control_focus: str
    show_steps: bool
    max_actions: int
    template_source: str
    feeling_blocks: list[FeelingSensationBlock]
    warnings: list[str]
    errors: list[str]
    llm_model: str


class FeelingSensationManagementControlConfig(BaseToolConfig):
    ...
    prompt_path: Path | None = Field(
        ...
        / "core"
        / "prompts"
        / "feeling_sensation_management_control.md",
...
class FeelingSensationManagementControlState(BaseToolState):
    pass


class FeelingSensationManagementControl(
    BaseTool[
        FeelingSensationManagementControlArgs,
        FeelingSensationManagementControlResult,
        FeelingSensationManagementControlConfig,
        FeelingSensationManagementControlState,
    ],
    ToolUIData[FeelingSensationManagementControlArgs, FeelingSensationManagementControlResult],
):
    description: ClassVar[str] = (
        "Manage and regulate sensation signals aligned to feelings using a prompt template."
    )
    ...
    async def run(
        self, args: FeelingSensationManagementControlArgs
    ) -> FeelingSensationManagementControlResult:
        warnings: list[str] = []
        errors: list[str] = []
        ...
        return FeelingSensationManagementControlResult(
            answer=answer,
    ...
    def _validate_llm_settings(self, args: FeelingSensationManagementControlArgs) -> None:
    ...
    def _collect_feeling_context(
        self, args: FeelingSensationManagementControlArgs, warnings: list[str]
    ) -> list[FeelingSensationBlock]:
        max_item = (
            args.max_item_chars
        ...
        blocks: list[FeelingSensationBlock] = []
        total_chars = 0
    ...
    def _resolve_feeling_sources(
        self, args: FeelingSensationManagementControlArgs, warnings: list[str]
    ) -> list[_FeelingSensationSource]:
        sources: list[_FeelingSensationSource] = []
        if args.items:
    ...
    def _load_feeling_item(self, item: FeelingSensationContextItem) -> _FeelingSensationSource:
        ...
        if item.path:
            ...
            return _FeelingSensationSource(
                content=text,
    ...
    def _format_feeling_text(self, item: FeelingSensationContextItem) -> str:
        lines: list[str] = []
    ...
    def _format_feeling_blocks(self, blocks: list[FeelingSensationBlock]) -> str:
        if not blocks:
            return ""
        parts = ["Feeling-aligned sensation context:"]
        for block in blocks:
    ...
    def _inject_feeling_context(
        ...
        messages: list[FeelingSensationManagementMessage],
        blocks: list[FeelingSensationBlock],
    ) -> list[FeelingSensationManagementMessage]:
        if not blocks:
            return messages
        ...
        for idx in range(len(messages) - 1, -1, -1):
            if messages[idx].role == "user":
                ...
                return messages
        messages.append(FeelingSensationManagementMessage(role="user", content=feeling_text))
        return messages
    ...
    def _normalize_messages(
        self, args: FeelingSensationManagementControlArgs, system_prompt: str
    ) -> list[FeelingSensationManagementMessage]:
        messages: list[FeelingSensationManagementMessage] = []
        if args.messages:
            for msg in args.messages:
                ...
                if not role:
                    role = "user"
                messages.append(FeelingSensationManagementMessage(role=role, content=content))
        elif args.prompt and args.prompt.strip():
            messages.append(
                FeelingSensationManagementMessage(role="user", content=args.prompt.strip())
            )
        else:
        ...
        if system_prompt.strip():
            messages.insert(
                0,
                FeelingSensationManagementMessage(
    ...
    def _call_llm(
        self,
        messages: list[FeelingSensationManagementMessage],
        args: FeelingSensationManagementControlArgs,
    ) -> str:
    ...
    def _resolve_model(self, args: FeelingSensationManagementControlArgs) -> str:
        return args.llm_model or self.config.llm_model

    @classmethod
    def get_call_display(cls, event: ToolCallEvent) -> ToolCallDisplay:
        if not isinstance(event.args, FeelingSensationManagementControlArgs):
            return ToolCallDisplay(summary="feeling_sensation_management_control")
        return ToolCallDisplay(
            summary="feeling_sensation_management_control",
            details={
    ...
    def get_result_display(cls, event: ToolResultEvent) -> ToolResultDisplay:
        if not isinstance(event.result, FeelingSensationManagementControlResult):
            return ToolResultDisplay(
                success=False, message=event.error or event.skip_reason or "No result"
            )
        message = "Feeling-aligned sensation management and control complete"
        if event.result.errors:
            message = "Feeling-aligned sensation management and control finished with errors"
        return ToolResultDisplay(
    ...
    def get_status_text(cls) -> str:
        return "Feeling-aligned sensation management and control"
