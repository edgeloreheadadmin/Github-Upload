...
# Source : tools/text_embeddings.py
...
class TextEmbeddingsConfig(BaseToolConfig):
    ...
    model_name: str = Field(
        ...
        description="Local embeddings model name.",
    )
...
class TextEmbeddingsState(BaseToolState):
    pass


class TextEmbeddingsArgs(BaseModel):
...
class TextEmbeddingsResult(BaseModel):
    model: str
    output_dtype: str
...
class TextEmbeddings(
    ...
    ToolUIData[TextEmbeddingsArgs, TextEmbeddingsResult],
):
    description: ClassVar[str] = (
        "Generate embeddings for text using a local embeddings backend."
    )

    async def run(self, args: TextEmbeddingsArgs) -> TextEmbeddingsResult:
    ...
    def _resolve_inputs(self, args: TextEmbeddingsArgs) -> list[str]:
    ...
    def get_call_display(cls, event: ToolCallEvent) -> ToolCallDisplay:
        if not isinstance(event.args, TextEmbeddingsArgs):
            return ToolCallDisplay(summary="text_embeddings")

        count = len(event.args.inputs or []) if event.args.inputs else 1
        return ToolCallDisplay(
            summary=f"text_embeddings ({count} input(s))",
            details={
    ...
    def get_result_display(cls, event: ToolResultEvent) -> ToolResultDisplay:
        if not isinstance(event.result, TextEmbeddingsResult):
