...
# Source : tools/cache_prompting_single_layer.py
...
DEFAULT_PROMPT_TEMPLATE = """### CACHE PROMPTING: SINGLE LAYER (OPT-IN)
Decide cache reuse with a single computational layer.
- Score cache candidates using similarity, length match, and recency.
- Prefer cache only when confidence is high.
...
TOOL_PROMPT = (
    "Use `cache_prompting_single_layer` for cache prompting driven by a "
    "single-layer scoring function. Provide `prompt` or `messages`."
)

FEATURE_NAMES = ("similarity", "length_match", "recency")
VALID_CACHE_MODES = {"cache-first", "cache-only", "skip"}
VALID_ACTIVATIONS = {"sigmoid", "linear"}
...
class CachePromptingSingleLayerMessage(BaseModel):
    role: str
    content: str


class CachePromptingSingleLayerArgs(BaseModel):
    prompt: str | None = Field(
        default=None, description="User prompt to solve."
    )
    messages: list[CachePromptingSingleLayerMessage] | None = Field(
    ...
    activation: str | None = Field(
        default=None, description="sigmoid or linear."
    )
...
class CachePromptingSingleLayerResult(BaseModel):
    answer: str
    system_prompt: str
    messages: list[CachePromptingSingleLayerMessage]
    cache_mode: str
    cache_hit: bool
    cache_used: bool
...
class CachePromptingSingleLayerConfig(BaseToolConfig):
    ...
    cache_path: Path = Field(
        ...
        / ".codex"
        / "cache"
        / "cache_prompting_single_layer.json",
    ...
    prompt_path: Path | None = Field(
        ...
        / "core"
        / "prompts"
        / "cache_prompting_single_layer.md",
...
class CachePromptingSingleLayerState(BaseToolState):
    pass


class CachePromptingSingleLayer(
    BaseTool[
        CachePromptingSingleLayerArgs,
        CachePromptingSingleLayerResult,
        CachePromptingSingleLayerConfig,
        CachePromptingSingleLayerState,
    ],
    ToolUIData[CachePromptingSingleLayerArgs, CachePromptingSingleLayerResult],
):
    description: ClassVar[str] = (
        "Cache prompting with a single-layer scoring function."
    )
    ...
    async def run(
        self, args: CachePromptingSingleLayerArgs
    ) -> CachePromptingSingleLayerResult:
        warnings: list[str] = []
        errors: list[str] = []
        ...
        return CachePromptingSingleLayerResult(
            answer=answer,
    ...
    def _validate_llm_settings(self, args: CachePromptingSingleLayerArgs) -> None:
    ...
    def _validate_cache_settings(
        ...
        if activation == "sigmoid" and not (0 <= decision_threshold <= 1):
            raise ToolError(
                "decision_threshold must be between 0 and 1 for sigmoid activation."
            )
    ...
    def _normalize_messages(
        self, args: CachePromptingSingleLayerArgs, system_prompt: str
    ) -> list[CachePromptingSingleLayerMessage]:
        messages: list[CachePromptingSingleLayerMessage] = []
        if args.messages:
            for msg in args.messages:
                ...
                if not role:
                    role = "user"
                messages.append(
                    CachePromptingSingleLayerMessage(role=role, content=content)
                )
        elif args.prompt and args.prompt.strip():
            messages.append(
                CachePromptingSingleLayerMessage(
    ...
    def _build_prompt_text(
        self, messages: list[CachePromptingSingleLayerMessage]
    ) -> str:
    ...
    def _find_cache_candidate(
        ...
        for entry in entries:
            ...
            score = self._layer_score(features, weights, bias, activation)
    ...
    def _layer_score(
        ...
        raw = sum(w * x for w, x in zip(weights, features)) + bias
    ...
    def _call_llm(
        self,
        messages: list[CachePromptingSingleLayerMessage],
        args: CachePromptingSingleLayerArgs,
    ) -> str:
    ...
    def _resolve_model(self, args: CachePromptingSingleLayerArgs) -> str:
        return args.llm_model or self.config.llm_model

    @classmethod
    def get_call_display(cls, event: ToolCallEvent) -> ToolCallDisplay:
        if not isinstance(event.args, CachePromptingSingleLayerArgs):
            return ToolCallDisplay(summary="cache_prompting_single_layer")
        return ToolCallDisplay(
            summary="cache_prompting_single_layer",
            details={
    ...
    def get_result_display(cls, event: ToolResultEvent) -> ToolResultDisplay:
        if not isinstance(event.result, CachePromptingSingleLayerResult):
            return ToolResultDisplay(
                success=False, message=event.error or event.skip_reason or "No result"
            )
        message = "Cache prompting (single layer) complete"
        if event.result.errors:
            message = "Cache prompting (single layer) finished with errors"
        return ToolResultDisplay(
    ...
    def get_status_text(cls) -> str:
        return "Cache prompting (single layer)"