...
# Source : tools/spatial_memory_recall_recollected_timeframe.py
...
DEFAULT_PROMPT_TEMPLATE = """### SPATIAL MEMORY RECALL MODE (OPT-IN)
Work with spatial memory recall and retention cues.
- Identify spatial anchors, timeline markers, and recall signals.
...
TOOL_PROMPT = (
    "Use spatial_memory_recall_recollected_timeframe for spatial memory recall or retention guidance. "
    "Provide prompt or messages, and optionally set focus, show_steps, and max_results."
)


class SpatialMemoryRecallMessage(BaseModel):
    role: str
    content: str


class SpatialMemoryRecallArgs(BaseModel):
    prompt: str | None = Field(default=None, description="User prompt to solve.")
    messages: list[SpatialMemoryRecallMessage] | None = Field(
...
class SpatialMemoryRecallResult(BaseModel):
    answer: str
    system_prompt: str
    messages: list[SpatialMemoryRecallMessage]
    used_focus: str
    show_steps: bool
    max_results: int
    template_source: str
...
class SpatialMemoryRecallConfig(BaseToolConfig):
    ...
    default_focus: str = Field(
        default="Recall recollected spatial memory based on timeframe.",
    ...
    prompt_path: Path | None = Field(
        ...
        / "core"
        / "prompts"
        / "spatial_memory_recall_recollected_timeframe.md",
        description="Optional path to a prompt template.",
    )
    prompt_max_chars: int = Field(default=8000, description="Maximum template characters to load.")


class SpatialMemoryRecallState(BaseToolState):
    pass


class SpatialMemoryRecallRecollectedTimeframe(
    BaseTool[
        SpatialMemoryRecallArgs,
        SpatialMemoryRecallResult,
        SpatialMemoryRecallConfig,
        SpatialMemoryRecallState,
    ],
    ToolUIData[SpatialMemoryRecallArgs, SpatialMemoryRecallResult],
):
    description: ClassVar[str] = "Recall recollected spatial memory based on timeframe."

    @classmethod
    def get_tool_prompt(cls) -> str | None:
        return TOOL_PROMPT

    async def run(self, args: SpatialMemoryRecallArgs) -> SpatialMemoryRecallResult:
        warnings: list[str] = []
        errors: list[str] = []
        ...
        return SpatialMemoryRecallResult(
            answer=answer,
    ...
    def _validate_llm_settings(self, args: SpatialMemoryRecallArgs) -> None:
    ...
    def _normalize_messages(self, args: SpatialMemoryRecallArgs, system_prompt: str) -> list[SpatialMemoryRecallMessage]:
        messages: list[SpatialMemoryRecallMessage] = []
        if args.messages:
            for msg in args.messages:
                ...
                if content:
                    messages.append(SpatialMemoryRecallMessage(role=role, content=content))
        elif args.prompt and args.prompt.strip():
            messages.append(SpatialMemoryRecallMessage(role="user", content=args.prompt.strip()))
        else:
            raise ToolError("Provide prompt or messages.")
        if not messages:
            raise ToolError("No usable messages provided.")
        if system_prompt.strip():
            messages.insert(0, SpatialMemoryRecallMessage(role="system", content=system_prompt.strip()))
        return messages

    def _call_llm(self, messages: list[SpatialMemoryRecallMessage], args: SpatialMemoryRecallArgs) -> str:
    ...
    def _resolve_model(self, args: SpatialMemoryRecallArgs) -> str:
        return args.llm_model or self.config.llm_model

    @classmethod
    def get_call_display(cls, event: ToolCallEvent) -> ToolCallDisplay:
        if not isinstance(event.args, SpatialMemoryRecallArgs):
            return ToolCallDisplay(summary="spatial_memory_recall_recollected_timeframe")
        return ToolCallDisplay(
            summary="spatial_memory_recall_recollected_timeframe",
            details={
    ...
    def get_result_display(cls, event: ToolResultEvent) -> ToolResultDisplay:
        if not isinstance(event.result, SpatialMemoryRecallResult):
            return ToolResultDisplay(success=False, message=event.error or event.skip_reason or "No result")
        message = "Spatial memory recall processing complete"
        if event.result.errors:
            message = "Spatial memory recall processing finished with errors"
    ...
    def get_status_text(cls) -> str:
        return "Spatial memory recall processing complete"