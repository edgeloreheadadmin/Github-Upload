...
# Source : tools/memory_recovery_refresh_classification_sectors.py
...
DEFAULT_PROMPT_TEMPLATE = """### MEMORY RECOVERY / SYNC MODE (OPT-IN)
Work with memory recovery, refresh, and short/long-term synchronization.
- Identify recovery cues, linkage anchors, and feedback measures.
...
TOOL_PROMPT = (
    "Use memory_recovery_refresh_classification_sectors for memory recovery, refresh, or short/long-term linkage analysis. "
    "Provide prompt or messages, and optionally set focus, show_steps, and max_results."
)


class MemoryRecoveryMessage(BaseModel):
    role: str
    content: str


class MemoryRecoveryArgs(BaseModel):
    prompt: str | None = Field(default=None, description="User prompt to solve.")
    messages: list[MemoryRecoveryMessage] | None = Field(
...
class MemoryRecoveryResult(BaseModel):
    answer: str
    system_prompt: str
    messages: list[MemoryRecoveryMessage]
    used_focus: str
    show_steps: bool
    max_results: int
    template_source: str
...
class MemoryRecoveryConfig(BaseToolConfig):
    ...
    default_focus: str = Field(
        default="Recover erased memories, refresh memory, and classify recovered data by sectors.",
    ...
    prompt_path: Path | None = Field(
        ...
        / "core"
        / "prompts"
        / "memory_recovery_refresh_classification_sectors.md",
        description="Optional path to a prompt template.",
    )
    prompt_max_chars: int = Field(default=8000, description="Maximum template characters to load.")


class MemoryRecoveryState(BaseToolState):
    pass


class MemoryRecoveryRefreshClassificationSectors(
    BaseTool[
        MemoryRecoveryArgs,
    ...
    ToolUIData[MemoryRecoveryArgs, MemoryRecoveryResult],
):
    description: ClassVar[str] = "Recover erased memories, refresh memory, and classify recovered data by sectors."

    @classmethod
    def get_tool_prompt(cls) -> str | None:
        return TOOL_PROMPT

    async def run(self, args: MemoryRecoveryArgs) -> MemoryRecoveryResult:
        warnings: list[str] = []
        errors: list[str] = []

        focus = (args.focus or self.config.default_focus).strip() or "memory recovery"
    ...
    def _validate_llm_settings(self, args: MemoryRecoveryArgs) -> None:
    ...
    def _normalize_messages(self, args: MemoryRecoveryArgs, system_prompt: str) -> list[MemoryRecoveryMessage]:
        messages: list[MemoryRecoveryMessage] = []
        if args.messages:
            for msg in args.messages:
                ...
                if content:
                    messages.append(MemoryRecoveryMessage(role=role, content=content))
        elif args.prompt and args.prompt.strip():
            messages.append(MemoryRecoveryMessage(role="user", content=args.prompt.strip()))
        else:
            raise ToolError("Provide prompt or messages.")
        if not messages:
            raise ToolError("No usable messages provided.")
        if system_prompt.strip():
            messages.insert(0, MemoryRecoveryMessage(role="system", content=system_prompt.strip()))
        return messages

    def _call_llm(self, messages: list[MemoryRecoveryMessage], args: MemoryRecoveryArgs) -> str:
    ...
    def _resolve_model(self, args: MemoryRecoveryArgs) -> str:
        return args.llm_model or self.config.llm_model

    @classmethod
    def get_call_display(cls, event: ToolCallEvent) -> ToolCallDisplay:
        if not isinstance(event.args, MemoryRecoveryArgs):
            return ToolCallDisplay(summary="memory_recovery_refresh_classification_sectors")
        return ToolCallDisplay(
            summary="memory_recovery_refresh_classification_sectors",
            details={
    ...
    def get_result_display(cls, event: ToolResultEvent) -> ToolResultDisplay:
        if not isinstance(event.result, MemoryRecoveryResult):
            return ToolResultDisplay(success=False, message=event.error or event.skip_reason or "No result")
        message = "Memory recovery/sync processing complete"
        if event.result.errors:
            message = "Memory recovery/sync processing finished with errors"
    ...
    def get_status_text(cls) -> str:
        return "Memory recovery/sync processing complete"