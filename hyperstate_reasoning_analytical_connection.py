...
# Source : tools/hyperstate_reasoning_analytical_connection.py
...
DEFAULT_PROMPT_TEMPLATE = """### ADVANCED MEMORY / REASONING MODE (OPT-IN)
Work with memory patterns, frequency signals, or advanced reasoning.
- Identify anchors, linkages, and analytical cues.
...
TOOL_PROMPT = (
    "Use hyperstate_reasoning_analytical_connection for memory pattern/frequency analysis or advanced reasoning. "
    "Provide prompt or messages, and optionally set focus, show_steps, and max_results."
)


class AdvancedMemoryMessage(BaseModel):
    role: str
    content: str


class AdvancedMemoryArgs(BaseModel):
    prompt: str | None = Field(default=None, description="User prompt to solve.")
    messages: list[AdvancedMemoryMessage] | None = Field(
...
class AdvancedMemoryResult(BaseModel):
    answer: str
    system_prompt: str
    messages: list[AdvancedMemoryMessage]
    used_focus: str
    show_steps: bool
    max_results: int
    template_source: str
...
class AdvancedMemoryConfig(BaseToolConfig):
    ...
    default_focus: str = Field(
        default="Use hyperstate reasoning for advanced analytical connections.",
    ...
    prompt_path: Path | None = Field(
        ...
        / "core"
        / "prompts"
        / "hyperstate_reasoning_analytical_connection.md",
        description="Optional path to a prompt template.",
    )
    prompt_max_chars: int = Field(default=8000, description="Maximum template characters to load.")


class AdvancedMemoryState(BaseToolState):
    pass


class HyperstateReasoningAnalyticalConnection(
    BaseTool[
        AdvancedMemoryArgs,
    ...
    ToolUIData[AdvancedMemoryArgs, AdvancedMemoryResult],
):
    description: ClassVar[str] = "Use hyperstate reasoning to form advanced analytical connections."

    @classmethod
    def get_tool_prompt(cls) -> str | None:
        return TOOL_PROMPT

    async def run(self, args: AdvancedMemoryArgs) -> AdvancedMemoryResult:
        warnings: list[str] = []
        errors: list[str] = []

        focus = (args.focus or self.config.default_focus).strip() or "advanced memory"
    ...
    def _validate_llm_settings(self, args: AdvancedMemoryArgs) -> None:
    ...
    def _normalize_messages(self, args: AdvancedMemoryArgs, system_prompt: str) -> list[AdvancedMemoryMessage]:
        messages: list[AdvancedMemoryMessage] = []
        if args.messages:
            for msg in args.messages:
                ...
                if content:
                    messages.append(AdvancedMemoryMessage(role=role, content=content))
        elif args.prompt and args.prompt.strip():
            messages.append(AdvancedMemoryMessage(role="user", content=args.prompt.strip()))
        else:
            raise ToolError("Provide prompt or messages.")
        if not messages:
            raise ToolError("No usable messages provided.")
        if system_prompt.strip():
            messages.insert(0, AdvancedMemoryMessage(role="system", content=system_prompt.strip()))
        return messages

    def _call_llm(self, messages: list[AdvancedMemoryMessage], args: AdvancedMemoryArgs) -> str:
    ...
    def _resolve_model(self, args: AdvancedMemoryArgs) -> str:
        return args.llm_model or self.config.llm_model

    @classmethod
    def get_call_display(cls, event: ToolCallEvent) -> ToolCallDisplay:
        if not isinstance(event.args, AdvancedMemoryArgs):
            return ToolCallDisplay(summary="hyperstate_reasoning_analytical_connection")
        return ToolCallDisplay(
            summary="hyperstate_reasoning_analytical_connection",
            details={
    ...
    def get_result_display(cls, event: ToolResultEvent) -> ToolResultDisplay:
        if not isinstance(event.result, AdvancedMemoryResult):
            return ToolResultDisplay(success=False, message=event.error or event.skip_reason or "No result")
        message = "Advanced memory reasoning complete"
        if event.result.errors:
            message = "Advanced memory reasoning finished with errors"
    ...
    def get_status_text(cls) -> str:
        return "Advanced memory reasoning complete"