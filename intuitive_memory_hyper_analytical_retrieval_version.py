...
# Source : tools/intuitive_memory_hyper_analytical_retrieval_version.py
...
DEFAULT_PROMPT_TEMPLATE = """### PRE/INTUITIVE MEMORY MODE (OPT-IN)
Work with pre-recognition, precognition, and intuitive analytical retrieval.
- Identify anticipatory cues, memory formation anchors, and analytical prompts.
...
TOOL_PROMPT = (
    "Use intuitive_memory_hyper_analytical_retrieval_version for pre-recognition, precognition, or intuitive analytical retrieval. "
    "Provide prompt or messages, and optionally set focus, show_steps, and max_results."
)


class IntuitiveMemoryMessage(BaseModel):
    role: str
    content: str


class IntuitiveMemoryArgs(BaseModel):
    prompt: str | None = Field(default=None, description="User prompt to solve.")
    messages: list[IntuitiveMemoryMessage] | None = Field(
...
class IntuitiveMemoryResult(BaseModel):
    answer: str
    system_prompt: str
    messages: list[IntuitiveMemoryMessage]
    used_focus: str
    show_steps: bool
    max_results: int
    template_source: str
...
class IntuitiveMemoryConfig(BaseToolConfig):
    ...
    default_focus: str = Field(
        default="Intuitive memory responses for hyper-analytical memory retrieval.",
    ...
    prompt_path: Path | None = Field(
        ...
        / "core"
        / "prompts"
        / "intuitive_memory_hyper_analytical_retrieval_version.md",
        description="Optional path to a prompt template.",
    )
    prompt_max_chars: int = Field(default=8000, description="Maximum template characters to load.")


class IntuitiveMemoryState(BaseToolState):
    pass


class IntuitiveMemoryHyperAnalyticalRetrievalVersion(
    BaseTool[
        IntuitiveMemoryArgs,
        IntuitiveMemoryResult,
        IntuitiveMemoryConfig,
        IntuitiveMemoryState,
    ],
    ToolUIData[IntuitiveMemoryArgs, IntuitiveMemoryResult],
):
    description: ClassVar[str] = "Intuitive memory responses for hyper-analytical memory retrieval."

    @classmethod
    def get_tool_prompt(cls) -> str | None:
        return TOOL_PROMPT

    async def run(self, args: IntuitiveMemoryArgs) -> IntuitiveMemoryResult:
        warnings: list[str] = []
        errors: list[str] = []

        focus = (args.focus or self.config.default_focus).strip() or "intuitive memory"
    ...
    def _validate_llm_settings(self, args: IntuitiveMemoryArgs) -> None:
    ...
    def _normalize_messages(self, args: IntuitiveMemoryArgs, system_prompt: str) -> list[IntuitiveMemoryMessage]:
        messages: list[IntuitiveMemoryMessage] = []
        if args.messages:
            for msg in args.messages:
                ...
                if content:
                    messages.append(IntuitiveMemoryMessage(role=role, content=content))
        elif args.prompt and args.prompt.strip():
            messages.append(IntuitiveMemoryMessage(role="user", content=args.prompt.strip()))
        else:
            raise ToolError("Provide prompt or messages.")
        if not messages:
            raise ToolError("No usable messages provided.")
        if system_prompt.strip():
            messages.insert(0, IntuitiveMemoryMessage(role="system", content=system_prompt.strip()))
        return messages

    def _call_llm(self, messages: list[IntuitiveMemoryMessage], args: IntuitiveMemoryArgs) -> str:
    ...
    def _resolve_model(self, args: IntuitiveMemoryArgs) -> str:
        return args.llm_model or self.config.llm_model

    @classmethod
    def get_call_display(cls, event: ToolCallEvent) -> ToolCallDisplay:
        if not isinstance(event.args, IntuitiveMemoryArgs):
            return ToolCallDisplay(summary="intuitive_memory_hyper_analytical_retrieval_version")
        return ToolCallDisplay(
            summary="intuitive_memory_hyper_analytical_retrieval_version",
            details={
    ...
    def get_result_display(cls, event: ToolResultEvent) -> ToolResultDisplay:
        if not isinstance(event.result, IntuitiveMemoryResult):
            return ToolResultDisplay(success=False, message=event.error or event.skip_reason or "No result")
        message = "Intuitive memory processing complete"
        if event.result.errors:
            message = "Intuitive memory processing finished with errors"
    ...
    def get_status_text(cls) -> str:
        return "Intuitive memory processing complete"