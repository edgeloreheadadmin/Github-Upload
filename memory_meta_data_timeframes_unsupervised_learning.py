...
# Source : tools/memory_meta_data_timeframes_unsupervised_learning.py
...
DEFAULT_PROMPT_TEMPLATE = """### MEMORY META-DATA / RECALL MODE (OPT-IN)
Work with memory meta-data, timelines, and recall optimization.
- Identify metadata anchors, timeline cues, and recall pathways.
...
TOOL_PROMPT = (
    "Use memory_meta_data_timeframes_unsupervised_learning for memory meta-data, timelines, or recall optimization. "
    "Provide prompt or messages, and optionally set focus, show_steps, and max_results."
)


class MemoryMetaRecallMessage(BaseModel):
    role: str
    content: str


class MemoryMetaRecallArgs(BaseModel):
    prompt: str | None = Field(default=None, description="User prompt to solve.")
    messages: list[MemoryMetaRecallMessage] | None = Field(
...
class MemoryMetaRecallResult(BaseModel):
    answer: str
    system_prompt: str
    messages: list[MemoryMetaRecallMessage]
    used_focus: str
    show_steps: bool
    max_results: int
    template_source: str
...
class MemoryMetaRecallConfig(BaseToolConfig):
    ...
    default_focus: str = Field(
        default="Memory meta-data timeframes for unsupervised learning.",
    ...
    prompt_path: Path | None = Field(
        ...
        / "core"
        / "prompts"
        / "memory_meta_data_timeframes_unsupervised_learning.md",
        description="Optional path to a prompt template.",
    )
    prompt_max_chars: int = Field(default=8000, description="Maximum template characters to load.")


class MemoryMetaRecallState(BaseToolState):
    pass


class MemoryMetaDataTimeframesUnsupervisedLearning(
    BaseTool[
        MemoryMetaRecallArgs,
        MemoryMetaRecallResult,
        MemoryMetaRecallConfig,
        MemoryMetaRecallState,
    ],
    ToolUIData[MemoryMetaRecallArgs, MemoryMetaRecallResult],
):
    description: ClassVar[str] = "Work with memory meta-data timeframes for unsupervised learning."

    @classmethod
    def get_tool_prompt(cls) -> str | None:
        return TOOL_PROMPT

    async def run(self, args: MemoryMetaRecallArgs) -> MemoryMetaRecallResult:
        warnings: list[str] = []
        errors: list[str] = []

        focus = (args.focus or self.config.default_focus).strip() or "memory meta-data"
    ...
    def _validate_llm_settings(self, args: MemoryMetaRecallArgs) -> None:
    ...
    def _normalize_messages(self, args: MemoryMetaRecallArgs, system_prompt: str) -> list[MemoryMetaRecallMessage]:
        messages: list[MemoryMetaRecallMessage] = []
        if args.messages:
            for msg in args.messages:
                ...
                if content:
                    messages.append(MemoryMetaRecallMessage(role=role, content=content))
        elif args.prompt and args.prompt.strip():
            messages.append(MemoryMetaRecallMessage(role="user", content=args.prompt.strip()))
        else:
            raise ToolError("Provide prompt or messages.")
        if not messages:
            raise ToolError("No usable messages provided.")
        if system_prompt.strip():
            messages.insert(0, MemoryMetaRecallMessage(role="system", content=system_prompt.strip()))
        return messages

    def _call_llm(self, messages: list[MemoryMetaRecallMessage], args: MemoryMetaRecallArgs) -> str:
    ...
    def _resolve_model(self, args: MemoryMetaRecallArgs) -> str:
        return args.llm_model or self.config.llm_model

    @classmethod
    def get_call_display(cls, event: ToolCallEvent) -> ToolCallDisplay:
        if not isinstance(event.args, MemoryMetaRecallArgs):
            return ToolCallDisplay(summary="memory_meta_data_timeframes_unsupervised_learning")
        return ToolCallDisplay(
            summary="memory_meta_data_timeframes_unsupervised_learning",
            details={
    ...
    def get_result_display(cls, event: ToolResultEvent) -> ToolResultDisplay:
        if not isinstance(event.result, MemoryMetaRecallResult):
            return ToolResultDisplay(success=False, message=event.error or event.skip_reason or "No result")
        message = "Memory meta-data/recall processing complete"
        if event.result.errors:
            message = "Memory meta-data/recall processing finished with errors"
    ...
    def get_status_text(cls) -> str:
        return "Memory meta-data/recall processing complete"