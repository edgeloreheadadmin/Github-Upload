...
class TraitSource(BaseModel):
    path: str
    category: str = Field(default="Traits")
    encoding: str = Field(default="utf-8")


class ContextTraitProfilesConfig(BaseToolConfig):
    permission: ToolPermission = ToolPermission.ALWAYS
    trait_sources: list[TraitSource] = Field(
        default_factory=lambda: [
            TraitSource(
                path=r"C:\Users\Zack\.vibe\definitions\trait_profiles_data.py",
                category="Traits",
            )
        ],
        description="Sources that define custom human characteristics and natures.",
    )
...
class ContextTraitProfilesState(BaseToolState):
    pass
...
class ContextTraitProfilesArgs(BaseModel):
    items: list[TraitItem] = Field(description="Items to evaluate.")
...
class ContextTraitProfilesResult(BaseModel):
    items: list[TraitInsight]
    item_count: int
    total_matches: int
...
class ContextTraitProfiles(
    BaseTool[
        ContextTraitProfilesArgs,
        ContextTraitProfilesResult,
        ContextTraitProfilesConfig,
        ContextTraitProfilesState,
    ],
    ToolUIData[ContextTraitProfilesArgs, ContextTraitProfilesResult],
):
    description: ClassVar[str] = (
        "Load custom human characteristics and natures, then match them in text."
    )

    async def run(self, args: ContextTraitProfilesArgs) -> ContextTraitProfilesResult:
        ...
        if not definitions:
            raise ToolError("No trait definitions available.")

        errors: list[str] = []
        warnings: list[str] = def_warnings.copy()
        insights: list[TraitInsight] = []
        total_bytes = 0
        truncated = False
        total_matches = 0
        ...
        return ContextTraitProfilesResult(
            items=insights,
    ...
    def _match_definitions(
        self, content: str, definitions: list[TraitDefinition]
    ) -> list[TraitMatch]:
        lower = content.lower()
        matches: list[TraitMatch] = []
    ...
    def _load_item(self, item: TraitItem) -> tuple[str, str | None, int | None]:
    ...
    def _load_definitions(self) -> tuple[list[TraitDefinition], list[str]]:
        definitions: list[TraitDefinition] = []
        warnings: list[str] = []
        seen: set[tuple[str, str]] = set()
        for src in self.config.trait_sources:
            try:
                path = Path(src.path).expanduser()
                if not path.is_absolute():
                    path = (self.config.effective_workdir / path).resolve()
                if not path.exists():
                    warnings.append(f"Trait definitions missing: {path}")
                    continue
    ...
    def _parse_python_definitions(
        self, path: Path, source: TraitSource
        ...
        spec = importlib.util.spec_from_file_location("trait_profiles_data", path)
    ...
    def get_call_display(cls, event: ToolCallEvent) -> ToolCallDisplay:
        if not isinstance(event.args, ContextTraitProfilesArgs):
            return ToolCallDisplay(summary="context_trait_profiles")
        if not event.args.items:
            return ToolCallDisplay(summary="context_trait_profiles")
        first = event.args.items[0]
        return ToolCallDisplay(
            summary="context_trait_profiles",
            details={
    ...
    def get_result_display(cls, event: ToolResultEvent) -> ToolResultDisplay:
        if not isinstance(event.result, ContextTraitProfilesResult):
        ...
        return ToolResultDisplay(
            ...
            message=(
                ...
                f"{event.result.total_matches} trait matches"
            ),
