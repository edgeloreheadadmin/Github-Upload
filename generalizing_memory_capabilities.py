...
# Source : tools/generalizing_memory_capabilities.py
...
DEFAULT_PROMPT_TEMPLATE = """### GENERALIZING MEMORY CAPABILITIES MODE (OPT-IN)
Generalize different memory capabilities before answering.
- Identify common patterns across memory types and capabilities.
...
TOOL_PROMPT = (
    "Use `generalizing_memory_capabilities` to generalize memory capabilities. "
    "Provide `prompt` or `messages`, and optionally set `generalization_style`, "
    "`memory_types`, `generalization_focus`, `show_steps`, and `max_generalizations`."
)
...
class MemoryGeneralizationMessage(BaseModel):
    role: str
    content: str


class MemoryCapabilityItem(BaseModel):
    ...
    capability: str | None = Field(
        default=None, description="Memory capability description."
    )
...
class GeneralizingMemoryCapabilitiesArgs(BaseModel):
    prompt: str | None = Field(
        default=None, description="User prompt to solve."
    )
    messages: list[MemoryGeneralizationMessage] | None = Field(
    ...
    items: list[MemoryCapabilityItem] | None = Field(
        default=None, description="Memory capability context items."
    )
...
class MemoryCapabilityBlock(BaseModel):
    label: str | None
    memory_type: str | None
    source_path: str | None
    content: str
...
class GeneralizingMemoryCapabilitiesResult(BaseModel):
    answer: str
    system_prompt: str
    messages: list[MemoryGeneralizationMessage]
    ...
    template_source: str
    item_blocks: list[MemoryCapabilityBlock]
    warnings: list[str]
    errors: list[str]
    llm_model: str


class GeneralizingMemoryCapabilitiesConfig(BaseToolConfig):
    ...
    prompt_path: Path | None = Field(
        ...
        / "core"
        / "prompts"
        / "generalizing_memory_capabilities.md",
...
class GeneralizingMemoryCapabilitiesState(BaseToolState):
    pass


class GeneralizingMemoryCapabilities(
    BaseTool[
        GeneralizingMemoryCapabilitiesArgs,
        GeneralizingMemoryCapabilitiesResult,
        GeneralizingMemoryCapabilitiesConfig,
        GeneralizingMemoryCapabilitiesState,
    ],
    ToolUIData[
        GeneralizingMemoryCapabilitiesArgs,
        GeneralizingMemoryCapabilitiesResult,
    ...
    description: ClassVar[str] = (
        "Generalize memory capabilities using a prompt template."
    )
    ...
    async def run(
        self, args: GeneralizingMemoryCapabilitiesArgs
    ) -> GeneralizingMemoryCapabilitiesResult:
        warnings: list[str] = []
        errors: list[str] = []
        ...
        if not generalization_focus:
            generalization_focus = "memory-capabilities"
        ...
        return GeneralizingMemoryCapabilitiesResult(
            answer=answer,
    ...
    def _validate_llm_settings(self, args: GeneralizingMemoryCapabilitiesArgs) -> None:
    ...
    def _collect_items(
        self, args: GeneralizingMemoryCapabilitiesArgs, warnings: list[str]
    ) -> list[MemoryCapabilityBlock]:
        max_item = (
            args.max_item_chars
        ...
        blocks: list[MemoryCapabilityBlock] = []
        total_chars = 0
    ...
    def _load_item(self, item: MemoryCapabilityItem) -> _MemorySource:
    ...
    def _format_item_text(self, item: MemoryCapabilityItem) -> str:
        lines: list[str] = []
    ...
    def _format_item_blocks(self, blocks: list[MemoryCapabilityBlock]) -> str:
        if not blocks:
            return ""
    ...
    def _inject_items(
        ...
        messages: list[MemoryGeneralizationMessage],
        blocks: list[MemoryCapabilityBlock],
    ) -> list[MemoryGeneralizationMessage]:
        if not blocks:
            return messages
        ...
        for idx in range(len(messages) - 1, -1, -1):
            if messages[idx].role == "user":
                ...
                return messages
        messages.append(MemoryGeneralizationMessage(role="user", content=context_text))
        return messages
    ...
    def _normalize_messages(
        self, args: GeneralizingMemoryCapabilitiesArgs, system_prompt: str
    ) -> list[MemoryGeneralizationMessage]:
        messages: list[MemoryGeneralizationMessage] = []
        if args.messages:
            for msg in args.messages:
                ...
                if not role:
                    role = "user"
                messages.append(MemoryGeneralizationMessage(role=role, content=content))
        elif args.prompt and args.prompt.strip():
            messages.append(
                MemoryGeneralizationMessage(role="user", content=args.prompt.strip())
            )
        else:
    ...
    def _call_llm(
        self,
        messages: list[MemoryGeneralizationMessage],
        args: GeneralizingMemoryCapabilitiesArgs,
    ) -> str:
    ...
    def _resolve_model(self, args: GeneralizingMemoryCapabilitiesArgs) -> str:
        return args.llm_model or self.config.llm_model

    @classmethod
    def get_call_display(cls, event: ToolCallEvent) -> ToolCallDisplay:
        if not isinstance(event.args, GeneralizingMemoryCapabilitiesArgs):
            return ToolCallDisplay(summary="generalizing_memory_capabilities")
        return ToolCallDisplay(
            summary="generalizing_memory_capabilities",
            details={
    ...
    def get_result_display(cls, event: ToolResultEvent) -> ToolResultDisplay:
        if not isinstance(event.result, GeneralizingMemoryCapabilitiesResult):
            return ToolResultDisplay(
                success=False, message=event.error or event.skip_reason or "No result"
            )
        message = "Generalizing memory capabilities complete"
        if event.result.errors:
            message = "Generalizing memory capabilities finished with errors"
        return ToolResultDisplay(
    ...
    def get_status_text(cls) -> str:
        return "Generalizing memory capabilities"