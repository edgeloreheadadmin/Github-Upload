...
# Source : tools/conditional_habit_compatibility.py
...
DEFAULT_PROMPT_TEMPLATE = """### INHERITED CONDITIONS MODE (OPT-IN)
Work with inherited affirmations, conditions, and compatibility logic.
- Identify inheritance paths, condition gates, and retention cues.
...
TOOL_PROMPT = (
    "Use conditional_habit_compatibility to work with inherited affirmations/conditions or compatibility logic. "
    "Provide prompt or messages, and optionally set focus, show_steps, and max_results."
)


class InheritedConditionMessage(BaseModel):
    role: str
    content: str


class InheritedConditionArgs(BaseModel):
    prompt: str | None = Field(default=None, description="User prompt to solve.")
    messages: list[InheritedConditionMessage] | None = Field(
...
class InheritedConditionResult(BaseModel):
    answer: str
    system_prompt: str
    messages: list[InheritedConditionMessage]
    used_focus: str
    show_steps: bool
    max_results: int
    template_source: str
...
class InheritedConditionConfig(BaseToolConfig):
    ...
    default_focus: str = Field(
        default="Apply conditional compatibility to habits or behaviors and nullify effects when conditions break.",
    ...
    prompt_path: Path | None = Field(
        ...
        / "core"
        / "prompts"
        / "conditional_habit_compatibility.md",
        description="Optional path to a prompt template.",
    )
    prompt_max_chars: int = Field(default=8000, description="Maximum template characters to load.")


class InheritedConditionState(BaseToolState):
    pass


class ConditionalHabitCompatibility(
    BaseTool[
        InheritedConditionArgs,
        InheritedConditionResult,
        InheritedConditionConfig,
        InheritedConditionState,
    ],
    ToolUIData[InheritedConditionArgs, InheritedConditionResult],
):
    description: ClassVar[str] = "Apply conditional compatibility to habits or behaviors and nullify effects when conditions break."

    @classmethod
    def get_tool_prompt(cls) -> str | None:
        return TOOL_PROMPT

    async def run(self, args: InheritedConditionArgs) -> InheritedConditionResult:
        warnings: list[str] = []
        errors: list[str] = []

        focus = (args.focus or self.config.default_focus).strip() or "inherited conditions"
        ...
        return InheritedConditionResult(
            answer=answer,
    ...
    def _validate_llm_settings(self, args: InheritedConditionArgs) -> None:
    ...
    def _normalize_messages(self, args: InheritedConditionArgs, system_prompt: str) -> list[InheritedConditionMessage]:
        messages: list[InheritedConditionMessage] = []
        if args.messages:
            for msg in args.messages:
                ...
                if content:
                    messages.append(InheritedConditionMessage(role=role, content=content))
        elif args.prompt and args.prompt.strip():
            messages.append(InheritedConditionMessage(role="user", content=args.prompt.strip()))
        else:
            raise ToolError("Provide prompt or messages.")
        if not messages:
            raise ToolError("No usable messages provided.")
        if system_prompt.strip():
            messages.insert(0, InheritedConditionMessage(role="system", content=system_prompt.strip()))
        return messages

    def _call_llm(self, messages: list[InheritedConditionMessage], args: InheritedConditionArgs) -> str:
    ...
    def _resolve_model(self, args: InheritedConditionArgs) -> str:
        return args.llm_model or self.config.llm_model

    @classmethod
    def get_call_display(cls, event: ToolCallEvent) -> ToolCallDisplay:
        if not isinstance(event.args, InheritedConditionArgs):
            return ToolCallDisplay(summary="conditional_habit_compatibility")
        return ToolCallDisplay(
            summary="conditional_habit_compatibility",
            details={
    ...
    def get_result_display(cls, event: ToolResultEvent) -> ToolResultDisplay:
        if not isinstance(event.result, InheritedConditionResult):
            return ToolResultDisplay(success=False, message=event.error or event.skip_reason or "No result")
        message = "Inherited conditions processing complete"
        if event.result.errors:
            message = "Inherited conditions processing finished with errors"
    ...
    def get_status_text(cls) -> str:
        return "Inherited conditions processing complete"