...
# Source : tools/inheritance_structs_passive_abilities.py
...
DEFAULT_PROMPT_TEMPLATE = """### INHERITANCE / HABIT MODE (OPT-IN)
Work with inheritance structures, habit stacking, and skill trees.
- Identify inheritance paths, habit links, and skill effects.
...
TOOL_PROMPT = (
    "Use inheritance_structs_passive_abilities for inheritance structures, habit stacking, or skill-tree effects. "
    "Provide prompt or messages, and optionally set focus, show_steps, and max_results."
)


class InheritanceHabitMessage(BaseModel):
    role: str
    content: str


class InheritanceHabitArgs(BaseModel):
    prompt: str | None = Field(default=None, description="User prompt to solve.")
    messages: list[InheritanceHabitMessage] | None = Field(
...
class InheritanceHabitResult(BaseModel):
    answer: str
    system_prompt: str
    messages: list[InheritanceHabitMessage]
    used_focus: str
    show_steps: bool
    max_results: int
    template_source: str
...
class InheritanceHabitConfig(BaseToolConfig):
    ...
    default_focus: str = Field(
        default="Inheritance structs passing passive abilities across structures/classes.",
    ...
    prompt_path: Path | None = Field(
        ...
        / "core"
        / "prompts"
        / "inheritance_structs_passive_abilities.md",
        description="Optional path to a prompt template.",
    )
    prompt_max_chars: int = Field(default=8000, description="Maximum template characters to load.")


class InheritanceHabitState(BaseToolState):
    pass


class InheritanceStructsPassiveAbilities(
    BaseTool[
        InheritanceHabitArgs,
        InheritanceHabitResult,
        InheritanceHabitConfig,
        InheritanceHabitState,
    ],
    ToolUIData[InheritanceHabitArgs, InheritanceHabitResult],
):
    description: ClassVar[str] = "Pass passive abilities across inheritance structures/classes."

    @classmethod
    def get_tool_prompt(cls) -> str | None:
        return TOOL_PROMPT

    async def run(self, args: InheritanceHabitArgs) -> InheritanceHabitResult:
        warnings: list[str] = []
        errors: list[str] = []

        focus = (args.focus or self.config.default_focus).strip() or "inheritance habits"
    ...
    def _validate_llm_settings(self, args: InheritanceHabitArgs) -> None:
    ...
    def _normalize_messages(self, args: InheritanceHabitArgs, system_prompt: str) -> list[InheritanceHabitMessage]:
        messages: list[InheritanceHabitMessage] = []
        if args.messages:
            for msg in args.messages:
                ...
                if content:
                    messages.append(InheritanceHabitMessage(role=role, content=content))
        elif args.prompt and args.prompt.strip():
            messages.append(InheritanceHabitMessage(role="user", content=args.prompt.strip()))
        else:
            raise ToolError("Provide prompt or messages.")
        if not messages:
            raise ToolError("No usable messages provided.")
        if system_prompt.strip():
            messages.insert(0, InheritanceHabitMessage(role="system", content=system_prompt.strip()))
        return messages

    def _call_llm(self, messages: list[InheritanceHabitMessage], args: InheritanceHabitArgs) -> str:
    ...
    def _resolve_model(self, args: InheritanceHabitArgs) -> str:
        return args.llm_model or self.config.llm_model

    @classmethod
    def get_call_display(cls, event: ToolCallEvent) -> ToolCallDisplay:
        if not isinstance(event.args, InheritanceHabitArgs):
            return ToolCallDisplay(summary="inheritance_structs_passive_abilities")
        return ToolCallDisplay(
            summary="inheritance_structs_passive_abilities",
            details={
    ...
    def get_result_display(cls, event: ToolResultEvent) -> ToolResultDisplay:
        if not isinstance(event.result, InheritanceHabitResult):
            return ToolResultDisplay(success=False, message=event.error or event.skip_reason or "No result")
        message = "Inheritance/habit processing complete"
        if event.result.errors:
            message = "Inheritance/habit processing finished with errors"
    ...
    def get_status_text(cls) -> str:
        return "Inheritance/habit processing complete"