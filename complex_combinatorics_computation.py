...
# Source : tools/complex_combinatorics_computation.py
...
DEFAULT_PROMPT_TEMPLATE = """### COMBINATORICS / SUBCLASS MODE (OPT-IN)
Work with combinatorics computation and subclass inheritance.
- Identify combinatorics constraints, inheritance paths, and type links.
...
TOOL_PROMPT = (
    "Use complex_combinatorics_computation for combinatorics computation or subclass inheritance. "
    "Provide prompt or messages, and optionally set focus, show_steps, and max_results."
)


class CombinatoricsMessage(BaseModel):
    role: str
    content: str


class CombinatoricsArgs(BaseModel):
    prompt: str | None = Field(default=None, description="User prompt to solve.")
    messages: list[CombinatoricsMessage] | None = Field(
...
class CombinatoricsResult(BaseModel):
    answer: str
    system_prompt: str
    messages: list[CombinatoricsMessage]
    used_focus: str
    show_steps: bool
    max_results: int
    template_source: str
...
class CombinatoricsConfig(BaseToolConfig):
    ...
    default_focus: str = Field(
        default="Compute combinations across complex combinatorics systems.",
    ...
    prompt_path: Path | None = Field(
        ...
        / "core"
        / "prompts"
        / "complex_combinatorics_computation.md",
        description="Optional path to a prompt template.",
    )
    prompt_max_chars: int = Field(default=8000, description="Maximum template characters to load.")


class CombinatoricsState(BaseToolState):
    pass


class ComplexCombinatoricsComputation(
    BaseTool[
        CombinatoricsArgs,
        CombinatoricsResult,
        CombinatoricsConfig,
    ...
    ToolUIData[CombinatoricsArgs, CombinatoricsResult],
):
    description: ClassVar[str] = "Compute combinations across complex combinatorics systems."

    @classmethod
    def get_tool_prompt(cls) -> str | None:
        return TOOL_PROMPT

    async def run(self, args: CombinatoricsArgs) -> CombinatoricsResult:
        warnings: list[str] = []
        errors: list[str] = []

        focus = (args.focus or self.config.default_focus).strip() or "combinatorics"
    ...
    def _validate_llm_settings(self, args: CombinatoricsArgs) -> None:
    ...
    def _normalize_messages(self, args: CombinatoricsArgs, system_prompt: str) -> list[CombinatoricsMessage]:
        messages: list[CombinatoricsMessage] = []
        if args.messages:
            for msg in args.messages:
                ...
                if content:
                    messages.append(CombinatoricsMessage(role=role, content=content))
        elif args.prompt and args.prompt.strip():
            messages.append(CombinatoricsMessage(role="user", content=args.prompt.strip()))
        else:
            raise ToolError("Provide prompt or messages.")
        if not messages:
            raise ToolError("No usable messages provided.")
        if system_prompt.strip():
            messages.insert(0, CombinatoricsMessage(role="system", content=system_prompt.strip()))
        return messages

    def _call_llm(self, messages: list[CombinatoricsMessage], args: CombinatoricsArgs) -> str:
    ...
    def _resolve_model(self, args: CombinatoricsArgs) -> str:
        return args.llm_model or self.config.llm_model

    @classmethod
    def get_call_display(cls, event: ToolCallEvent) -> ToolCallDisplay:
        if not isinstance(event.args, CombinatoricsArgs):
            return ToolCallDisplay(summary="complex_combinatorics_computation")
        return ToolCallDisplay(
            summary="complex_combinatorics_computation",
            details={
    ...
    def get_result_display(cls, event: ToolResultEvent) -> ToolResultDisplay:
        if not isinstance(event.result, CombinatoricsResult):
            return ToolResultDisplay(success=False, message=event.error or event.skip_reason or "No result")
        message = "Combinatorics/subclass processing complete"
        if event.result.errors:
            message = "Combinatorics/subclass processing finished with errors"
    ...
    def get_status_text(cls) -> str:
        return "Combinatorics/subclass processing complete"