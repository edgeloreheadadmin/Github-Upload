...
# Source : tools/membrane_calculations_time_series_forecasting.py
...
DEFAULT_PROMPT_TEMPLATE = """### MEMBRANE CALCULATIONS REMEMBERING DATA + TIME SERIES FORECASTING MODE (OPT-IN)
Apply membrane-style calculations when remembering data with time series forecasting.
- Identify forecasting cues, boundaries, and flow constraints.
...
TOOL_PROMPT = (
    "Use `membrane_calculations_time_series_forecasting` to apply membrane "
    "calculations when remembering data with time series forecasting. Provide "
    "`prompt` or `messages`, and optionally set `focus`, `show_steps`, and "
    "`max_results`."
)


class MembraneTimeSeriesMessage(BaseModel):
    role: str
    content: str


class MembraneTimeSeriesArgs(BaseModel):
    prompt: str | None = Field(default=None, description="User prompt to solve.")
    messages: list[MembraneTimeSeriesMessage] | None = Field(
...
class MembraneTimeSeriesResult(BaseModel):
    answer: str
    system_prompt: str
    messages: list[MembraneTimeSeriesMessage]
    used_focus: str
    show_steps: bool
    max_results: int
    template_source: str
...
class MembraneTimeSeriesConfig(BaseToolConfig):
    ...
    default_focus: str = Field(
        default="Membrane calculations for time series forecasting.",
    ...
    prompt_path: Path | None = Field(
        ...
        / "core"
        / "prompts"
        / "membrane_calculations_time_series_forecasting.md",
        description="Optional path to a prompt template.",
    )
    prompt_max_chars: int = Field(default=8000, description="Maximum template characters to load.")


class MembraneTimeSeriesState(BaseToolState):
    pass


class MembraneCalculationsTimeSeriesForecasting(
    BaseTool[
        MembraneTimeSeriesArgs,
        MembraneTimeSeriesResult,
        MembraneTimeSeriesConfig,
        MembraneTimeSeriesState,
    ],
    ToolUIData[MembraneTimeSeriesArgs, MembraneTimeSeriesResult],
):
    description: ClassVar[str] = "Apply membrane calculations for time series forecasting."

    @classmethod
    def get_tool_prompt(cls) -> str | None:
        return TOOL_PROMPT

    async def run(self, args: MembraneTimeSeriesArgs) -> MembraneTimeSeriesResult:
        warnings: list[str] = []
        errors: list[str] = []
        focus = (args.focus or self.config.default_focus).strip() or "time series forecasting"
        ...
        return MembraneTimeSeriesResult(
            answer=answer,
    ...
    def _validate_llm_settings(self, args: MembraneTimeSeriesArgs) -> None:
    ...
    def _normalize_messages(self, args: MembraneTimeSeriesArgs, system_prompt: str) -> list[MembraneTimeSeriesMessage]:
        messages: list[MembraneTimeSeriesMessage] = []
        if args.messages:
            for msg in args.messages:
                ...
                if content:
                    messages.append(MembraneTimeSeriesMessage(role=role, content=content))
        elif args.prompt and args.prompt.strip():
            messages.append(MembraneTimeSeriesMessage(role="user", content=args.prompt.strip()))
        else:
            raise ToolError("Provide prompt or messages.")
        if not messages:
            raise ToolError("No usable messages provided.")
        if system_prompt.strip():
            messages.insert(0, MembraneTimeSeriesMessage(role="system", content=system_prompt.strip()))
        return messages

    def _call_llm(self, messages: list[MembraneTimeSeriesMessage], args: MembraneTimeSeriesArgs) -> str:
    ...
    def _resolve_model(self, args: MembraneTimeSeriesArgs) -> str:
        return args.llm_model or self.config.llm_model

    @classmethod
    def get_call_display(cls, event: ToolCallEvent) -> ToolCallDisplay:
        if not isinstance(event.args, MembraneTimeSeriesArgs):
            return ToolCallDisplay(summary="membrane_calculations_time_series_forecasting")
        return ToolCallDisplay(
            summary="membrane_calculations_time_series_forecasting",
            details={
    ...
    def get_result_display(cls, event: ToolResultEvent) -> ToolResultDisplay:
        if not isinstance(event.result, MembraneTimeSeriesResult):
            return ToolResultDisplay(success=False, message=event.error or event.skip_reason or "No result")
        message = "Membrane calculations for time series forecasting complete"
        if event.result.errors:
            message = "Membrane calculations for time series forecasting finished with errors"
    ...
    def get_status_text(cls) -> str:
        return "Membrane calculations time series forecasting"