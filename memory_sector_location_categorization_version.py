...
# Source : tools/memory_sector_location_categorization_version.py
...
DEFAULT_PROMPT_TEMPLATE = """### MEMORY RECOGNITION MODE (OPT-IN)
Work with memory linkage correlations, patterns, frequencies, and sectors.
- Identify correlation anchors, pattern cues, and sector categorization.
...
TOOL_PROMPT = (
    "Use memory_sector_location_categorization_version for memory correlation/pattern/frequency recognition or sector categorization. "
    "Provide prompt or messages, and optionally set focus, show_steps, and max_results."
)


class MemoryRecognitionMessage(BaseModel):
    role: str
    content: str


class MemoryRecognitionArgs(BaseModel):
    prompt: str | None = Field(default=None, description="User prompt to solve.")
    messages: list[MemoryRecognitionMessage] | None = Field(
...
class MemoryRecognitionResult(BaseModel):
    answer: str
    system_prompt: str
    messages: list[MemoryRecognitionMessage]
    used_focus: str
    show_steps: bool
    max_results: int
    template_source: str
...
class MemoryRecognitionConfig(BaseToolConfig):
    ...
    default_focus: str = Field(
        default="Recognize memory location/allocation sectors with sector retrieval and central categorization.",
    ...
    prompt_path: Path | None = Field(
        ...
        / "core"
        / "prompts"
        / "memory_sector_location_categorization_version.md",
        description="Optional path to a prompt template.",
    )
    prompt_max_chars: int = Field(default=8000, description="Maximum template characters to load.")


class MemoryRecognitionState(BaseToolState):
    pass


class MemorySectorLocationCategorizationVersion(
    BaseTool[
        MemoryRecognitionArgs,
        MemoryRecognitionResult,
        MemoryRecognitionConfig,
        MemoryRecognitionState,
    ],
    ToolUIData[MemoryRecognitionArgs, MemoryRecognitionResult],
):
    description: ClassVar[str] = "Recognize memory location/allocation sectors with retrieval and central categorization."

    @classmethod
    def get_tool_prompt(cls) -> str | None:
        return TOOL_PROMPT

    async def run(self, args: MemoryRecognitionArgs) -> MemoryRecognitionResult:
        warnings: list[str] = []
        errors: list[str] = []

        focus = (args.focus or self.config.default_focus).strip() or "memory recognition"
        ...
        return MemoryRecognitionResult(
            answer=answer,
    ...
    def _validate_llm_settings(self, args: MemoryRecognitionArgs) -> None:
    ...
    def _normalize_messages(self, args: MemoryRecognitionArgs, system_prompt: str) -> list[MemoryRecognitionMessage]:
        messages: list[MemoryRecognitionMessage] = []
        if args.messages:
            for msg in args.messages:
                ...
                if content:
                    messages.append(MemoryRecognitionMessage(role=role, content=content))
        elif args.prompt and args.prompt.strip():
            messages.append(MemoryRecognitionMessage(role="user", content=args.prompt.strip()))
        else:
            raise ToolError("Provide prompt or messages.")
        if not messages:
            raise ToolError("No usable messages provided.")
        if system_prompt.strip():
            messages.insert(0, MemoryRecognitionMessage(role="system", content=system_prompt.strip()))
        return messages

    def _call_llm(self, messages: list[MemoryRecognitionMessage], args: MemoryRecognitionArgs) -> str:
    ...
    def _resolve_model(self, args: MemoryRecognitionArgs) -> str:
        return args.llm_model or self.config.llm_model

    @classmethod
    def get_call_display(cls, event: ToolCallEvent) -> ToolCallDisplay:
        if not isinstance(event.args, MemoryRecognitionArgs):
            return ToolCallDisplay(summary="memory_sector_location_categorization_version")
        return ToolCallDisplay(
            summary="memory_sector_location_categorization_version",
            details={
    ...
    def get_result_display(cls, event: ToolResultEvent) -> ToolResultDisplay:
        if not isinstance(event.result, MemoryRecognitionResult):
            return ToolResultDisplay(success=False, message=event.error or event.skip_reason or "No result")
        message = "Memory recognition processing complete"
        if event.result.errors:
            message = "Memory recognition processing finished with errors"
    ...
    def get_status_text(cls) -> str:
        return "Memory recognition processing complete"