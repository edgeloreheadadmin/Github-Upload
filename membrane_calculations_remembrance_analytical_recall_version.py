...
# Source : tools/membrane_calculations_remembrance_analytical_recall_version.py
...
DEFAULT_PROMPT_TEMPLATE = """### MEMORY CATEGORY / GENRE MODE (OPT-IN)
Work with memory categories, genre classification, intuitive retrieval, and membrane calculations.
- Identify category anchors, genre cues, and recall/recollection signals.
...
TOOL_PROMPT = (
    "Use membrane_calculations_remembrance_analytical_recall_version for memory category creation, genre classification, intuitive retrieval, or membrane calculations. "
    "Provide prompt or messages, and optionally set focus, show_steps, and max_results."
)


class MemoryCategoryMessage(BaseModel):
    role: str
    content: str


class MemoryCategoryArgs(BaseModel):
    prompt: str | None = Field(default=None, description="User prompt to solve.")
    messages: list[MemoryCategoryMessage] | None = Field(
...
class MemoryCategoryResult(BaseModel):
    answer: str
    system_prompt: str
    messages: list[MemoryCategoryMessage]
    used_focus: str
    show_steps: bool
    max_results: int
    template_source: str
...
class MemoryCategoryConfig(BaseToolConfig):
    ...
    default_focus: str = Field(
        default="Membrane calculations for memory remembrance/remembering with analytical recollection or recall.",
    ...
    prompt_path: Path | None = Field(
        ...
        / "core"
        / "prompts"
        / "membrane_calculations_remembrance_analytical_recall_version.md",
        description="Optional path to a prompt template.",
    )
    prompt_max_chars: int = Field(default=8000, description="Maximum template characters to load.")


class MemoryCategoryState(BaseToolState):
    pass


class MembraneCalculationsRemembranceAnalyticalRecallVersion(
    BaseTool[
        MemoryCategoryArgs,
        MemoryCategoryResult,
        MemoryCategoryConfig,
        MemoryCategoryState,
    ],
    ToolUIData[MemoryCategoryArgs, MemoryCategoryResult],
):
    description: ClassVar[str] = "Membrane calculations for memory remembrance/remembering with analytical recollection or recall."

    @classmethod
    def get_tool_prompt(cls) -> str | None:
        return TOOL_PROMPT

    async def run(self, args: MemoryCategoryArgs) -> MemoryCategoryResult:
        warnings: list[str] = []
        errors: list[str] = []

        focus = (args.focus or self.config.default_focus).strip() or "memory categories"
    ...
    def _validate_llm_settings(self, args: MemoryCategoryArgs) -> None:
    ...
    def _normalize_messages(self, args: MemoryCategoryArgs, system_prompt: str) -> list[MemoryCategoryMessage]:
        messages: list[MemoryCategoryMessage] = []
        if args.messages:
            for msg in args.messages:
                ...
                if content:
                    messages.append(MemoryCategoryMessage(role=role, content=content))
        elif args.prompt and args.prompt.strip():
            messages.append(MemoryCategoryMessage(role="user", content=args.prompt.strip()))
        else:
            raise ToolError("Provide prompt or messages.")
        if not messages:
            raise ToolError("No usable messages provided.")
        if system_prompt.strip():
            messages.insert(0, MemoryCategoryMessage(role="system", content=system_prompt.strip()))
        return messages

    def _call_llm(self, messages: list[MemoryCategoryMessage], args: MemoryCategoryArgs) -> str:
    ...
    def _resolve_model(self, args: MemoryCategoryArgs) -> str:
        return args.llm_model or self.config.llm_model

    @classmethod
    def get_call_display(cls, event: ToolCallEvent) -> ToolCallDisplay:
        if not isinstance(event.args, MemoryCategoryArgs):
            return ToolCallDisplay(summary="membrane_calculations_remembrance_analytical_recall_version")
        return ToolCallDisplay(
            summary="membrane_calculations_remembrance_analytical_recall_version",
            details={
    ...
    def get_result_display(cls, event: ToolResultEvent) -> ToolResultDisplay:
        if not isinstance(event.result, MemoryCategoryResult):
            return ToolResultDisplay(success=False, message=event.error or event.skip_reason or "No result")
        message = "Memory category processing complete"
        if event.result.errors:
            message = "Memory category processing finished with errors"
    ...
    def get_status_text(cls) -> str:
        return "Memory category processing complete"