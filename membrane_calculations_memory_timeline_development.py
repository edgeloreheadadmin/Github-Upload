...
# Source : tools/membrane_calculations_memory_timeline_development.py
...
DEFAULT_PROMPT_TEMPLATE = """### MEMBRANE CALCULATIONS REMEMBERING DATA + MEMORY TIMELINE MODE (OPT-IN)
Apply membrane-style calculations when remembering data with memory timeline development.
- Identify timeline cues, boundaries, and flow constraints.
...
TOOL_PROMPT = (
    "Use `membrane_calculations_memory_timeline_development` to apply membrane "
    "calculations when remembering data with memory timeline development. Provide "
    "`prompt` or `messages`, and optionally set `focus`, `show_steps`, and "
    "`max_results`."
)


class MembraneTimelineMessage(BaseModel):
    role: str
    content: str


class MembraneTimelineArgs(BaseModel):
    prompt: str | None = Field(default=None, description="User prompt to solve.")
    messages: list[MembraneTimelineMessage] | None = Field(
...
class MembraneTimelineResult(BaseModel):
    answer: str
    system_prompt: str
    messages: list[MembraneTimelineMessage]
    used_focus: str
    show_steps: bool
    max_results: int
    template_source: str
...
class MembraneTimelineConfig(BaseToolConfig):
    ...
    default_focus: str = Field(
        default="Membrane calculations for memory timeline development.",
    ...
    prompt_path: Path | None = Field(
        ...
        / "core"
        / "prompts"
        / "membrane_calculations_memory_timeline_development.md",
        description="Optional path to a prompt template.",
    )
    prompt_max_chars: int = Field(default=8000, description="Maximum template characters to load.")


class MembraneTimelineState(BaseToolState):
    pass


class MembraneCalculationsMemoryTimelineDevelopment(
    BaseTool[
        MembraneTimelineArgs,
        MembraneTimelineResult,
        MembraneTimelineConfig,
        MembraneTimelineState,
    ],
    ToolUIData[MembraneTimelineArgs, MembraneTimelineResult],
):
    description: ClassVar[str] = "Apply membrane calculations when remembering data with timeline development."

    @classmethod
    def get_tool_prompt(cls) -> str | None:
        return TOOL_PROMPT

    async def run(self, args: MembraneTimelineArgs) -> MembraneTimelineResult:
        warnings: list[str] = []
        errors: list[str] = []
        focus = (args.focus or self.config.default_focus).strip() or "memory timeline"
    ...
    def _validate_llm_settings(self, args: MembraneTimelineArgs) -> None:
    ...
    def _normalize_messages(self, args: MembraneTimelineArgs, system_prompt: str) -> list[MembraneTimelineMessage]:
        messages: list[MembraneTimelineMessage] = []
        if args.messages:
            for msg in args.messages:
                ...
                if content:
                    messages.append(MembraneTimelineMessage(role=role, content=content))
        elif args.prompt and args.prompt.strip():
            messages.append(MembraneTimelineMessage(role="user", content=args.prompt.strip()))
        else:
            raise ToolError("Provide prompt or messages.")
        if not messages:
            raise ToolError("No usable messages provided.")
        if system_prompt.strip():
            messages.insert(0, MembraneTimelineMessage(role="system", content=system_prompt.strip()))
        return messages

    def _call_llm(self, messages: list[MembraneTimelineMessage], args: MembraneTimelineArgs) -> str:
    ...
    def _resolve_model(self, args: MembraneTimelineArgs) -> str:
        return args.llm_model or self.config.llm_model

    @classmethod
    def get_call_display(cls, event: ToolCallEvent) -> ToolCallDisplay:
        if not isinstance(event.args, MembraneTimelineArgs):
            return ToolCallDisplay(summary="membrane_calculations_memory_timeline_development")
        return ToolCallDisplay(
            summary="membrane_calculations_memory_timeline_development",
            details={
    ...
    def get_result_display(cls, event: ToolResultEvent) -> ToolResultDisplay:
        if not isinstance(event.result, MembraneTimelineResult):
            return ToolResultDisplay(success=False, message=event.error or event.skip_reason or "No result")
        message = "Membrane calculations for memory timeline development complete"
        if event.result.errors:
            message = "Membrane calculations for memory timeline development finished with errors"
    ...
    def get_status_text(cls) -> str:
        return "Membrane calculations memory timeline development"