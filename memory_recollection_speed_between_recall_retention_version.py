...
# Source : tools/memory_recollection_speed_between_recall_retention_version.py
...
DEFAULT_PROMPT_TEMPLATE = """### MEMORY SPEED MODE (OPT-IN)
Work with memory recall/retrieval speed and location linking.
- Identify speed constraints, recall cues, and location anchors.
...
TOOL_PROMPT = (
    "Use memory_recollection_speed_between_recall_retention_version for memory speed or location-linking guidance. "
    "Provide prompt or messages, and optionally set focus, show_steps, and max_results."
)


class MemorySpeedMessage(BaseModel):
    role: str
    content: str


class MemorySpeedArgs(BaseModel):
    prompt: str | None = Field(default=None, description="User prompt to solve.")
    messages: list[MemorySpeedMessage] | None = Field(
...
class MemorySpeedResult(BaseModel):
    answer: str
    system_prompt: str
    messages: list[MemorySpeedMessage]
    used_focus: str
    show_steps: bool
    max_results: int
    template_source: str
...
class MemorySpeedConfig(BaseToolConfig):
    ...
    prompt_path: Path | None = Field(
        ...
        / "core"
        / "prompts"
        / "memory_recollection_speed_between_recall_retention_version.md",
        description="Optional path to a prompt template.",
    )
    prompt_max_chars: int = Field(default=8000, description="Maximum template characters to load.")


class MemorySpeedState(BaseToolState):
    pass


class MemoryRecollectionSpeedBetweenRecallRetentionVersion(
    BaseTool[
        MemorySpeedArgs,
        MemorySpeedResult,
        MemorySpeedConfig,
    ...
    ToolUIData[MemorySpeedArgs, MemorySpeedResult],
):
    description: ClassVar[str] = "Memory recollection speed between recall and retention."

    @classmethod
    def get_tool_prompt(cls) -> str | None:
        return TOOL_PROMPT

    async def run(self, args: MemorySpeedArgs) -> MemorySpeedResult:
        warnings: list[str] = []
        errors: list[str] = []

        focus = (args.focus or self.config.default_focus).strip() or "memory speed"
    ...
    def _validate_llm_settings(self, args: MemorySpeedArgs) -> None:
    ...
    def _normalize_messages(self, args: MemorySpeedArgs, system_prompt: str) -> list[MemorySpeedMessage]:
        messages: list[MemorySpeedMessage] = []
        if args.messages:
            for msg in args.messages:
                ...
                if content:
                    messages.append(MemorySpeedMessage(role=role, content=content))
        elif args.prompt and args.prompt.strip():
            messages.append(MemorySpeedMessage(role="user", content=args.prompt.strip()))
        else:
            raise ToolError("Provide prompt or messages.")
        if not messages:
            raise ToolError("No usable messages provided.")
        if system_prompt.strip():
            messages.insert(0, MemorySpeedMessage(role="system", content=system_prompt.strip()))
        return messages

    def _call_llm(self, messages: list[MemorySpeedMessage], args: MemorySpeedArgs) -> str:
    ...
    def _resolve_model(self, args: MemorySpeedArgs) -> str:
        return args.llm_model or self.config.llm_model

    @classmethod
    def get_call_display(cls, event: ToolCallEvent) -> ToolCallDisplay:
        if not isinstance(event.args, MemorySpeedArgs):
            return ToolCallDisplay(summary="memory_recollection_speed_between_recall_retention_version")
        return ToolCallDisplay(
            summary="memory_recollection_speed_between_recall_retention_version",
            details={
    ...
    def get_result_display(cls, event: ToolResultEvent) -> ToolResultDisplay:
        if not isinstance(event.result, MemorySpeedResult):
            return ToolResultDisplay(success=False, message=event.error or event.skip_reason or "No result")
        message = "Memory speed processing complete"
        if event.result.errors:
            message = "Memory speed processing finished with errors"
    ...
    def get_status_text(cls) -> str:
        return "Memory speed processing complete"