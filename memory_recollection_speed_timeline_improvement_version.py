...
# Source : tools/memory_recollection_speed_timeline_improvement_version.py
...
Work with timeline-based memory classification, speed improvements, and cache prompting.
- Identify timeline anchors, time position cues, and cache prompts.
...
TOOL_PROMPT = (
    "Use memory_recollection_speed_timeline_improvement_version for timeline memory classification, speed improvements, or cache prompting. "
    "Provide prompt or messages, and optionally set focus, show_steps, and max_results."
)


class TimelineMemoryMessage(BaseModel):
    role: str
    content: str


class TimelineMemoryArgs(BaseModel):
    prompt: str | None = Field(default=None, description="User prompt to solve.")
    messages: list[TimelineMemoryMessage] | None = Field(
...
class TimelineMemoryResult(BaseModel):
    answer: str
    system_prompt: str
    messages: list[TimelineMemoryMessage]
    used_focus: str
    show_steps: bool
    max_results: int
    template_source: str
...
class TimelineMemoryConfig(BaseToolConfig):
    ...
    default_focus: str = Field(
        default="Recollection speed improvements based on timeline.",
    ...
    prompt_path: Path | None = Field(
        ...
        / "core"
        / "prompts"
        / "memory_recollection_speed_timeline_improvement_version.md",
        description="Optional path to a prompt template.",
    )
    prompt_max_chars: int = Field(default=8000, description="Maximum template characters to load.")


class TimelineMemoryState(BaseToolState):
    pass


class MemoryRecollectionSpeedTimelineImprovementVersion(
    BaseTool[
        TimelineMemoryArgs,
        TimelineMemoryResult,
        TimelineMemoryConfig,
        TimelineMemoryState,
    ],
    ToolUIData[TimelineMemoryArgs, TimelineMemoryResult],
):
    description: ClassVar[str] = "Recollection speed improvements based on timeline."

    @classmethod
    def get_tool_prompt(cls) -> str | None:
        return TOOL_PROMPT

    async def run(self, args: TimelineMemoryArgs) -> TimelineMemoryResult:
        warnings: list[str] = []
        errors: list[str] = []

        focus = (args.focus or self.config.default_focus).strip() or "timeline memory"
    ...
    def _validate_llm_settings(self, args: TimelineMemoryArgs) -> None:
    ...
    def _normalize_messages(self, args: TimelineMemoryArgs, system_prompt: str) -> list[TimelineMemoryMessage]:
        messages: list[TimelineMemoryMessage] = []
        if args.messages:
            for msg in args.messages:
                ...
                if content:
                    messages.append(TimelineMemoryMessage(role=role, content=content))
        elif args.prompt and args.prompt.strip():
            messages.append(TimelineMemoryMessage(role="user", content=args.prompt.strip()))
        else:
            raise ToolError("Provide prompt or messages.")
        if not messages:
            raise ToolError("No usable messages provided.")
        if system_prompt.strip():
            messages.insert(0, TimelineMemoryMessage(role="system", content=system_prompt.strip()))
        return messages

    def _call_llm(self, messages: list[TimelineMemoryMessage], args: TimelineMemoryArgs) -> str:
    ...
    def _resolve_model(self, args: TimelineMemoryArgs) -> str:
        return args.llm_model or self.config.llm_model

    @classmethod
    def get_call_display(cls, event: ToolCallEvent) -> ToolCallDisplay:
        if not isinstance(event.args, TimelineMemoryArgs):
            return ToolCallDisplay(summary="memory_recollection_speed_timeline_improvement_version")
        return ToolCallDisplay(
            summary="memory_recollection_speed_timeline_improvement_version",
            details={
    ...
    def get_result_display(cls, event: ToolResultEvent) -> ToolResultDisplay:
        if not isinstance(event.result, TimelineMemoryResult):
            return ToolResultDisplay(success=False, message=event.error or event.skip_reason or "No result")
        message = "Timeline memory processing complete"
        if event.result.errors:
            message = "Timeline memory processing finished with errors"
    ...
    def get_status_text(cls) -> str:
        return "Timeline memory processing complete"