...
# Source : tools/genre_classification_recognition_general.py
...
TOOL_PROMPT = (
    "Use `genre_classification_recognition_general` to recognize genre classifications. "
    "Provide `prompt` or `messages`, and optionally set `focus`, `show_steps`, "
    "and `max_genres`."
)


class GenreRecognitionMessage(BaseModel):
    role: str
    content: str


class GenreRecognitionArgs(BaseModel):
    prompt: str | None = Field(
        default=None, description="User prompt to solve."
    )
    messages: list[GenreRecognitionMessage] | None = Field(
...
class GenreRecognitionResult(BaseModel):
    answer: str
    system_prompt: str
    messages: list[GenreRecognitionMessage]
    used_focus: str
    show_steps: bool
    max_genres: int
    template_source: str
...
class GenreRecognitionConfig(BaseToolConfig):
    ...
    default_focus: str = Field(
        default="Genre classification recognition.",
    ...
    default_max_genres: int = Field(
        default=6, description="Default max genres."
    )
    prompt_path: Path | None = Field(
        ...
        / "core"
        / "prompts"
        / "genre_classification_recognition_general.md",
...
class GenreRecognitionState(BaseToolState):
    pass


class GenreClassificationRecognitionGeneral(
    BaseTool[
        GenreRecognitionArgs,
        GenreRecognitionResult,
        GenreRecognitionConfig,
        GenreRecognitionState,
    ],
    ToolUIData[GenreRecognitionArgs, GenreRecognitionResult],
):
    description: ClassVar[str] = (
        "Recognize genre classifications with a dedicated prompt."
    )
    ...
    async def run(
        self, args: GenreRecognitionArgs
    ) -> GenreRecognitionResult:
        warnings: list[str] = []
        errors: list[str] = []
        ...
        if not focus:
            focus = "genre classification recognition"
    ...
    def _validate_llm_settings(self, args: GenreRecognitionArgs) -> None:
    ...
    def _normalize_messages(
        self, args: GenreRecognitionArgs, system_prompt: str
    ) -> list[GenreRecognitionMessage]:
        messages: list[GenreRecognitionMessage] = []
        if args.messages:
            for msg in args.messages:
                ...
                if not role:
                    role = "user"
                messages.append(
                    GenreRecognitionMessage(role=role, content=content)
                )
        elif args.prompt and args.prompt.strip():
            messages.append(
                GenreRecognitionMessage(
    ...
    def _resolve_model(self, args: GenreRecognitionArgs) -> str:
        return args.llm_model or self.config.llm_model

    @classmethod
    def get_call_display(cls, event: ToolCallEvent) -> ToolCallDisplay:
        if not isinstance(event.args, GenreRecognitionArgs):
            return ToolCallDisplay(summary="genre_classification_recognition_general")
        return ToolCallDisplay(
            summary="genre_classification_recognition_general",
            details={
    ...
    def get_result_display(cls, event: ToolResultEvent) -> ToolResultDisplay:
        if not isinstance(event.result, GenreRecognitionResult):
