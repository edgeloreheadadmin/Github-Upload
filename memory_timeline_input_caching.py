...
# Source : tools/memory_timeline_input_caching.py
...
DEFAULT_PROMPT_TEMPLATE = """### MEMORY TIMELINE META-DATA MODE (OPT-IN)
Work with memory timelines, timeframes, and cached inputs.
- Identify timeline anchors, timeframe buckets, and caching behavior.
...
TOOL_PROMPT = (
    "Use memory_timeline_input_caching to analyze memory timelines, timeframes, or cached inputs. "
    "Provide prompt or messages, and optionally set focus, show_steps, and max_results."
)


class MemoryTimelineMetaMessage(BaseModel):
    role: str
    content: str


class MemoryTimelineMetaArgs(BaseModel):
    prompt: str | None = Field(default=None, description="User prompt to solve.")
    messages: list[MemoryTimelineMetaMessage] | None = Field(
...
class MemoryTimelineMetaResult(BaseModel):
    answer: str
    system_prompt: str
    messages: list[MemoryTimelineMetaMessage]
    used_focus: str
    show_steps: bool
    max_results: int
    template_source: str
...
class MemoryTimelineMetaConfig(BaseToolConfig):
    ...
    default_focus: str = Field(
        default="Explain how timeframe input caching creates timeline inputs for memory.",
    ...
    prompt_path: Path | None = Field(
        ...
        / "core"
        / "prompts"
        / "memory_timeline_input_caching.md",
        description="Optional path to a prompt template.",
    )
    prompt_max_chars: int = Field(default=8000, description="Maximum template characters to load.")


class MemoryTimelineMetaState(BaseToolState):
    pass


class MemoryTimelineInputCaching(
    BaseTool[
        MemoryTimelineMetaArgs,
        MemoryTimelineMetaResult,
        MemoryTimelineMetaConfig,
        MemoryTimelineMetaState,
    ],
    ToolUIData[MemoryTimelineMetaArgs, MemoryTimelineMetaResult],
):
    description: ClassVar[str] = "Model timeframe input caching and its role in building memory timelines."

    @classmethod
    def get_tool_prompt(cls) -> str | None:
        return TOOL_PROMPT

    async def run(self, args: MemoryTimelineMetaArgs) -> MemoryTimelineMetaResult:
        warnings: list[str] = []
        errors: list[str] = []

        focus = (args.focus or self.config.default_focus).strip() or "memory timeline meta-data"
        ...
        return MemoryTimelineMetaResult(
            answer=answer,
    ...
    def _validate_llm_settings(self, args: MemoryTimelineMetaArgs) -> None:
    ...
    def _normalize_messages(self, args: MemoryTimelineMetaArgs, system_prompt: str) -> list[MemoryTimelineMetaMessage]:
        messages: list[MemoryTimelineMetaMessage] = []
        if args.messages:
            for msg in args.messages:
                ...
                if content:
                    messages.append(MemoryTimelineMetaMessage(role=role, content=content))
        elif args.prompt and args.prompt.strip():
            messages.append(MemoryTimelineMetaMessage(role="user", content=args.prompt.strip()))
        else:
            raise ToolError("Provide prompt or messages.")
        if not messages:
            raise ToolError("No usable messages provided.")
        if system_prompt.strip():
            messages.insert(0, MemoryTimelineMetaMessage(role="system", content=system_prompt.strip()))
        return messages

    def _call_llm(self, messages: list[MemoryTimelineMetaMessage], args: MemoryTimelineMetaArgs) -> str:
    ...
    def _resolve_model(self, args: MemoryTimelineMetaArgs) -> str:
        return args.llm_model or self.config.llm_model

    @classmethod
    def get_call_display(cls, event: ToolCallEvent) -> ToolCallDisplay:
        if not isinstance(event.args, MemoryTimelineMetaArgs):
            return ToolCallDisplay(summary="memory_timeline_input_caching")
        return ToolCallDisplay(
            summary="memory_timeline_input_caching",
            details={
    ...
    def get_result_display(cls, event: ToolResultEvent) -> ToolResultDisplay:
        if not isinstance(event.result, MemoryTimelineMetaResult):
            return ToolResultDisplay(success=False, message=event.error or event.skip_reason or "No result")
        message = "Memory timeline meta-data processing complete"
        if event.result.errors:
            message = "Memory timeline meta-data processing finished with errors"
    ...
    def get_status_text(cls) -> str:
        return "Memory timeline meta-data processing complete"