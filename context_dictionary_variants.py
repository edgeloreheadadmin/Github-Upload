...
VARIANT_SYNONYMS = {
    ...
    "anglosaxon": "anglo_saxon",
    "anglo_saxon": "anglo_saxon",
    "anglo-saxon": "anglo_saxon",
    "modernenglish": "modern_english",
    "modern_english": "modern_english",
    "modern": "modern",
    "british": "british",
    "britishold": "british_old",
    "british_old": "british_old",
    "britishmodern": "british_modern",
    "british_modern": "british_modern",
    "american": "american",
    "americanstandard": "american_standard",
...
class ContextDictionaryVariantsConfig(BaseToolConfig):
    ...
    max_variant_overlaps: int = Field(
        default=200, description="Maximum variant overlap entries returned."
    )
    max_variant_recommendations: int = Field(
        default=5, description="Maximum variant recommendations per chunk."
    )


class ContextDictionaryVariantsState(BaseToolState):
    pass
...
class DictionaryVariantSource(BaseModel):
    ...
    variant: str | None = Field(default=None, description="Primary variant label.")
    variants: list[str] | None = Field(
        default=None, description="Additional variant labels."
    )
    era: str | None = Field(default=None, description="Era tag (old, modern).")
    region: str | None = Field(default=None, description="Region tag (british, american).")
    register_: str | None = Field(
        default=None, alias="register", description="Register tag (academic, standard)."
    )
    institution: str | None = Field(
        default=None, description="Institution tag (college, university).")
...
class ContextDictionaryVariantsArgs(BaseModel):
    dictionaries: list[DictionaryVariantSource] = Field(
    ...
    max_variant_overlaps: int | None = Field(
        default=None, description="Override max_variant_overlaps."
    )
    max_variant_recommendations: int | None = Field(
        default=None, description="Override max_variant_recommendations."
    )
...
class ContextSignals(BaseModel):
    archaic_pronouns: int
    archaic_suffixes: int
    british_spellings: int
    american_spellings: int
    academic_markers: int
    modern_markers: int
...
class ChunkAnalysis(BaseModel):
    ...
    variant_usages: list[VariantUsage]
    variant_recommendations: list[VariantRecommendation]
...
class VariantOverlap(BaseModel):
    left_tag: str
    right_tag: str
    shared_words: int
    similarity: float
...
class ContextDictionaryVariantsResult(BaseModel):
    dictionaries: list[DictionarySummary]
    chunks: list[ChunkAnalysis]
    dictionary_overlaps: list[DictionaryOverlap]
    variant_overlaps: list[VariantOverlap]
    dictionary_count: int
    chunk_count: int
    overlap_count: int
    variant_overlap_count: int
    truncated: bool
    errors: list[str]

class ContextDictionaryVariants(
    BaseTool[
        ContextDictionaryVariantsArgs,
        ContextDictionaryVariantsResult,
        ContextDictionaryVariantsConfig,
        ContextDictionaryVariantsState,
    ],
    ToolUIData[ContextDictionaryVariantsArgs, ContextDictionaryVariantsResult],
):
    description: ClassVar[str] = (
        "Chunk and reason across English variants using multiple dictionaries."
    )

    async def run(
        self, args: ContextDictionaryVariantsArgs
    ) -> ContextDictionaryVariantsResult:
        ...
        max_variant_overlaps = (
            args.max_variant_overlaps
            if args.max_variant_overlaps is not None
            else self.config.max_variant_overlaps
        )
        max_variant_recommendations = (
            args.max_variant_recommendations
            if args.max_variant_recommendations is not None
            else self.config.max_variant_recommendations
        )
        ...
        if max_variant_overlaps < 0:
            raise ToolError("max_variant_overlaps must be >= 0.")
        if max_variant_recommendations < 0:
            raise ToolError("max_variant_recommendations must be >= 0.")

        dictionaries: list[DictionarySummary] = []
        dict_word_sets: dict[str, set[str]] = {}
        dict_names: dict[str, str | None] = {}
        dict_entries: dict[str, list[_DictEntry]] = {}
        dict_variant_tags: dict[str, set[str]] = {}
        errors: list[str] = []
        truncated = False
        ...
        for idx, source in enumerate(args.dictionaries, start=1):
            try:
                ...
                tags = self._build_variant_tags(source)
                ...
                dict_variant_tags[dict_id] = tags
        ...
        variant_overlaps, variant_truncated = self._build_variant_overlaps(
            dict_word_sets, dict_variant_tags, max_variant_overlaps
        )
        if variant_truncated:
            truncated = True

        return ContextDictionaryVariantsResult(
            ...
            variant_overlap_count=len(variant_overlaps),
            truncated=truncated,
            errors=errors,
        )

    def _resolve_dict_id(
        self, source: DictionaryVariantSource, index: int, used_ids: set[str]
    ) -> str:
    ...
    def _build_variant_tags(self, source: DictionaryVariantSource) -> set[str]:
        tags: set[str] = set()
        for value in [source.variant, source.era, source.region, source.register_, source.institution]:
            if value:
                tags.update(self._split_tags(value))
        for value in source.variants or []:
            tags.update(self._split_tags(value))
        ...
        for tag in normalized:
            if tag == "anglo_saxon":
                expanded.add("old_english")
            if tag == "british_old":
                expanded.add("british")
            if tag == "british_modern":
                expanded.add("british")
                expanded.add("modern")
            if tag == "modern_english":
                expanded.add("modern")
            if tag == "american_standard":
    ...
    def _normalize_tag(self, raw: str) -> str | None:
        ...
        if not cleaned:
            return None
        mapped = VARIANT_SYNONYMS.get(cleaned, cleaned)
        mapped = mapped.replace("-", "_")
        return mapped

    def _load_dictionary_entries(
        self,
        source: DictionaryVariantSource,
        dict_id: str,
        dict_name: str | None,
    ...
    def _load_dictionary_content(
        self, source: DictionaryVariantSource, max_dictionary_bytes: int
    ) -> str:
    ...
    def _analyze_chunk(
        ...
        preview_chars: int,
        max_variant_recommendations: int,
        ...
        variant_word_map: dict[str, set[str]] = {}
        variant_dicts: dict[str, set[str]] = {}
        ...
        for word in word_set:
            ...
            for dict_id in dict_ids:
                tags.update(dict_variant_tags.get(dict_id, set()))
                for tag in dict_variant_tags.get(dict_id, set()):
                    variant_word_map.setdefault(tag, set()).add(word)
                    variant_dicts.setdefault(tag, set()).add(dict_id)
        ...
        variant_usages = self._build_variant_usages(
            variant_word_map,
            variant_dicts,
            len(word_set),
            max_sample_words,
        ...
        available_tags = set()
        for tags in dict_variant_tags.values():
            available_tags.update(tags)
        signals = self._build_context_signals(word_freq)
        recommendations = self._recommend_variants(
            variant_usages, signals, available_tags, max_variant_recommendations
        )
    ...
    def _build_variant_usages(
        ...
        usages: list[VariantUsage] = []
        for tag, words in variant_word_map.items():
            ...
            usages.append(
                VariantUsage(
                    ...
                    coverage=round(coverage, 6),
                    dictionary_ids=sorted(variant_dicts.get(tag, set())),
                    sample_words=sample,
                )
            )
    ...
    def _build_context_signals(self, word_freq: dict[str, int]) -> ContextSignals:
        archaic_pronouns = sum(word_freq.get(word, 0) for word in ARCHAIC_PRONOUNS)
        archaic_suffixes = sum(
            count for word, count in word_freq.items() if word.endswith("eth") or word.endswith("est")
        )

        british_spellings = 0
        american_spellings = 0
        for british, american in BRITISH_AMERICAN_PAIRS:
            british_spellings += word_freq.get(british, 0)
            american_spellings += word_freq.get(american, 0)

        academic_markers = sum(word_freq.get(word, 0) for word in ACADEMIC_MARKERS)
        modern_markers = sum(word_freq.get(word, 0) for word in MODERN_MARKERS)

        return ContextSignals(
            archaic_pronouns=archaic_pronouns,
            archaic_suffixes=archaic_suffixes,
            british_spellings=british_spellings,
            american_spellings=american_spellings,
    ...
    def _recommend_variants(
        ...
        archaic_hits = signals.archaic_pronouns + signals.archaic_suffixes
        if archaic_hits > 0:
            for tag in ("old_english", "anglo_saxon", "british_old"):
                if tag in scores:
                    scores[tag] += 0.2
                    reasons[tag].append("archaic markers")

        if signals.british_spellings > signals.american_spellings:
            for tag in ("british", "british_modern", "british_old"):
                if tag in scores:
                    scores[tag] += 0.1
                    reasons[tag].append("british spelling")
        elif signals.american_spellings > signals.british_spellings:
            for tag in ("american", "american_standard"):
                if tag in scores:
                    scores[tag] += 0.1
                    reasons[tag].append("american spelling")

        if signals.academic_markers > 0:
            for tag in ("academic", "college"):
                if tag in scores:
                    scores[tag] += 0.1
                    reasons[tag].append("academic markers")

        if signals.modern_markers > 0:
            for tag in ("modern", "modern_english", "british_modern", "american_standard"):
                if tag in scores:
                    scores[tag] += 0.05
                    reasons[tag].append("modern markers")

        ranked = sorted(scores.items(), key=lambda item: (-item[1], item[0]))
        recommendations: list[VariantRecommendation] = []
        for tag, score in ranked:
            usage = next((u for u in variant_usages if u.tag == tag), None)
            ...
            if max_variant_recommendations > 0 and len(recommendations) >= max_variant_recommendations:
                break
    ...
    def _build_variant_overlaps(
        self,
        dict_word_sets: dict[str, set[str]],
        dict_variant_tags: dict[str, set[str]],
        max_variant_overlaps: int,
    ) -> tuple[list[VariantOverlap], bool]:
        variant_sets: dict[str, set[str]] = {}
        for dict_id, tags in dict_variant_tags.items():
            words = dict_word_sets.get(dict_id, set())
            for tag in tags:
                variant_sets.setdefault(tag, set()).update(words)

        tags = sorted(variant_sets.keys())
        overlaps: list[VariantOverlap] = []
        for i in range(len(tags)):
            for j in range(i + 1, len(tags)):
                left_tag = tags[i]
                right_tag = tags[j]
                left_words = variant_sets[left_tag]
                right_words = variant_sets[right_tag]
                shared = len(left_words & right_words)
                if shared == 0:
                    continue
                union = len(left_words | right_words)
        ...
        truncated = False
        if max_variant_overlaps > 0 and len(overlaps) > max_variant_overlaps:
            truncated = True
            overlaps = overlaps[:max_variant_overlaps]
        return overlaps, truncated

    @classmethod
    def get_call_display(cls, event: ToolCallEvent) -> ToolCallDisplay:
        if not isinstance(event.args, ContextDictionaryVariantsArgs):
            return ToolCallDisplay(summary="context_dictionary_variants")

        summary = f"context_dictionary_variants: {len(event.args.items)} item(s)"
    ...
    def get_result_display(cls, event: ToolResultEvent) -> ToolResultDisplay:
        if not isinstance(event.result, ContextDictionaryVariantsResult):
        ...
        return ToolResultDisplay(
            ...
            details={
                ...
                "variant_overlap_count": event.result.variant_overlap_count,
                "truncated": event.result.truncated,
                "errors": event.result.errors,
                "dictionaries": event.result.dictionaries,
                "chunks": event.result.chunks,
                "dictionary_overlaps": event.result.dictionary_overlaps,
                "variant_overlaps": event.result.variant_overlaps,
            },
        )

    @classmethod
    def get_status_text(cls) -> str:
        return "Analyzing English variants"
