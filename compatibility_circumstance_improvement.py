...
# Source : tools/compatibility_circumstance_improvement.py
...
DEFAULT_PROMPT_TEMPLATE = """### COMPATIBILITY + CIRCUMSTANCE IMPROVEMENT MODE (OPT-IN)
Improve outputs by aligning with compatibility requirements and current circumstances.
- Consider interfaces, dependencies, and environment constraints.
- Adapt to situational factors without unnecessary complexity.
- Note risks or tradeoffs when compatibility or circumstances are unclear.
...
TOOL_PROMPT = (
    "Use `compatibility_circumstance_improvement` to align responses with "
    "compatibility requirements and situational constraints. Provide `prompt` "
    "or `messages`, and optionally set `compatibility_focus`, `circumstance_focus`, "
    "`show_steps`, and `max_adjustments` to control the guidance."
)


class CompatibilityCircumstanceMessage(BaseModel):
    role: str
    content: str


class CompatibilityCircumstanceArgs(BaseModel):
    prompt: str | None = Field(
        default=None, description="User prompt to solve."
    )
    messages: list[CompatibilityCircumstanceMessage] | None = Field(
...
class CompatibilityCircumstanceResult(BaseModel):
    answer: str
    system_prompt: str
    messages: list[CompatibilityCircumstanceMessage]
...
class CompatibilityCircumstanceConfig(BaseToolConfig):
    ...
    prompt_path: Path | None = Field(
        ...
        / "core"
        / "prompts"
        / "compatibility_circumstance_improvement.md",
...
class CompatibilityCircumstanceState(BaseToolState):
    pass


class CompatibilityCircumstanceImprovement(
    BaseTool[
        CompatibilityCircumstanceArgs,
        CompatibilityCircumstanceResult,
        CompatibilityCircumstanceConfig,
        CompatibilityCircumstanceState,
    ],
    ToolUIData[CompatibilityCircumstanceArgs, CompatibilityCircumstanceResult],
):
    description: ClassVar[str] = (
        "Generate responses guided by compatibility and circumstance improvements."
    )
    ...
    async def run(
        self, args: CompatibilityCircumstanceArgs
    ) -> CompatibilityCircumstanceResult:
        warnings: list[str] = []
        errors: list[str] = []
        ...
        return CompatibilityCircumstanceResult(
            answer=answer,
    ...
    def _validate_llm_settings(self, args: CompatibilityCircumstanceArgs) -> None:
    ...
    def _normalize_messages(
        self, args: CompatibilityCircumstanceArgs, system_prompt: str
    ) -> list[CompatibilityCircumstanceMessage]:
        messages: list[CompatibilityCircumstanceMessage] = []
        if args.messages:
            for msg in args.messages:
                ...
                if not role:
                    role = "user"
                messages.append(
                    CompatibilityCircumstanceMessage(role=role, content=content)
                )
        elif args.prompt and args.prompt.strip():
            messages.append(
                CompatibilityCircumstanceMessage(
    ...
    def _call_llm(
        self,
        messages: list[CompatibilityCircumstanceMessage],
        args: CompatibilityCircumstanceArgs,
    ) -> str:
    ...
    def _resolve_model(self, args: CompatibilityCircumstanceArgs) -> str:
        return args.llm_model or self.config.llm_model

    @classmethod
    def get_call_display(cls, event: ToolCallEvent) -> ToolCallDisplay:
        if not isinstance(event.args, CompatibilityCircumstanceArgs):
            return ToolCallDisplay(summary="compatibility_circumstance_improvement")
        return ToolCallDisplay(
            summary="compatibility_circumstance_improvement",
            details={
    ...
    def get_result_display(cls, event: ToolResultEvent) -> ToolResultDisplay:
        if not isinstance(event.result, CompatibilityCircumstanceResult):
            return ToolResultDisplay(
                success=False, message=event.error or event.skip_reason or "No result"
            )
        message = "Compatibility + circumstance improvement complete"
        if event.result.errors:
            message = "Compatibility + circumstance improvement finished with errors"
        return ToolResultDisplay(
    ...
    def get_status_text(cls) -> str:
        return "Compatibility + circumstance improvement"