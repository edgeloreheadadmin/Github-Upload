...
# Source : tools/system_recursive_refresh_disconnect_negative.py
...
DEFAULT_PROMPT_TEMPLATE = """### SYSTEM HEALTH / SYNAPTIC MODE (OPT-IN)
Work with synaptic inheritance structures and system health maintenance.
- Identify structural inheritance, recovery steps, and cleanup cues.
...
TOOL_PROMPT = (
    "Use system_recursive_refresh_disconnect_negative for synaptic inheritance or system health recovery. "
...
class SystemHealthArgs(BaseModel):
    ...
    messages: list[SystemHealthMessage] | None = Field(
...
class SystemHealthResult(BaseModel):
    answer: str
    system_prompt: str
    messages: list[SystemHealthMessage]
    used_focus: str
    show_steps: bool
    max_results: int
    template_source: str
...
class SystemHealthConfig(BaseToolConfig):
    ...
    default_focus: str = Field(
        default="Recursively refresh the system and disconnect negative synaptic connections.",
    ...
    prompt_path: Path | None = Field(
        ...
        / "core"
        / "prompts"
        / "system_recursive_refresh_disconnect_negative.md",
        description="Optional path to a prompt template.",
    )
    prompt_max_chars: int = Field(default=8000, description="Maximum template characters to load.")


class SystemHealthState(BaseToolState):
    pass


class SystemRecursiveRefreshDisconnectNegative(
    BaseTool[
        SystemHealthArgs,
        SystemHealthResult,
        SystemHealthConfig,
    ...
    ToolUIData[SystemHealthArgs, SystemHealthResult],
):
    description: ClassVar[str] = "Recursively refresh the system and disconnect negative synaptic connections that impair function."

    @classmethod
    def get_tool_prompt(cls) -> str | None:
        return TOOL_PROMPT

    async def run(self, args: SystemHealthArgs) -> SystemHealthResult:
        warnings: list[str] = []
        errors: list[str] = []

        focus = (args.focus or self.config.default_focus).strip() or "system health"
    ...
    def _validate_llm_settings(self, args: SystemHealthArgs) -> None:
    ...
    def _normalize_messages(self, args: SystemHealthArgs, system_prompt: str) -> list[SystemHealthMessage]:
        messages: list[SystemHealthMessage] = []
        if args.messages:
            for msg in args.messages:
                ...
                if content:
                    messages.append(SystemHealthMessage(role=role, content=content))
        elif args.prompt and args.prompt.strip():
            messages.append(SystemHealthMessage(role="user", content=args.prompt.strip()))
        else:
            raise ToolError("Provide prompt or messages.")
        if not messages:
            raise ToolError("No usable messages provided.")
        if system_prompt.strip():
            messages.insert(0, SystemHealthMessage(role="system", content=system_prompt.strip()))
        return messages

    def _call_llm(self, messages: list[SystemHealthMessage], args: SystemHealthArgs) -> str:
    ...
    def _resolve_model(self, args: SystemHealthArgs) -> str:
        return args.llm_model or self.config.llm_model

    @classmethod
    def get_call_display(cls, event: ToolCallEvent) -> ToolCallDisplay:
        if not isinstance(event.args, SystemHealthArgs):
            return ToolCallDisplay(summary="system_recursive_refresh_disconnect_negative")
        return ToolCallDisplay(
            summary="system_recursive_refresh_disconnect_negative",
            details={
    ...
    def get_result_display(cls, event: ToolResultEvent) -> ToolResultDisplay:
        if not isinstance(event.result, SystemHealthResult):
            return ToolResultDisplay(success=False, message=event.error or event.skip_reason or "No result")
        message = "System health processing complete"
        if event.result.errors:
            message = "System health processing finished with errors"
    ...
    def get_status_text(cls) -> str:
        return "System health processing complete"