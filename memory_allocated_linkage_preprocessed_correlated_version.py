...
# Source : tools/memory_allocated_linkage_preprocessed_correlated_version.py
...
DEFAULT_PROMPT_TEMPLATE = """### MEMORY STRUCTURE MODELING MODE (OPT-IN)
Work with memory structure retrieval, modeling, and cross-referencing.
- Identify retrieval anchors, modeling cues, and linkage paths.
...
TOOL_PROMPT = (
    "Use memory_allocated_linkage_preprocessed_correlated_version for memory structure retrieval, modeling, or linkage. "
    "Provide prompt or messages, and optionally set focus, show_steps, and max_results."
)


class MemoryModelingMessage(BaseModel):
    role: str
    content: str


class MemoryModelingArgs(BaseModel):
    prompt: str | None = Field(default=None, description="User prompt to solve.")
    messages: list[MemoryModelingMessage] | None = Field(
...
class MemoryModelingResult(BaseModel):
    answer: str
    system_prompt: str
    messages: list[MemoryModelingMessage]
    used_focus: str
    show_steps: bool
    max_results: int
    template_source: str
...
class MemoryModelingConfig(BaseToolConfig):
    ...
    default_focus: str = Field(
        default="Memory allocated linkage between pre-processed and correlated memory patterns.",
    ...
    prompt_path: Path | None = Field(
        ...
        / "core"
        / "prompts"
        / "memory_allocated_linkage_preprocessed_correlated_version.md",
        description="Optional path to a prompt template.",
    )
    prompt_max_chars: int = Field(default=8000, description="Maximum template characters to load.")


class MemoryModelingState(BaseToolState):
    pass


class MemoryAllocatedLinkagePreprocessedCorrelatedVersion(
    BaseTool[
        MemoryModelingArgs,
        MemoryModelingResult,
        MemoryModelingConfig,
        MemoryModelingState,
    ],
    ToolUIData[MemoryModelingArgs, MemoryModelingResult],
):
    description: ClassVar[str] = "Memory allocated linkage between pre-processed and correlated memory patterns."

    @classmethod
    def get_tool_prompt(cls) -> str | None:
        return TOOL_PROMPT

    async def run(self, args: MemoryModelingArgs) -> MemoryModelingResult:
        warnings: list[str] = []
        errors: list[str] = []

        focus = (args.focus or self.config.default_focus).strip() or "memory modeling"
    ...
    def _validate_llm_settings(self, args: MemoryModelingArgs) -> None:
    ...
    def _normalize_messages(self, args: MemoryModelingArgs, system_prompt: str) -> list[MemoryModelingMessage]:
        messages: list[MemoryModelingMessage] = []
        if args.messages:
            for msg in args.messages:
                ...
                if content:
                    messages.append(MemoryModelingMessage(role=role, content=content))
        elif args.prompt and args.prompt.strip():
            messages.append(MemoryModelingMessage(role="user", content=args.prompt.strip()))
        else:
            raise ToolError("Provide prompt or messages.")
        if not messages:
            raise ToolError("No usable messages provided.")
        if system_prompt.strip():
            messages.insert(0, MemoryModelingMessage(role="system", content=system_prompt.strip()))
        return messages

    def _call_llm(self, messages: list[MemoryModelingMessage], args: MemoryModelingArgs) -> str:
    ...
    def _resolve_model(self, args: MemoryModelingArgs) -> str:
        return args.llm_model or self.config.llm_model

    @classmethod
    def get_call_display(cls, event: ToolCallEvent) -> ToolCallDisplay:
        if not isinstance(event.args, MemoryModelingArgs):
            return ToolCallDisplay(summary="memory_allocated_linkage_preprocessed_correlated_version")
        return ToolCallDisplay(
            summary="memory_allocated_linkage_preprocessed_correlated_version",
            details={
    ...
    def get_result_display(cls, event: ToolResultEvent) -> ToolResultDisplay:
        if not isinstance(event.result, MemoryModelingResult):
            return ToolResultDisplay(success=False, message=event.error or event.skip_reason or "No result")
        message = "Memory modeling processing complete"
        if event.result.errors:
            message = "Memory modeling processing finished with errors"
    ...
    def get_status_text(cls) -> str:
        return "Memory modeling processing complete"