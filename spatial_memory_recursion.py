...
# Source : tools/spatial_memory_recursion.py
...
DEFAULT_PROMPT_TEMPLATE = """### MEMORY MODE (OPT-IN)
Work with spatial or temporal memory and recall rate behaviors.
- Identify anchors, recursion or temporal cues, and recall metrics.
...
TOOL_PROMPT = (
    "Use spatial_memory_recursion for spatial/temporal memory or recall-rate guidance. "
    "Provide prompt or messages, and optionally set focus, show_steps, and max_results."
)


class MemoryModeMessage(BaseModel):
    role: str
    content: str


class MemoryModeArgs(BaseModel):
    prompt: str | None = Field(default=None, description="User prompt to solve.")
    messages: list[MemoryModeMessage] | None = Field(
...
class MemoryModeResult(BaseModel):
    answer: str
    system_prompt: str
    messages: list[MemoryModeMessage]
    used_focus: str
    show_steps: bool
    max_results: int
    template_source: str
...
class MemoryModeConfig(BaseToolConfig):
    ...
    default_focus: str = Field(
        default="Use spatial memory with recursion.",
...
class SpatialMemoryRecursion(
    ...
    ToolUIData[MemoryModeArgs, MemoryModeResult],
):
    description: ClassVar[str] = "Use spatial memory with recursion."

    @classmethod
    def get_tool_prompt(cls) -> str | None:
        return TOOL_PROMPT

    async def run(self, args: MemoryModeArgs) -> MemoryModeResult:
        warnings: list[str] = []
        errors: list[str] = []

        focus = (args.focus or self.config.default_focus).strip() or "memory"
    ...
    def _validate_llm_settings(self, args: MemoryModeArgs) -> None:
    ...
    def _normalize_messages(self, args: MemoryModeArgs, system_prompt: str) -> list[MemoryModeMessage]:
        messages: list[MemoryModeMessage] = []
        if args.messages:
            for msg in args.messages:
                ...
                if content:
                    messages.append(MemoryModeMessage(role=role, content=content))
        elif args.prompt and args.prompt.strip():
            messages.append(MemoryModeMessage(role="user", content=args.prompt.strip()))
        else:
            raise ToolError("Provide prompt or messages.")
        if not messages:
            raise ToolError("No usable messages provided.")
        if system_prompt.strip():
            messages.insert(0, MemoryModeMessage(role="system", content=system_prompt.strip()))
        return messages

    def _call_llm(self, messages: list[MemoryModeMessage], args: MemoryModeArgs) -> str:
    ...
    def _resolve_model(self, args: MemoryModeArgs) -> str:
        return args.llm_model or self.config.llm_model

    @classmethod
    def get_call_display(cls, event: ToolCallEvent) -> ToolCallDisplay:
        if not isinstance(event.args, MemoryModeArgs):
            return ToolCallDisplay(summary="spatial_memory_recursion")
        return ToolCallDisplay(
            summary="spatial_memory_recursion",
            details={
    ...
    def get_result_display(cls, event: ToolResultEvent) -> ToolResultDisplay:
        if not isinstance(event.result, MemoryModeResult):
        ...
        if event.result.errors:
            message = "Memory processing finished with errors"
