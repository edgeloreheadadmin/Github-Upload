...
# Source : tools/pdf_vector_search.py
...
class PdfVectorSearchConfig(BaseToolConfig):
...
class PdfVectorSearchState(BaseToolState):
    pass


class PdfVectorSearchArgs(BaseModel):
...
class PdfVectorSearchResultItem(BaseModel):
    score: float
    source_path: str
    chunk_index: int
    unit: str
...
class PdfVectorSearchResult(BaseModel):
    results: list[PdfVectorSearchResultItem]
    count: int
    db_path: str
...
class PdfVectorSearch(
    BaseTool[
        PdfVectorSearchArgs,
        PdfVectorSearchResult,
        PdfVectorSearchConfig,
        PdfVectorSearchState,
    ],
    ToolUIData[PdfVectorSearchArgs, PdfVectorSearchResult],
):
    description: ClassVar[str] = "Search indexed PDF vectors using Ollama/GPT-OSS embeddings."

    async def run(self, args: PdfVectorSearchArgs) -> PdfVectorSearchResult:
        ...
        results: list[tuple[float, PdfVectorSearchResultItem]] = []
        with sqlite3.connect(str(db_path)) as conn:
            ...
            for row in cursor:
                ...
                if args.min_score is not None and score < args.min_score:
                    continue
                item = PdfVectorSearchResultItem(
                    score=score,
                    source_path=row[0],
                    chunk_index=row[1],
                    unit=row[2],
        ...
        results_sorted = [item for _, item in sorted(results, key=lambda r: r[0], reverse=True)]
        return PdfVectorSearchResult(
    ...
    def get_call_display(cls, event: ToolCallEvent) -> ToolCallDisplay:
        if not isinstance(event.args, PdfVectorSearchArgs):
            return ToolCallDisplay(summary="pdf_vector_search")

        summary = "pdf_vector_search"
    ...
    def get_result_display(cls, event: ToolResultEvent) -> ToolResultDisplay:
        if not isinstance(event.result, PdfVectorSearchResult):
