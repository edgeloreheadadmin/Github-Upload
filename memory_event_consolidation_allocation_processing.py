...
# Source : tools/memory_event_consolidation_allocation_processing.py
...
DEFAULT_PROMPT_TEMPLATE = """### MEMORY EVENT/TIMELINE MODE (OPT-IN)
Work with memory events, timelines, and allocation/processing steps.
- Identify event anchors, timeline segments, and allocation cues.
...
TOOL_PROMPT = (
    "Use memory_event_consolidation_allocation_processing to classify or process memories within events/timelines. "
    "Provide prompt or messages, and optionally set focus, show_steps, and max_results."
)


class MemoryEventTimelineMessage(BaseModel):
    role: str
    content: str


class MemoryEventTimelineArgs(BaseModel):
    prompt: str | None = Field(default=None, description="User prompt to solve.")
    messages: list[MemoryEventTimelineMessage] | None = Field(
...
class MemoryEventTimelineResult(BaseModel):
    answer: str
    system_prompt: str
    messages: list[MemoryEventTimelineMessage]
    used_focus: str
    show_steps: bool
    max_results: int
    template_source: str
...
class MemoryEventTimelineConfig(BaseToolConfig):
    ...
    default_focus: str = Field(
        default="Allocate and process memories within events during consolidation.",
    ...
    prompt_path: Path | None = Field(
        ...
        / "core"
        / "prompts"
        / "memory_event_consolidation_allocation_processing.md",
        description="Optional path to a prompt template.",
    )
    prompt_max_chars: int = Field(default=8000, description="Maximum template characters to load.")


class MemoryEventTimelineState(BaseToolState):
    pass


class MemoryEventConsolidationAllocationProcessing(
    BaseTool[
        MemoryEventTimelineArgs,
        MemoryEventTimelineResult,
        MemoryEventTimelineConfig,
        MemoryEventTimelineState,
    ],
    ToolUIData[MemoryEventTimelineArgs, MemoryEventTimelineResult],
):
    description: ClassVar[str] = "Allocate and process memories within events during memory consolidation."

    @classmethod
    def get_tool_prompt(cls) -> str | None:
        return TOOL_PROMPT

    async def run(self, args: MemoryEventTimelineArgs) -> MemoryEventTimelineResult:
        warnings: list[str] = []
        errors: list[str] = []

        focus = (args.focus or self.config.default_focus).strip() or "memory events"
        ...
        return MemoryEventTimelineResult(
            answer=answer,
    ...
    def _validate_llm_settings(self, args: MemoryEventTimelineArgs) -> None:
    ...
    def _normalize_messages(self, args: MemoryEventTimelineArgs, system_prompt: str) -> list[MemoryEventTimelineMessage]:
        messages: list[MemoryEventTimelineMessage] = []
        if args.messages:
            for msg in args.messages:
                ...
                if content:
                    messages.append(MemoryEventTimelineMessage(role=role, content=content))
        elif args.prompt and args.prompt.strip():
            messages.append(MemoryEventTimelineMessage(role="user", content=args.prompt.strip()))
        else:
            raise ToolError("Provide prompt or messages.")
        if not messages:
            raise ToolError("No usable messages provided.")
        if system_prompt.strip():
            messages.insert(0, MemoryEventTimelineMessage(role="system", content=system_prompt.strip()))
        return messages

    def _call_llm(self, messages: list[MemoryEventTimelineMessage], args: MemoryEventTimelineArgs) -> str:
    ...
    def _resolve_model(self, args: MemoryEventTimelineArgs) -> str:
        return args.llm_model or self.config.llm_model

    @classmethod
    def get_call_display(cls, event: ToolCallEvent) -> ToolCallDisplay:
        if not isinstance(event.args, MemoryEventTimelineArgs):
            return ToolCallDisplay(summary="memory_event_consolidation_allocation_processing")
        return ToolCallDisplay(
            summary="memory_event_consolidation_allocation_processing",
            details={
    ...
    def get_result_display(cls, event: ToolResultEvent) -> ToolResultDisplay:
        if not isinstance(event.result, MemoryEventTimelineResult):
            return ToolResultDisplay(success=False, message=event.error or event.skip_reason or "No result")
        message = "Memory event/timeline processing complete"
        if event.result.errors:
            message = "Memory event/timeline processing finished with errors"
    ...
    def get_status_text(cls) -> str:
        return "Memory event/timeline processing complete"