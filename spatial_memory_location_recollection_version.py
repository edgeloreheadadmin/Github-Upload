...
# Source : tools/spatial_memory_location_recollection_version.py
...
DEFAULT_PROMPT_TEMPLATE = """### MEMORY MAINTENANCE MODE (OPT-IN)
Work with spatial recollection, memory refresh, and recovered data classification.
- Identify spatial anchors, refresh cues, and sector classification markers.
...
TOOL_PROMPT = (
    "Use spatial_memory_location_recollection_version for spatial recollection, memory refresh, or recovered data classification. "
    "Provide prompt or messages, and optionally set focus, show_steps, and max_results."
)


class MemoryMaintenanceMessage(BaseModel):
    role: str
    content: str


class MemoryMaintenanceArgs(BaseModel):
    prompt: str | None = Field(default=None, description="User prompt to solve.")
    messages: list[MemoryMaintenanceMessage] | None = Field(
...
class MemoryMaintenanceResult(BaseModel):
    answer: str
    system_prompt: str
    messages: list[MemoryMaintenanceMessage]
    used_focus: str
    show_steps: bool
    max_results: int
    template_source: str
...
class MemoryMaintenanceConfig(BaseToolConfig):
    ...
    default_focus: str = Field(
        default="Spatial memory recollection of specific spatial locations within memory.",
    ...
    prompt_path: Path | None = Field(
        ...
        / "core"
        / "prompts"
        / "spatial_memory_location_recollection_version.md",
        description="Optional path to a prompt template.",
    )
    prompt_max_chars: int = Field(default=8000, description="Maximum template characters to load.")


class MemoryMaintenanceState(BaseToolState):
    pass


class SpatialMemoryLocationRecollectionVersion(
    BaseTool[
        MemoryMaintenanceArgs,
        MemoryMaintenanceResult,
        MemoryMaintenanceConfig,
        MemoryMaintenanceState,
    ],
    ToolUIData[MemoryMaintenanceArgs, MemoryMaintenanceResult],
):
    description: ClassVar[str] = "Spatial memory recollection of specific spatial locations within memory."

    @classmethod
    def get_tool_prompt(cls) -> str | None:
        return TOOL_PROMPT

    async def run(self, args: MemoryMaintenanceArgs) -> MemoryMaintenanceResult:
        warnings: list[str] = []
        errors: list[str] = []

        focus = (args.focus or self.config.default_focus).strip() or "memory maintenance"
    ...
    def _validate_llm_settings(self, args: MemoryMaintenanceArgs) -> None:
    ...
    def _normalize_messages(self, args: MemoryMaintenanceArgs, system_prompt: str) -> list[MemoryMaintenanceMessage]:
        messages: list[MemoryMaintenanceMessage] = []
        if args.messages:
            for msg in args.messages:
                ...
                if content:
                    messages.append(MemoryMaintenanceMessage(role=role, content=content))
        elif args.prompt and args.prompt.strip():
            messages.append(MemoryMaintenanceMessage(role="user", content=args.prompt.strip()))
        else:
            raise ToolError("Provide prompt or messages.")
        if not messages:
            raise ToolError("No usable messages provided.")
        if system_prompt.strip():
            messages.insert(0, MemoryMaintenanceMessage(role="system", content=system_prompt.strip()))
        return messages

    def _call_llm(self, messages: list[MemoryMaintenanceMessage], args: MemoryMaintenanceArgs) -> str:
    ...
    def _resolve_model(self, args: MemoryMaintenanceArgs) -> str:
        return args.llm_model or self.config.llm_model

    @classmethod
    def get_call_display(cls, event: ToolCallEvent) -> ToolCallDisplay:
        if not isinstance(event.args, MemoryMaintenanceArgs):
            return ToolCallDisplay(summary="spatial_memory_location_recollection_version")
        return ToolCallDisplay(
            summary="spatial_memory_location_recollection_version",
            details={
    ...
    def get_result_display(cls, event: ToolResultEvent) -> ToolResultDisplay:
        if not isinstance(event.result, MemoryMaintenanceResult):
            return ToolResultDisplay(success=False, message=event.error or event.skip_reason or "No result")
        message = "Memory maintenance processing complete"
        if event.result.errors:
            message = "Memory maintenance processing finished with errors"
    ...
    def get_status_text(cls) -> str:
        return "Memory maintenance processing complete"