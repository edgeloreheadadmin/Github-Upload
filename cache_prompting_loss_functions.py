...
# Source : tools/cache_prompting_loss_functions.py
...
DEFAULT_PROMPT_TEMPLATE = """### CACHE PROMPTING: LOSS FUNCTIONS (OPT-IN)
Use selectable loss functions to adjust cache decision weights.
- Score cache candidates using similarity, length match, and recency.
- Apply the chosen loss function when feedback is provided.
...
TOOL_PROMPT = (
    "Use `cache_prompting_loss_functions` for cache prompting with selectable "
    "loss functions for weight updates."
)
...
VALID_LOSSES = {"hinge", "logistic", "mse"}
...
class CachePromptingLossMessage(BaseModel):
    role: str
    content: str


class CachePromptingLossArgs(BaseModel):
    prompt: str | None = Field(
        default=None, description="User prompt to solve."
    )
    messages: list[CachePromptingLossMessage] | None = Field(
    ...
    loss_weights: list[float] | None = Field(
        default=None, description="Override weight vector."
    )
    loss_bias: float | None = Field(
        default=None, description="Override bias."
    )
    decision_threshold: float | None = Field(
        default=None, description="Decision threshold for cache hit."
    )
    learning_rate: float | None = Field(
        default=None, description="Learning rate for updates."
    )
    regularization: float | None = Field(
        default=None, description="Regularization strength."
    )
    loss_function: str | None = Field(
        default=None, description="hinge, logistic, or mse."
    )
...
class CachePromptingLossResult(BaseModel):
    answer: str
    system_prompt: str
    messages: list[CachePromptingLossMessage]
    cache_mode: str
    cache_hit: bool
    cache_used: bool
...
class CachePromptingLossConfig(BaseToolConfig):
    ...
    default_loss_weights: list[float] = Field(
        default_factory=lambda: [0.6, 0.25, 0.15],
        description="Default weight vector.",
    )
    default_loss_bias: float = Field(
        default=0.0, description="Default bias."
    )
    default_decision_threshold: float = Field(
        default=0.5, description="Default decision threshold."
    )
    default_learning_rate: float = Field(
        default=0.2, description="Default learning rate."
    )
    default_regularization: float = Field(
        default=0.01, description="Default regularization strength."
    )
    default_loss_function: str = Field(
        default="logistic", description="Default loss function."
    )
    ...
    cache_path: Path = Field(
        ...
        / ".codex"
        / "cache"
        / "cache_prompting_loss_functions.json",
    ...
    weights_path: Path = Field(
        ...
        / ".codex"
        / "cache"
        / "cache_prompting_loss_weights.json",
    ...
    prompt_path: Path | None = Field(
        ...
        / "core"
        / "prompts"
        / "cache_prompting_loss_functions.md",
...
class CachePromptingLossState(BaseToolState):
    pass


class CachePromptingLossFunctions(
    BaseTool[
        CachePromptingLossArgs,
        CachePromptingLossResult,
        CachePromptingLossConfig,
        CachePromptingLossState,
    ],
    ToolUIData[CachePromptingLossArgs, CachePromptingLossResult],
):
    description: ClassVar[str] = (
        "Cache prompting with selectable loss functions for updates."
    )
    ...
    async def run(
        self, args: CachePromptingLossArgs
    ) -> CachePromptingLossResult:
        warnings: list[str] = []
        errors: list[str] = []
        ...
        decision_threshold = (
            ...
            else self._default_threshold(args.loss_function)
        )
        learning_rate = (
            args.learning_rate
        ...
        regularization = (
            ...
            else self.config.default_regularization
        )
        loss_function = (
            args.loss_function or self.config.default_loss_function
        ).strip().lower()
        show_steps = (
            args.show_steps
        ...
        self._validate_cache_settings(
            cache_similarity, cache_max_items, learning_rate, regularization
        )
        if loss_function not in VALID_LOSSES:
            raise ToolError(f"loss_function must be one of {sorted(VALID_LOSSES)}.")

        weights_path = self._resolve_weights_path(args.weights_path)
        weights, bias = self._load_weights(weights_path, warnings)
        if args.loss_weights is not None:
            weights = list(args.loss_weights)
        if args.loss_bias is not None:
            bias = args.loss_bias
        ...
        if cache_candidate:
            ...
            cache_score = cache_candidate.predicted
        ...
        if feedback_label is not None:
            if cache_candidate:
                weights, bias, weights_updated = self._apply_loss_update(
                    loss_function,
                    weights,
                    bias,
                    cache_candidate.features,
                    cache_candidate.raw_score,
                    feedback_label,
                    learning_rate,
                    regularization,
                )
        ...
        return CachePromptingLossResult(
            answer=answer,
    ...
    def _default_threshold(self, loss_function: str | None) -> float:
        if loss_function and loss_function.strip().lower() == "hinge":
            return 0.0
        return self.config.default_decision_threshold

    def _validate_llm_settings(self, args: CachePromptingLossArgs) -> None:
    ...
    def _render_template(
        ...
        rendered = rendered.replace("{loss_function}", loss_function)
        rendered = rendered.replace("{show_steps}", show_steps_text)
        return rendered

    def _normalize_messages(
        self, args: CachePromptingLossArgs, system_prompt: str
    ) -> list[CachePromptingLossMessage]:
        messages: list[CachePromptingLossMessage] = []
        if args.messages:
            for msg in args.messages:
                ...
                if not role:
                    role = "user"
                messages.append(
                    CachePromptingLossMessage(role=role, content=content)
                )
        elif args.prompt and args.prompt.strip():
            messages.append(
                CachePromptingLossMessage(
    ...
    def _build_prompt_text(
        self, messages: list[CachePromptingLossMessage]
    ) -> str:
    ...
    def _find_cache_candidate(
        ...
        for entry in entries:
            ...
            predicted = self._predict(raw_score, loss_function)
    ...
    def _predict(self, raw_score: float, loss_function: str) -> float:
        if loss_function in {"logistic", "mse"}:
    ...
    def _cache_decision(
        ...
        if candidate.similarity < similarity_threshold:
            return False
        return candidate.predicted >= decision_threshold

    def _apply_loss_update(
        self,
        loss_function: str,
        weights: list[float],
        bias: float,
        ...
        if loss_function == "hinge":
            margin = feedback_label * raw_score
            if margin < 1:
                updated = [
                    (1.0 - learning_rate * regularization) * w
                    + learning_rate * feedback_label * x
                    for w, x in zip(weights, features)
                ]
                return updated, bias + learning_rate * feedback_label, True
            updated = [
                (1.0 - learning_rate * regularization) * w for w in weights
            ]
            return updated, bias, False

        target = 1.0 if feedback_label == 1 else 0.0
        pred = self._sigmoid(raw_score)
        if loss_function == "logistic":
            error = pred - target
            updated = [
                w - learning_rate * (error * x + regularization * w)
                for w, x in zip(weights, features)
            ]
            return updated, bias - learning_rate * error, True

        error = pred - target
        grad = error * pred * (1.0 - pred)
        updated = [
            w - learning_rate * (grad * x + regularization * w)
            for w, x in zip(weights, features)
        ]
        return updated, bias - learning_rate * grad, True
    ...
    def _load_weights(
        ...
        if not path.exists():
            return (
                list(self.config.default_loss_weights),
                self.config.default_loss_bias,
            )
        try:
            raw = path.read_text("utf-8")
            data = json.loads(raw)
        ...
        if not isinstance(bias, (int, float)):
            bias = self.config.default_loss_bias
    ...
    def _resolve_model(self, args: CachePromptingLossArgs) -> str:
        return args.llm_model or self.config.llm_model

    @classmethod
    def get_call_display(cls, event: ToolCallEvent) -> ToolCallDisplay:
        if not isinstance(event.args, CachePromptingLossArgs):
            return ToolCallDisplay(summary="cache_prompting_loss_functions")
        return ToolCallDisplay(
            summary="cache_prompting_loss_functions",
            details={
                "cache_mode": event.args.cache_mode,
                "loss_function": event.args.loss_function,
    ...
    def get_result_display(cls, event: ToolResultEvent) -> ToolResultDisplay:
        if not isinstance(event.result, CachePromptingLossResult):
            return ToolResultDisplay(
                success=False, message=event.error or event.skip_reason or "No result"
            )
        message = "Cache prompting (loss functions) complete"
        if event.result.errors:
            message = "Cache prompting (loss functions) finished with errors"
        return ToolResultDisplay(
    ...
    def get_status_text(cls) -> str:
        return "Cache prompting (loss functions)"