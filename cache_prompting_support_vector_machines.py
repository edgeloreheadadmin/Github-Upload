...
# Source : tools/cache_prompting_support_vector_machines.py
...
DEFAULT_PROMPT_TEMPLATE = """### CACHE PROMPTING: SUPPORT VECTOR MACHINES (OPT-IN)
Use a linear SVM to decide cache hit classification.
- Score cache candidates using similarity, length match, and recency.
- Update weights with hinge loss when feedback is provided.
...
TOOL_PROMPT = (
    "Use `cache_prompting_support_vector_machines` for cache prompting driven "
    "by a linear SVM with optional feedback updates."
)
...
class CachePromptingSVMMessage(BaseModel):
    role: str
    content: str


class CachePromptingSVMArgs(BaseModel):
    prompt: str | None = Field(
        default=None, description="User prompt to solve."
    )
    messages: list[CachePromptingSVMMessage] | None = Field(
    ...
    svm_weights: list[float] | None = Field(
        default=None, description="Override SVM weights."
    )
    svm_bias: float | None = Field(
        default=None, description="Override SVM bias."
    )
    decision_threshold: float | None = Field(
        default=None, description="Decision threshold for cache hit."
    )
    learning_rate: float | None = Field(
        default=None, description="SVM learning rate."
    )
    regularization: float | None = Field(
        default=None, description="SVM regularization strength."
    )
...
class CachePromptingSVMResult(BaseModel):
    answer: str
    system_prompt: str
    messages: list[CachePromptingSVMMessage]
    cache_mode: str
    cache_hit: bool
    cache_used: bool
...
class CachePromptingSVMConfig(BaseToolConfig):
    ...
    default_svm_weights: list[float] = Field(
        default_factory=lambda: [0.6, 0.25, 0.15],
        description="Default SVM weights.",
    )
    default_svm_bias: float = Field(
        default=0.0, description="Default SVM bias."
    )
    ...
    default_learning_rate: float = Field(
        default=0.1, description="Default learning rate."
    )
    ...
    weights_path: Path = Field(
        ...
        / ".codex"
        / "cache"
        / "cache_prompting_svm_weights.json",
    ...
    prompt_path: Path | None = Field(
        ...
        / "core"
        / "prompts"
        / "cache_prompting_support_vector_machines.md",
...
class CachePromptingSVMState(BaseToolState):
    pass


class CachePromptingSupportVectorMachines(
    BaseTool[
        CachePromptingSVMArgs,
        CachePromptingSVMResult,
        CachePromptingSVMConfig,
        CachePromptingSVMState,
    ],
    ToolUIData[CachePromptingSVMArgs, CachePromptingSVMResult],
):
    description: ClassVar[str] = (
        "Cache prompting with a linear SVM and feedback updates."
    )
    ...
    async def run(
        self, args: CachePromptingSVMArgs
    ) -> CachePromptingSVMResult:
        warnings: list[str] = []
        errors: list[str] = []
        ...
        if args.svm_weights is not None:
            weights = list(args.svm_weights)
        if args.svm_bias is not None:
            bias = args.svm_bias
        ...
        if feedback_label is not None:
            if cache_candidate:
                weights, bias, weights_updated = self._apply_svm_update(
                    weights,
                    bias,
        ...
        return CachePromptingSVMResult(
            answer=answer,
    ...
    def _validate_llm_settings(self, args: CachePromptingSVMArgs) -> None:
    ...
    def _align_weights(self, weights: list[float], warnings: list[str]) -> list[float]:
        ...
        if len(weights) < target:
            warnings.append("SVM weights padded to match feature count.")
            weights = weights + [0.0] * (target - len(weights))
        elif len(weights) > target:
            warnings.append("SVM weights trimmed to match feature count.")
    ...
    def _normalize_messages(
        self, args: CachePromptingSVMArgs, system_prompt: str
    ) -> list[CachePromptingSVMMessage]:
        messages: list[CachePromptingSVMMessage] = []
        if args.messages:
            for msg in args.messages:
                ...
                if not role:
                    role = "user"
                messages.append(
                    CachePromptingSVMMessage(role=role, content=content)
                )
        elif args.prompt and args.prompt.strip():
            messages.append(
                CachePromptingSVMMessage(
    ...
    def _build_prompt_text(
        self, messages: list[CachePromptingSVMMessage]
    ) -> str:
    ...
    def _load_weights(
        ...
        if not path.exists():
            return (
                list(self.config.default_svm_weights),
                self.config.default_svm_bias,
            )
        try:
            raw = path.read_text("utf-8")
            data = json.loads(raw)
        ...
        if not isinstance(bias, (int, float)):
            bias = self.config.default_svm_bias
    ...
    def _resolve_model(self, args: CachePromptingSVMArgs) -> str:
        return args.llm_model or self.config.llm_model

    @classmethod
    def get_call_display(cls, event: ToolCallEvent) -> ToolCallDisplay:
        if not isinstance(event.args, CachePromptingSVMArgs):
            return ToolCallDisplay(summary="cache_prompting_support_vector_machines")
        return ToolCallDisplay(
            summary="cache_prompting_support_vector_machines",
            details={
    ...
    def get_result_display(cls, event: ToolResultEvent) -> ToolResultDisplay:
        if not isinstance(event.result, CachePromptingSVMResult):
            return ToolResultDisplay(
                success=False, message=event.error or event.skip_reason or "No result"
            )
        message = "Cache prompting (SVM) complete"
        if event.result.errors:
            message = "Cache prompting (SVM) finished with errors"
        return ToolResultDisplay(
