...
# Source : tools/spatial_memory_recall_recollected_multifactor_version.py
...
DEFAULT_PROMPT_TEMPLATE = """### MEMORY RECALL/FRAGMENT MODE (OPT-IN)
Work with spatial recall, perfect retention, and fragment-based recovery.
- Identify recall anchors, retention cues, and fragment reconstruction steps.
...
TOOL_PROMPT = (
    "Use spatial_memory_recall_recollected_multifactor_version for spatial recall, perfect retention, or fragment recovery. "
    "Provide prompt or messages, and optionally set focus, show_steps, and max_results."
)


class MemoryRecallMessage(BaseModel):
    role: str
    content: str
...
class MemoryRecallArgs(BaseModel):
    ...
    messages: list[MemoryRecallMessage] | None = Field(
...
class MemoryRecallResult(BaseModel):
    answer: str
    system_prompt: str
    messages: list[MemoryRecallMessage]
    used_focus: str
    show_steps: bool
    max_results: int
    template_source: str
...
class MemoryRecallConfig(BaseToolConfig):
    ...
    default_focus: str = Field(
        default="Spatial memory recall of recollected info based on category, location, timeframe, and timeline.",
    ...
    prompt_path: Path | None = Field(
        ...
        / "core"
        / "prompts"
        / "spatial_memory_recall_recollected_multifactor_version.md",
        description="Optional path to a prompt template.",
    )
    prompt_max_chars: int = Field(default=8000, description="Maximum template characters to load.")


class MemoryRecallState(BaseToolState):
    pass


class SpatialMemoryRecallRecollectedMultifactorVersion(
    BaseTool[
        MemoryRecallArgs,
        MemoryRecallResult,
        MemoryRecallConfig,
    ...
    ToolUIData[MemoryRecallArgs, MemoryRecallResult],
):
    description: ClassVar[str] = "Spatial memory recall of recollected info based on category, location, timeframe, and timeline."

    @classmethod
    def get_tool_prompt(cls) -> str | None:
        return TOOL_PROMPT

    async def run(self, args: MemoryRecallArgs) -> MemoryRecallResult:
        warnings: list[str] = []
        errors: list[str] = []

        focus = (args.focus or self.config.default_focus).strip() or "memory recall"
    ...
    def _validate_llm_settings(self, args: MemoryRecallArgs) -> None:
    ...
    def _normalize_messages(self, args: MemoryRecallArgs, system_prompt: str) -> list[MemoryRecallMessage]:
        messages: list[MemoryRecallMessage] = []
        if args.messages:
            for msg in args.messages:
                ...
                if content:
                    messages.append(MemoryRecallMessage(role=role, content=content))
        elif args.prompt and args.prompt.strip():
            messages.append(MemoryRecallMessage(role="user", content=args.prompt.strip()))
        else:
            raise ToolError("Provide prompt or messages.")
        if not messages:
            raise ToolError("No usable messages provided.")
        if system_prompt.strip():
            messages.insert(0, MemoryRecallMessage(role="system", content=system_prompt.strip()))
        return messages

    def _call_llm(self, messages: list[MemoryRecallMessage], args: MemoryRecallArgs) -> str:
    ...
    def _resolve_model(self, args: MemoryRecallArgs) -> str:
        return args.llm_model or self.config.llm_model

    @classmethod
    def get_call_display(cls, event: ToolCallEvent) -> ToolCallDisplay:
        if not isinstance(event.args, MemoryRecallArgs):
            return ToolCallDisplay(summary="spatial_memory_recall_recollected_multifactor_version")
        return ToolCallDisplay(
            summary="spatial_memory_recall_recollected_multifactor_version",
            details={
    ...
    def get_result_display(cls, event: ToolResultEvent) -> ToolResultDisplay:
        if not isinstance(event.result, MemoryRecallResult):
            return ToolResultDisplay(success=False, message=event.error or event.skip_reason or "No result")
        message = "Memory recall processing complete"
        if event.result.errors:
            message = "Memory recall processing finished with errors"
    ...
    def get_status_text(cls) -> str:
        return "Memory recall processing complete"