...
# Source : tools/inheritance_parent_child_systematic.py
...
DEFAULT_PROMPT_TEMPLATE = """### INHERITANCE STRUCTURES MODE (OPT-IN)
Work with inheritance across structures, domains, and temporal hierarchies.
- Identify inheritance paths, node relationships, and structural links.
...
TOOL_PROMPT = (
    "Use inheritance_parent_child_systematic for inheritance across structures or domains. "
    "Provide prompt or messages, focus, show_steps, and max_results."
)


class InheritanceStructureMessage(BaseModel):
    role: str
    content: str


class InheritanceStructureArgs(BaseModel):
    prompt: str | None = Field(default=None, description="User prompt to solve.")
    messages: list[InheritanceStructureMessage] | None = Field(
...
class InheritanceStructureResult(BaseModel):
    answer: str
    system_prompt: str
    messages: list[InheritanceStructureMessage]
    used_focus: str
    show_steps: bool
    max_results: int
    template_source: str
...
class InheritanceStructureConfig(BaseToolConfig):
    ...
    default_focus: str = Field(
        default="Inheritance between parent and child nodes within a systematic structure.",
    ...
    prompt_path: Path | None = Field(
        ...
        / "core"
        / "prompts"
        / "inheritance_parent_child_systematic.md",
        description="Optional path to a prompt template.",
    )
    prompt_max_chars: int = Field(default=8000, description="Maximum template characters to load.")


class InheritanceStructureState(BaseToolState):
    pass


class InheritanceParentChildSystematic(
    BaseTool[
        InheritanceStructureArgs,
        InheritanceStructureResult,
        InheritanceStructureConfig,
        InheritanceStructureState,
    ],
    ToolUIData[InheritanceStructureArgs, InheritanceStructureResult],
):
    description: ClassVar[str] = "Inheritance between parent and child nodes within a systematic structure."

    @classmethod
    def get_tool_prompt(cls) -> str | None:
        return TOOL_PROMPT

    async def run(self, args: InheritanceStructureArgs) -> InheritanceStructureResult:
        warnings: list[str] = []
        errors: list[str] = []

        focus = (args.focus or self.config.default_focus).strip() or "inheritance structures"
        ...
        return InheritanceStructureResult(
            answer=answer,
    ...
    def _validate_llm_settings(self, args: InheritanceStructureArgs) -> None:
    ...
    def _normalize_messages(self, args: InheritanceStructureArgs, system_prompt: str) -> list[InheritanceStructureMessage]:
        messages: list[InheritanceStructureMessage] = []
        if args.messages:
            for msg in args.messages:
                ...
                if content:
                    messages.append(InheritanceStructureMessage(role=role, content=content))
        elif args.prompt and args.prompt.strip():
            messages.append(InheritanceStructureMessage(role="user", content=args.prompt.strip()))
        else:
            raise ToolError("Provide prompt or messages.")
        if not messages:
            raise ToolError("No usable messages provided.")
        if system_prompt.strip():
            messages.insert(0, InheritanceStructureMessage(role="system", content=system_prompt.strip()))
        return messages

    def _call_llm(self, messages: list[InheritanceStructureMessage], args: InheritanceStructureArgs) -> str:
    ...
    def _resolve_model(self, args: InheritanceStructureArgs) -> str:
        return args.llm_model or self.config.llm_model

    @classmethod
    def get_call_display(cls, event: ToolCallEvent) -> ToolCallDisplay:
        if not isinstance(event.args, InheritanceStructureArgs):
            return ToolCallDisplay(summary="inheritance_parent_child_systematic")
        return ToolCallDisplay(
            summary="inheritance_parent_child_systematic",
            details={
    ...
    def get_result_display(cls, event: ToolResultEvent) -> ToolResultDisplay:
        if not isinstance(event.result, InheritanceStructureResult):
            return ToolResultDisplay(success=False, message=event.error or event.skip_reason or "No result")
        message = "Inheritance structures processing complete"
        if event.result.errors:
            message = "Inheritance structures processing finished with errors"
    ...
    def get_status_text(cls) -> str:
        return "Inheritance structures processing complete"