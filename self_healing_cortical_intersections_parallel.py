...
# Source : tools/self_healing_cortical_intersections_parallel.py
...
DEFAULT_PROMPT_TEMPLATE = """### MULTIPLEX / INHERITANCE MODE (OPT-IN)
Work with multiplex processing, inheritance, and self-healing parallelism.
- Identify parallel anchors, inheritance links, and recovery cues.
...
TOOL_PROMPT = (
    "Use self_healing_cortical_intersections_parallel for multiplex processing, inheritance, or self-healing parallelism. "
    "Provide prompt or messages, and optionally set focus, show_steps, and max_results."
)


class MultiplexMessage(BaseModel):
    role: str
    content: str


class MultiplexArgs(BaseModel):
    prompt: str | None = Field(default=None, description="User prompt to solve.")
    messages: list[MultiplexMessage] | None = Field(
...
class MultiplexResult(BaseModel):
    answer: str
    system_prompt: str
    messages: list[MultiplexMessage]
    used_focus: str
    show_steps: bool
    max_results: int
    template_source: str
...
class MultiplexConfig(BaseToolConfig):
    ...
    default_focus: str = Field(
        default="Self-healing intersections across cortical sectors within parallel processes.",
    ...
    prompt_path: Path | None = Field(
        ...
        / "core"
        / "prompts"
        / "self_healing_cortical_intersections_parallel.md",
        description="Optional path to a prompt template.",
    )
    prompt_max_chars: int = Field(default=8000, description="Maximum template characters to load.")


class MultiplexState(BaseToolState):
    pass


class SelfHealingCorticalIntersectionsParallel(
    BaseTool[
        MultiplexArgs,
        MultiplexResult,
        MultiplexConfig,
    ...
    ToolUIData[MultiplexArgs, MultiplexResult],
):
    description: ClassVar[str] = "Self-healing intersections across cortical sectors within parallel processes."

    @classmethod
    def get_tool_prompt(cls) -> str | None:
        return TOOL_PROMPT

    async def run(self, args: MultiplexArgs) -> MultiplexResult:
        warnings: list[str] = []
        errors: list[str] = []

        focus = (args.focus or self.config.default_focus).strip() or "multiplex processing"
    ...
    def _validate_llm_settings(self, args: MultiplexArgs) -> None:
    ...
    def _normalize_messages(self, args: MultiplexArgs, system_prompt: str) -> list[MultiplexMessage]:
        messages: list[MultiplexMessage] = []
        if args.messages:
            for msg in args.messages:
                ...
                if content:
                    messages.append(MultiplexMessage(role=role, content=content))
        elif args.prompt and args.prompt.strip():
            messages.append(MultiplexMessage(role="user", content=args.prompt.strip()))
        else:
            raise ToolError("Provide prompt or messages.")
        if not messages:
            raise ToolError("No usable messages provided.")
        if system_prompt.strip():
            messages.insert(0, MultiplexMessage(role="system", content=system_prompt.strip()))
        return messages

    def _call_llm(self, messages: list[MultiplexMessage], args: MultiplexArgs) -> str:
    ...
    def _resolve_model(self, args: MultiplexArgs) -> str:
        return args.llm_model or self.config.llm_model

    @classmethod
    def get_call_display(cls, event: ToolCallEvent) -> ToolCallDisplay:
        if not isinstance(event.args, MultiplexArgs):
            return ToolCallDisplay(summary="self_healing_cortical_intersections_parallel")
        return ToolCallDisplay(
            summary="self_healing_cortical_intersections_parallel",
            details={
    ...
    def get_result_display(cls, event: ToolResultEvent) -> ToolResultDisplay:
        if not isinstance(event.result, MultiplexResult):
            return ToolResultDisplay(success=False, message=event.error or event.skip_reason or "No result")
        message = "Multiplex/inheritance processing complete"
        if event.result.errors:
            message = "Multiplex/inheritance processing finished with errors"
    ...
    def get_status_text(cls) -> str:
        return "Multiplex/inheritance processing complete"