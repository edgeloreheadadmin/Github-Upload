...
# Source : tools/membrane_calculations_time_series_forecasting_timelines.py
...
DEFAULT_PROMPT_TEMPLATE = """### MEMBRANE CALCULATIONS MODE (OPT-IN)
Apply membrane-style calculations to memory time series.
- Emphasize temporal structure, gates, and flow constraints.
- Provide concise analysis with confidence notes.
...
TOOL_PROMPT = (
    "Use membrane_calculations_time_series_forecasting_timelines to apply membrane-style calculations "
    "to memory time series. Provide prompt or messages, and optionally set "
    "ocus, show_steps, and max_results."
)
...
class MembraneTimeSeriesConfig(BaseToolConfig):
    ...
    default_focus: str = Field(
        default="Membrane calculations for memory time series forecasting with timelines.",
    ...
    prompt_path: Path | None = Field(
        ...
        / "core"
        / "prompts"
        / "membrane_calculations_time_series_forecasting_timelines.md",
...
class MembraneCalculationsTimeSeriesForecastingTimelines(
    BaseTool[
    ...
    description: ClassVar[str] = "Apply membrane-style calculations for memory time series forecasting with timeline tagging."
    ...
    async def run(self, args: MembraneTimeSeriesArgs) -> MembraneTimeSeriesResult:
        ...
        focus = (args.focus or self.config.default_focus).strip() or "membrane calculations"
    ...
    def get_call_display(cls, event: ToolCallEvent) -> ToolCallDisplay:
        if not isinstance(event.args, MembraneTimeSeriesArgs):
            return ToolCallDisplay(summary="membrane_calculations_time_series_forecasting_timelines")
        return ToolCallDisplay(
            summary="membrane_calculations_time_series_forecasting_timelines",
            details={
    ...
    def get_result_display(cls, event: ToolResultEvent) -> ToolResultDisplay:
        ...
        message = "Membrane calculations complete"
        if event.result.errors:
            message = "Membrane calculations finished with errors"
    ...
    def get_status_text(cls) -> str:
        return "Membrane calculations complete"