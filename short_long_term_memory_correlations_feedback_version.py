...
# Source : tools/short_long_term_memory_correlations_feedback_version.py
...
Work with memory recovery and short/long-term synchronization.
- Identify recovery cues, sync anchors, and feedback measures.
...
TOOL_PROMPT = (
    "Use short_long_term_memory_correlations_feedback_version for memory recovery or short/long-term sync/correlation. "
...
class MemorySyncConfig(BaseToolConfig):
    ...
    default_focus: str = Field(
        default="Short-term and long-term memory correlations and feedback measures implementation.",
    ...
    prompt_path: Path | None = Field(
        ...
        / "core"
        / "prompts"
        / "short_long_term_memory_correlations_feedback_version.md",
...
class ShortLongTermMemoryCorrelationsFeedbackVersion(
    BaseTool[
        MemorySyncArgs,
        MemorySyncResult,
        MemorySyncConfig,
    ...
    description: ClassVar[str] = "Short-term and long-term memory correlations and feedback measures implementation."
    ...
    async def run(self, args: MemorySyncArgs) -> MemorySyncResult:
        ...
        focus = (args.focus or self.config.default_focus).strip() or "memory sync"
    ...
    def _normalize_messages(self, args: MemorySyncArgs, system_prompt: str) -> list[MemorySyncMessage]:
        messages: list[MemorySyncMessage] = []
        if args.messages:
            for msg in args.messages:
                ...
                if content:
                    messages.append(MemorySyncMessage(role=role, content=content))
        elif args.prompt and args.prompt.strip():
            messages.append(MemorySyncMessage(role="user", content=args.prompt.strip()))
        else:
            raise ToolError("Provide prompt or messages.")
        if not messages:
            raise ToolError("No usable messages provided.")
        if system_prompt.strip():
            messages.insert(0, MemorySyncMessage(role="system", content=system_prompt.strip()))
        return messages

    def _call_llm(self, messages: list[MemorySyncMessage], args: MemorySyncArgs) -> str:
    ...
    def get_call_display(cls, event: ToolCallEvent) -> ToolCallDisplay:
        if not isinstance(event.args, MemorySyncArgs):
            return ToolCallDisplay(summary="short_long_term_memory_correlations_feedback_version")
        return ToolCallDisplay(
            summary="short_long_term_memory_correlations_feedback_version",
            details={
    ...
    def get_result_display(cls, event: ToolResultEvent) -> ToolResultDisplay:
        ...
        message = "Memory sync processing complete"
        if event.result.errors:
            message = "Memory sync processing finished with errors"
    ...
    def get_status_text(cls) -> str:
        return "Memory sync processing complete"