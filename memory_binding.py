...
# Source : tools/memory_binding.py
...
DEFAULT_PROMPT_TEMPLATE = """### MEMORY BINDING MODE (OPT-IN)
Bind memories into coherent associations.
- Identify binding anchors and linkage cues.
...
TOOL_PROMPT = (
    "Use `memory_binding` to bind memories into associations. Provide `prompt` "
    "or `messages`, and optionally set `focus`, `show_steps`, and `max_bindings`."
)


class MemoryBindingMessage(BaseModel):
    role: str
    content: str


class MemoryBindingArgs(BaseModel):
    prompt: str | None = Field(
        default=None, description="User prompt to solve."
    )
    messages: list[MemoryBindingMessage] | None = Field(
...
class MemoryBindingResult(BaseModel):
    answer: str
    system_prompt: str
    messages: list[MemoryBindingMessage]
    used_focus: str
    show_steps: bool
    max_bindings: int
    template_source: str
...
class MemoryBindingConfig(BaseToolConfig):
...
class MemoryBindingState(BaseToolState):
    pass
...
class MemoryBinding(
    ...
    ToolUIData[MemoryBindingArgs, MemoryBindingResult],
):
    description: ClassVar[str] = "Bind memories into associations."

    @classmethod
    def get_tool_prompt(cls) -> str | None:
        return TOOL_PROMPT

    async def run(self, args: MemoryBindingArgs) -> MemoryBindingResult:
        warnings: list[str] = []
        errors: list[str] = []

        focus = (args.focus or self.config.default_focus).strip()
        if not focus:
            focus = "memory binding"
    ...
    def _validate_llm_settings(self, args: MemoryBindingArgs) -> None:
    ...
    def _normalize_messages(
        self, args: MemoryBindingArgs, system_prompt: str
    ) -> list[MemoryBindingMessage]:
        messages: list[MemoryBindingMessage] = []
        if args.messages:
            for msg in args.messages:
                ...
                if not role:
                    role = "user"
                messages.append(
                    MemoryBindingMessage(role=role, content=content)
                )
        elif args.prompt and args.prompt.strip():
            messages.append(
                MemoryBindingMessage(
    ...
    def _resolve_model(self, args: MemoryBindingArgs) -> str:
        return args.llm_model or self.config.llm_model

    @classmethod
    def get_call_display(cls, event: ToolCallEvent) -> ToolCallDisplay:
        if not isinstance(event.args, MemoryBindingArgs):
            return ToolCallDisplay(summary="memory_binding")
        return ToolCallDisplay(
            summary="memory_binding",
            details={
    ...
    def get_result_display(
        ...
        if not isinstance(event.result, MemoryBindingResult):
            return ToolResultDisplay(
                success=False,
                message=event.error or event.skip_reason or "No result",
            )
        message = "Memory binding complete"
        if event.result.errors:
            message = "Memory binding finished with errors"
        return ToolResultDisplay(
