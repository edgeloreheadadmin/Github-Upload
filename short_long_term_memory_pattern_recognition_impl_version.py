...
# Source : tools/short_long_term_memory_pattern_recognition_impl_version.py
...
DEFAULT_PROMPT_TEMPLATE = """### MEMORY RATE/RECOGNITION MODE (OPT-IN)
Work with memory pattern/frequency recognition, retention rate, and synchronization.
- Identify recognition anchors, rate signals, and sync cues.
...
TOOL_PROMPT = (
    "Use short_long_term_memory_pattern_recognition_impl_version for memory recognition, retention rate, or synchronization. "
    "Provide prompt or messages, and optionally set focus, show_steps, and max_results."
)


class MemoryRateMessage(BaseModel):
    role: str
    content: str


class MemoryRateArgs(BaseModel):
    prompt: str | None = Field(default=None, description="User prompt to solve.")
    messages: list[MemoryRateMessage] | None = Field(
...
class MemoryRateResult(BaseModel):
    answer: str
    system_prompt: str
    messages: list[MemoryRateMessage]
    used_focus: str
    show_steps: bool
    max_results: int
    template_source: str
...
class MemoryRateConfig(BaseToolConfig):
    ...
    default_focus: str = Field(
        default="Short/long-term memory pattern recognition implementation.",
    ...
    prompt_path: Path | None = Field(
        ...
        / "core"
        / "prompts"
        / "short_long_term_memory_pattern_recognition_impl_version.md",
        description="Optional path to a prompt template.",
    )
    prompt_max_chars: int = Field(default=8000, description="Maximum template characters to load.")


class MemoryRateState(BaseToolState):
    pass


class ShortLongTermMemoryPatternRecognitionImplVersion(
    BaseTool[
        MemoryRateArgs,
        MemoryRateResult,
        MemoryRateConfig,
    ...
    ToolUIData[MemoryRateArgs, MemoryRateResult],
):
    description: ClassVar[str] = "Short/long-term memory pattern recognition implementation."

    @classmethod
    def get_tool_prompt(cls) -> str | None:
        return TOOL_PROMPT

    async def run(self, args: MemoryRateArgs) -> MemoryRateResult:
        warnings: list[str] = []
        errors: list[str] = []

        focus = (args.focus or self.config.default_focus).strip() or "memory rates"
    ...
    def _validate_llm_settings(self, args: MemoryRateArgs) -> None:
    ...
    def _normalize_messages(self, args: MemoryRateArgs, system_prompt: str) -> list[MemoryRateMessage]:
        messages: list[MemoryRateMessage] = []
        if args.messages:
            for msg in args.messages:
                ...
                if content:
                    messages.append(MemoryRateMessage(role=role, content=content))
        elif args.prompt and args.prompt.strip():
            messages.append(MemoryRateMessage(role="user", content=args.prompt.strip()))
        else:
            raise ToolError("Provide prompt or messages.")
        if not messages:
            raise ToolError("No usable messages provided.")
        if system_prompt.strip():
            messages.insert(0, MemoryRateMessage(role="system", content=system_prompt.strip()))
        return messages

    def _call_llm(self, messages: list[MemoryRateMessage], args: MemoryRateArgs) -> str:
    ...
    def _resolve_model(self, args: MemoryRateArgs) -> str:
        return args.llm_model or self.config.llm_model

    @classmethod
    def get_call_display(cls, event: ToolCallEvent) -> ToolCallDisplay:
        if not isinstance(event.args, MemoryRateArgs):
            return ToolCallDisplay(summary="short_long_term_memory_pattern_recognition_impl_version")
        return ToolCallDisplay(
            summary="short_long_term_memory_pattern_recognition_impl_version",
            details={
    ...
    def get_result_display(cls, event: ToolResultEvent) -> ToolResultDisplay:
        if not isinstance(event.result, MemoryRateResult):
            return ToolResultDisplay(success=False, message=event.error or event.skip_reason or "No result")
        message = "Memory rate processing complete"
        if event.result.errors:
            message = "Memory rate processing finished with errors"
    ...
    def get_status_text(cls) -> str:
        return "Memory rate processing complete"