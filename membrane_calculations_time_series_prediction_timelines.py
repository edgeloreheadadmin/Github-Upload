...
# Source : tools/membrane_calculations_time_series_prediction_timelines.py
...
DEFAULT_PROMPT_TEMPLATE = """### MEMORY TIMELINE CLASSIFICATION MODE (OPT-IN)
Work with memory timeline structure and time-based metadata.
- Identify timeline anchors, segments, and recall cues.
- Provide concise classification guidance with confidence notes.
...
TOOL_PROMPT = (
    "Use membrane_calculations_time_series_prediction_timelines to classify memory timeline data or apply timeline-tagged calculations. "
...
class MemoryTimelineConfig(BaseToolConfig):
    ...
    default_focus: str = Field(
        default="Membrane calculations for memory time series prediction with timelines.",
    ...
    prompt_path: Path | None = Field(
        ...
        / "core"
        / "prompts"
        / "membrane_calculations_time_series_prediction_timelines.md",
...
class MembraneCalculationsTimeSeriesPredictionTimelines(
    BaseTool[
        MemoryTimelineArgs,
    ...
    description: ClassVar[str] = "Apply membrane-style calculations for memory time series prediction with timeline tagging."
    ...
    def get_call_display(cls, event: ToolCallEvent) -> ToolCallDisplay:
        if not isinstance(event.args, MemoryTimelineArgs):
            return ToolCallDisplay(summary="membrane_calculations_time_series_prediction_timelines")
        return ToolCallDisplay(
            summary="membrane_calculations_time_series_prediction_timelines",
            details={
