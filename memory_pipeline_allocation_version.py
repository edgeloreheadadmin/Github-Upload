...
# Source : tools/memory_pipeline_allocation_version.py
...
DEFAULT_PROMPT_TEMPLATE = """### MEMORY CACHING/PIPELINE MODE (OPT-IN)
Work with memory caching, pipeline allocation, and memory typing.
- Identify cache anchors, pipeline stages, and type boundaries.
...
TOOL_PROMPT = (
    "Use memory_pipeline_allocation_version for memory caching, pipeline allocation, or memory type creation. "
    "Provide prompt or messages, and optionally set focus, show_steps, and max_results."
)


class MemoryCachingMessage(BaseModel):
    role: str
    content: str


class MemoryCachingArgs(BaseModel):
    prompt: str | None = Field(default=None, description="User prompt to solve.")
    messages: list[MemoryCachingMessage] | None = Field(
...
class MemoryCachingResult(BaseModel):
    answer: str
    system_prompt: str
    messages: list[MemoryCachingMessage]
    used_focus: str
    show_steps: bool
    max_results: int
    template_source: str
...
class MemoryCachingConfig(BaseToolConfig):
    ...
    prompt_path: Path | None = Field(
        ...
        / "core"
        / "prompts"
        / "memory_pipeline_allocation_version.md",
        description="Optional path to a prompt template.",
    )
    prompt_max_chars: int = Field(default=8000, description="Maximum template characters to load.")


class MemoryCachingState(BaseToolState):
    pass


class MemoryPipelineAllocationVersion(
    BaseTool[
        MemoryCachingArgs,
        MemoryCachingResult,
        MemoryCachingConfig,
    ...
    ToolUIData[MemoryCachingArgs, MemoryCachingResult],
):
    description: ClassVar[str] = "Memory pipeline allocation."

    @classmethod
    def get_tool_prompt(cls) -> str | None:
        return TOOL_PROMPT

    async def run(self, args: MemoryCachingArgs) -> MemoryCachingResult:
        warnings: list[str] = []
        errors: list[str] = []

        focus = (args.focus or self.config.default_focus).strip() or "memory caching"
    ...
    def _validate_llm_settings(self, args: MemoryCachingArgs) -> None:
    ...
    def _normalize_messages(self, args: MemoryCachingArgs, system_prompt: str) -> list[MemoryCachingMessage]:
        messages: list[MemoryCachingMessage] = []
        if args.messages:
            for msg in args.messages:
                ...
                if content:
                    messages.append(MemoryCachingMessage(role=role, content=content))
        elif args.prompt and args.prompt.strip():
            messages.append(MemoryCachingMessage(role="user", content=args.prompt.strip()))
        else:
            raise ToolError("Provide prompt or messages.")
        if not messages:
            raise ToolError("No usable messages provided.")
        if system_prompt.strip():
            messages.insert(0, MemoryCachingMessage(role="system", content=system_prompt.strip()))
        return messages

    def _call_llm(self, messages: list[MemoryCachingMessage], args: MemoryCachingArgs) -> str:
    ...
    def _resolve_model(self, args: MemoryCachingArgs) -> str:
        return args.llm_model or self.config.llm_model

    @classmethod
    def get_call_display(cls, event: ToolCallEvent) -> ToolCallDisplay:
        if not isinstance(event.args, MemoryCachingArgs):
            return ToolCallDisplay(summary="memory_pipeline_allocation_version")
        return ToolCallDisplay(
            summary="memory_pipeline_allocation_version",
            details={
    ...
    def get_result_display(cls, event: ToolResultEvent) -> ToolResultDisplay:
        if not isinstance(event.result, MemoryCachingResult):
            return ToolResultDisplay(success=False, message=event.error or event.skip_reason or "No result")
        message = "Memory caching processing complete"
        if event.result.errors:
            message = "Memory caching processing finished with errors"
    ...
    def get_status_text(cls) -> str:
        return "Memory caching processing complete"