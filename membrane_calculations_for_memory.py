...
# Source : tools/membrane_calculations_for_memory.py
...
DEFAULT_PROMPT_TEMPLATE = """### MEMBRANE CALCULATIONS FOR MEMORY MODE (OPT-IN)
Apply membrane-style calculations to memory structures.
- Identify memory boundaries, gates, and flow constraints.
...
TOOL_PROMPT = (
    "Use `membrane_calculations_for_memory` to apply membrane-style calculations "
    "to memory structures. Provide `prompt` or `messages`, and optionally set "
    "`focus`, `show_steps`, and `max_results`."
)


class MembraneMemoryMessage(BaseModel):
    role: str
    content: str


class MembraneMemoryArgs(BaseModel):
    prompt: str | None = Field(default=None, description="User prompt to solve.")
    messages: list[MembraneMemoryMessage] | None = Field(
...
class MembraneMemoryResult(BaseModel):
    answer: str
    system_prompt: str
    messages: list[MembraneMemoryMessage]
    used_focus: str
    show_steps: bool
    max_results: int
    template_source: str
...
class MembraneMemoryConfig(BaseToolConfig):
    ...
    default_focus: str = Field(
        default="Membrane calculations for memory.",
    ...
    prompt_path: Path | None = Field(
        ...
        / "core"
        / "prompts"
        / "membrane_calculations_for_memory.md",
        description="Optional path to a prompt template.",
    )
    prompt_max_chars: int = Field(default=8000, description="Maximum template characters to load.")


class MembraneMemoryState(BaseToolState):
    pass


class MembraneCalculationsForMemory(
    BaseTool[
        MembraneMemoryArgs,
        MembraneMemoryResult,
        MembraneMemoryConfig,
        MembraneMemoryState,
    ],
    ToolUIData[MembraneMemoryArgs, MembraneMemoryResult],
):
    description: ClassVar[str] = "Apply membrane-style calculations to memory structures."

    @classmethod
    def get_tool_prompt(cls) -> str | None:
        return TOOL_PROMPT

    async def run(self, args: MembraneMemoryArgs) -> MembraneMemoryResult:
        warnings: list[str] = []
        errors: list[str] = []
    ...
    def _validate_llm_settings(self, args: MembraneMemoryArgs) -> None:
    ...
    def _normalize_messages(self, args: MembraneMemoryArgs, system_prompt: str) -> list[MembraneMemoryMessage]:
        messages: list[MembraneMemoryMessage] = []
        if args.messages:
            for msg in args.messages:
                ...
                if content:
                    messages.append(MembraneMemoryMessage(role=role, content=content))
        elif args.prompt and args.prompt.strip():
            messages.append(MembraneMemoryMessage(role="user", content=args.prompt.strip()))
        else:
            raise ToolError("Provide prompt or messages.")
        if not messages:
            raise ToolError("No usable messages provided.")
        if system_prompt.strip():
            messages.insert(0, MembraneMemoryMessage(role="system", content=system_prompt.strip()))
        return messages

    def _call_llm(self, messages: list[MembraneMemoryMessage], args: MembraneMemoryArgs) -> str:
    ...
    def _resolve_model(self, args: MembraneMemoryArgs) -> str:
        return args.llm_model or self.config.llm_model

    @classmethod
    def get_call_display(cls, event: ToolCallEvent) -> ToolCallDisplay:
        if not isinstance(event.args, MembraneMemoryArgs):
            return ToolCallDisplay(summary="membrane_calculations_for_memory")
        return ToolCallDisplay(
            summary="membrane_calculations_for_memory",
            details={
    ...
    def get_result_display(cls, event: ToolResultEvent) -> ToolResultDisplay:
        if not isinstance(event.result, MembraneMemoryResult):
            return ToolResultDisplay(success=False, message=event.error or event.skip_reason or "No result")
        message = "Membrane calculations for memory complete"
        if event.result.errors:
            message = "Membrane calculations for memory finished with errors"
    ...
    def get_status_text(cls) -> str:
        return "Membrane calculations for memory"