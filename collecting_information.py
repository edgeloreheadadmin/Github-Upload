...
# Source : tools/collecting_information.py
...
DEFAULT_PROMPT_TEMPLATE = """### COLLECTING INFORMATION MODE (OPT-IN)
Collect and consolidate relevant information before answering.
- Gather key facts, constraints, and context from provided sources.
- Flag missing data or conflicting details.
- Keep internal collection private; do not mention this mode in the response.
...
TOOL_PROMPT = (
    "Use `collecting_information` to consolidate facts and context before answering. "
    "Provide `prompt` or `messages`, and optionally set `collection_style`, "
    "`show_steps`, and `max_items` to control the collection behavior."
)
...
class CollectingInformationMessage(BaseModel):
    role: str
    content: str


class CollectionSourceItem(BaseModel):
...
class CollectingInformationArgs(BaseModel):
    prompt: str | None = Field(
        default=None, description="User prompt to solve."
    )
    messages: list[CollectingInformationMessage] | None = Field(
        default=None, description="Optional chat messages."
    )
    system_prompt: str | None = Field(
        default=None, description="Optional system prompt prefix."
    )
    collection_style: str | None = Field(
        default=None, description="Collection style label."
    )
    ...
    sources: list[CollectionSourceItem] | None = Field(
        default=None, description="Collection source items."
    )
...
class CollectingInformationResult(BaseModel):
    answer: str
    system_prompt: str
    messages: list[CollectingInformationMessage]
    used_collection_style: str
    show_steps: bool
    max_items: int
    template_source: str
...
class CollectingInformationConfig(BaseToolConfig):
    ...
    default_collection_style: str = Field(
        default="systematic",
        description="Default collection style.",
    )
...
class CollectingInformationState(BaseToolState):
    pass


class CollectingInformation(
    BaseTool[
        CollectingInformationArgs,
        CollectingInformationResult,
        CollectingInformationConfig,
        CollectingInformationState,
    ],
    ToolUIData[CollectingInformationArgs, CollectingInformationResult],
):
    description: ClassVar[str] = (
        "Collect and consolidate information using a dedicated prompt template."
    )
    ...
    async def run(
        self, args: CollectingInformationArgs
    ) -> CollectingInformationResult:
        warnings: list[str] = []
        errors: list[str] = []

        collection_style = (
            args.collection_style or self.config.default_collection_style
        ).strip()
        if not collection_style:
            collection_style = "systematic"
        ...
        return CollectingInformationResult(
            answer=answer,
            system_prompt=system_prompt,
            messages=messages,
            used_collection_style=collection_style,
    ...
    def _validate_llm_settings(self, args: CollectingInformationArgs) -> None:
    ...
    def _collect_sources(
        self, args: CollectingInformationArgs, warnings: list[str]
    ) -> list[SourceBlock]:
        max_item = (
    ...
    def _load_source_item(self, item: CollectionSourceItem) -> _SourceItem:
    ...
    def _inject_sources(
        ...
        messages: list[CollectingInformationMessage],
        blocks: list[SourceBlock],
    ) -> list[CollectingInformationMessage]:
        if not blocks:
            return messages
        ...
        for idx in range(len(messages) - 1, -1, -1):
            if messages[idx].role == "user":
                ...
                return messages
        messages.append(CollectingInformationMessage(role="user", content=source_text))
        return messages
    ...
    def _render_template(
        ...
        had_placeholders = (
            "{collection_style}" in template
            or "{show_steps}" in template
            or "{max_items}" in template
        )
        rendered = template
        if "{collection_style}" in template:
            rendered = rendered.replace("{collection_style}", collection_style)
        ...
        extra = (
            f"Collection style: {collection_style}\n"
    ...
    def _normalize_messages(
        self, args: CollectingInformationArgs, system_prompt: str
    ) -> list[CollectingInformationMessage]:
        messages: list[CollectingInformationMessage] = []
        if args.messages:
            for msg in args.messages:
                ...
                if not role:
                    role = "user"
                messages.append(
                    CollectingInformationMessage(role=role, content=content)
                )
        elif args.prompt and args.prompt.strip():
            messages.append(
                CollectingInformationMessage(
    ...
    def _call_llm(
        self,
        messages: list[CollectingInformationMessage],
        args: CollectingInformationArgs,
    ) -> str:
    ...
    def _resolve_model(self, args: CollectingInformationArgs) -> str:
        return args.llm_model or self.config.llm_model

    @classmethod
    def get_call_display(cls, event: ToolCallEvent) -> ToolCallDisplay:
        if not isinstance(event.args, CollectingInformationArgs):
            return ToolCallDisplay(summary="collecting_information")
        return ToolCallDisplay(
            summary="collecting_information",
            details={
                "collection_style": event.args.collection_style,
    ...
    def get_result_display(cls, event: ToolResultEvent) -> ToolResultDisplay:
        if not isinstance(event.result, CollectingInformationResult):
            return ToolResultDisplay(
                success=False, message=event.error or event.skip_reason or "No result"
            )
        message = "Collecting information complete"
        if event.result.errors:
            message = "Collecting information finished with errors"
        return ToolResultDisplay(
            ...
            details={
                ...
                "used_collection_style": event.result.used_collection_style,
