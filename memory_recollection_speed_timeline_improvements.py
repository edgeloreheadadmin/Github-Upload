...
# Source : tools/memory_recollection_speed_timeline_improvements.py
...
DEFAULT_PROMPT_TEMPLATE = """### MEMORY RECOLLECTION TIMELINE MODE (OPT-IN)
Work with memory recollection cues and time-position metadata.
- Identify timeline anchors, segments, and recall cues.
- Provide concise classification or improvement guidance with confidence notes.
...
TOOL_PROMPT = (
    "Use memory_recollection_speed_timeline_improvements to classify or analyze memory recollection using timeline/timeframe data. "
    "Provide prompt or messages, and optionally set focus, show_steps, and max_results."
)


class MemoryRecollectionMessage(BaseModel):
    role: str
    content: str


class MemoryRecollectionArgs(BaseModel):
    prompt: str | None = Field(default=None, description="User prompt to solve.")
    messages: list[MemoryRecollectionMessage] | None = Field(
...
class MemoryRecollectionResult(BaseModel):
    answer: str
    system_prompt: str
    messages: list[MemoryRecollectionMessage]
    used_focus: str
    show_steps: bool
    max_results: int
    template_source: str
...
class MemoryRecollectionConfig(BaseToolConfig):
    ...
    default_focus: str = Field(
        default="Improve memory recollection speed based on timeline cues.",
    ...
    prompt_path: Path | None = Field(
        ...
        / "core"
        / "prompts"
        / "memory_recollection_speed_timeline_improvements.md",
        description="Optional path to a prompt template.",
    )
    prompt_max_chars: int = Field(default=8000, description="Maximum template characters to load.")


class MemoryRecollectionState(BaseToolState):
    pass


class MemoryRecollectionSpeedTimelineImprovements(
    BaseTool[
        MemoryRecollectionArgs,
        MemoryRecollectionResult,
        MemoryRecollectionConfig,
        MemoryRecollectionState,
    ],
    ToolUIData[MemoryRecollectionArgs, MemoryRecollectionResult],
):
    description: ClassVar[str] = "Improve memory recollection speed using timeline signals."

    @classmethod
    def get_tool_prompt(cls) -> str | None:
        return TOOL_PROMPT

    async def run(self, args: MemoryRecollectionArgs) -> MemoryRecollectionResult:
        warnings: list[str] = []
        errors: list[str] = []

        focus = (args.focus or self.config.default_focus).strip() or "memory recollection timeline"
        ...
        return MemoryRecollectionResult(
            answer=answer,
    ...
    def _validate_llm_settings(self, args: MemoryRecollectionArgs) -> None:
    ...
    def _normalize_messages(self, args: MemoryRecollectionArgs, system_prompt: str) -> list[MemoryRecollectionMessage]:
        messages: list[MemoryRecollectionMessage] = []
        if args.messages:
            for msg in args.messages:
                ...
                if content:
                    messages.append(MemoryRecollectionMessage(role=role, content=content))
        elif args.prompt and args.prompt.strip():
            messages.append(MemoryRecollectionMessage(role="user", content=args.prompt.strip()))
        else:
            raise ToolError("Provide prompt or messages.")
        if not messages:
            raise ToolError("No usable messages provided.")
        if system_prompt.strip():
            messages.insert(0, MemoryRecollectionMessage(role="system", content=system_prompt.strip()))
        return messages

    def _call_llm(self, messages: list[MemoryRecollectionMessage], args: MemoryRecollectionArgs) -> str:
    ...
    def _resolve_model(self, args: MemoryRecollectionArgs) -> str:
        return args.llm_model or self.config.llm_model

    @classmethod
    def get_call_display(cls, event: ToolCallEvent) -> ToolCallDisplay:
        if not isinstance(event.args, MemoryRecollectionArgs):
            return ToolCallDisplay(summary="memory_recollection_speed_timeline_improvements")
        return ToolCallDisplay(
            summary="memory_recollection_speed_timeline_improvements",
            details={
    ...
    def get_result_display(cls, event: ToolResultEvent) -> ToolResultDisplay:
        if not isinstance(event.result, MemoryRecollectionResult):
            return ToolResultDisplay(success=False, message=event.error or event.skip_reason or "No result")
        message = "Memory recollection processing complete"
        if event.result.errors:
            message = "Memory recollection processing finished with errors"
    ...
    def get_status_text(cls) -> str:
        return "Memory recollection processing complete"