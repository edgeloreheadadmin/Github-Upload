...
# Source : tools/inherited_elements_fields_attributes.py
...
DEFAULT_PROMPT_TEMPLATE = """### INHERITED ABILITIES MODE (OPT-IN)
Work with inherited abilities, structs, techniques, and attributes.
- Identify inheritance paths, repetition cycles, and transfer points.
...
TOOL_PROMPT = (
    "Use inherited_elements_fields_attributes for inherited abilities, structs, techniques, or attributes. "
    "Provide prompt or messages, and optionally set focus, show_steps, and max_results."
)


class InheritedAbilityMessage(BaseModel):
    role: str
    content: str


class InheritedAbilityArgs(BaseModel):
    prompt: str | None = Field(default=None, description="User prompt to solve.")
    messages: list[InheritedAbilityMessage] | None = Field(
...
class InheritedAbilityResult(BaseModel):
    answer: str
    system_prompt: str
    messages: list[InheritedAbilityMessage]
    used_focus: str
    show_steps: bool
    max_results: int
    template_source: str
...
class InheritedAbilityConfig(BaseToolConfig):
    ...
    default_focus: str = Field(
        default="Use inherited elements, fields, and attributes passed between structs and subclasses.",
    ...
    prompt_path: Path | None = Field(
        ...
        / "core"
        / "prompts"
        / "inherited_elements_fields_attributes.md",
        description="Optional path to a prompt template.",
    )
    prompt_max_chars: int = Field(default=8000, description="Maximum template characters to load.")


class InheritedAbilityState(BaseToolState):
    pass


class InheritedElementsFieldsAttributes(
    BaseTool[
        InheritedAbilityArgs,
        InheritedAbilityResult,
        InheritedAbilityConfig,
        InheritedAbilityState,
    ],
    ToolUIData[InheritedAbilityArgs, InheritedAbilityResult],
):
    description: ClassVar[str] = "Use inherited elements, fields, and attributes passed between structs and subclasses."

    @classmethod
    def get_tool_prompt(cls) -> str | None:
        return TOOL_PROMPT

    async def run(self, args: InheritedAbilityArgs) -> InheritedAbilityResult:
        warnings: list[str] = []
        errors: list[str] = []

        focus = (args.focus or self.config.default_focus).strip() or "inherited abilities"
    ...
    def _validate_llm_settings(self, args: InheritedAbilityArgs) -> None:
    ...
    def _normalize_messages(self, args: InheritedAbilityArgs, system_prompt: str) -> list[InheritedAbilityMessage]:
        messages: list[InheritedAbilityMessage] = []
        if args.messages:
            for msg in args.messages:
                ...
                if content:
                    messages.append(InheritedAbilityMessage(role=role, content=content))
        elif args.prompt and args.prompt.strip():
            messages.append(InheritedAbilityMessage(role="user", content=args.prompt.strip()))
        else:
            raise ToolError("Provide prompt or messages.")
        if not messages:
            raise ToolError("No usable messages provided.")
        if system_prompt.strip():
            messages.insert(0, InheritedAbilityMessage(role="system", content=system_prompt.strip()))
        return messages

    def _call_llm(self, messages: list[InheritedAbilityMessage], args: InheritedAbilityArgs) -> str:
    ...
    def _resolve_model(self, args: InheritedAbilityArgs) -> str:
        return args.llm_model or self.config.llm_model

    @classmethod
    def get_call_display(cls, event: ToolCallEvent) -> ToolCallDisplay:
        if not isinstance(event.args, InheritedAbilityArgs):
            return ToolCallDisplay(summary="inherited_elements_fields_attributes")
        return ToolCallDisplay(
            summary="inherited_elements_fields_attributes",
            details={
    ...
    def get_result_display(cls, event: ToolResultEvent) -> ToolResultDisplay:
        if not isinstance(event.result, InheritedAbilityResult):
            return ToolResultDisplay(success=False, message=event.error or event.skip_reason or "No result")
        message = "Inherited abilities processing complete"
        if event.result.errors:
            message = "Inherited abilities processing finished with errors"
    ...
    def get_status_text(cls) -> str:
        return "Inherited abilities processing complete"