...
# Source : tools/memory_timeline_time_series_forecast_analysis_prediction_version.py
...
Work with time-series timeline development, recollection, and spatial remembrance.
- Identify time-series anchors, recollection cues, and timeline event bindings.
...
TOOL_PROMPT = (
    "Use memory_timeline_time_series_forecast_analysis_prediction_version for time-series timeline development or recollection. "
    "Provide prompt or messages, and optionally set focus, show_steps, and max_results."
)


class TimelineSeriesMessage(BaseModel):
    role: str
    content: str


class TimelineSeriesArgs(BaseModel):
    prompt: str | None = Field(default=None, description="User prompt to solve.")
    messages: list[TimelineSeriesMessage] | None = Field(
...
class TimelineSeriesResult(BaseModel):
    answer: str
    system_prompt: str
    messages: list[TimelineSeriesMessage]
    used_focus: str
    show_steps: bool
    max_results: int
    template_source: str
...
class TimelineSeriesConfig(BaseToolConfig):
    ...
    default_focus: str = Field(
        default="Memory timeline development using time series forecast/analysis/prediction for time-stamped retrieval.",
    ...
    prompt_path: Path | None = Field(
        ...
        / "core"
        / "prompts"
        / "memory_timeline_time_series_forecast_analysis_prediction_version.md",
        description="Optional path to a prompt template.",
    )
    prompt_max_chars: int = Field(default=8000, description="Maximum template characters to load.")


class TimelineSeriesState(BaseToolState):
    pass


class MemoryTimelineTimeSeriesForecastAnalysisPredictionVersion(
    BaseTool[
        TimelineSeriesArgs,
    ...
    ToolUIData[TimelineSeriesArgs, TimelineSeriesResult],
):
    description: ClassVar[str] = "Memory timeline development using time series forecast/analysis/prediction for time-stamped retrieval."

    @classmethod
    def get_tool_prompt(cls) -> str | None:
        return TOOL_PROMPT

    async def run(self, args: TimelineSeriesArgs) -> TimelineSeriesResult:
        warnings: list[str] = []
        errors: list[str] = []

        focus = (args.focus or self.config.default_focus).strip() or "timeline series"
    ...
    def _validate_llm_settings(self, args: TimelineSeriesArgs) -> None:
    ...
    def _normalize_messages(self, args: TimelineSeriesArgs, system_prompt: str) -> list[TimelineSeriesMessage]:
        messages: list[TimelineSeriesMessage] = []
        if args.messages:
            for msg in args.messages:
                ...
                if content:
                    messages.append(TimelineSeriesMessage(role=role, content=content))
        elif args.prompt and args.prompt.strip():
            messages.append(TimelineSeriesMessage(role="user", content=args.prompt.strip()))
        else:
            raise ToolError("Provide prompt or messages.")
        if not messages:
            raise ToolError("No usable messages provided.")
        if system_prompt.strip():
            messages.insert(0, TimelineSeriesMessage(role="system", content=system_prompt.strip()))
        return messages

    def _call_llm(self, messages: list[TimelineSeriesMessage], args: TimelineSeriesArgs) -> str:
    ...
    def _resolve_model(self, args: TimelineSeriesArgs) -> str:
        return args.llm_model or self.config.llm_model

    @classmethod
    def get_call_display(cls, event: ToolCallEvent) -> ToolCallDisplay:
        if not isinstance(event.args, TimelineSeriesArgs):
            return ToolCallDisplay(summary="memory_timeline_time_series_forecast_analysis_prediction_version")
        return ToolCallDisplay(
            summary="memory_timeline_time_series_forecast_analysis_prediction_version",
            details={
    ...
    def get_result_display(cls, event: ToolResultEvent) -> ToolResultDisplay:
        if not isinstance(event.result, TimelineSeriesResult):
            return ToolResultDisplay(success=False, message=event.error or event.skip_reason or "No result")
        message = "Timeline series processing complete"
        if event.result.errors:
            message = "Timeline series processing finished with errors"
    ...
    def get_status_text(cls) -> str:
        return "Timeline series processing complete"